...
$glslc = Join-Path $PSScriptRoot 'ext\shaderc\build\glslc\glslc.exe'
if (-not (Test-Path -LiteralPath $glslc)) {
    Write-Host '[WARN] glslc not found - harness SPIR-V embed stays stub (zero length).'
    exit 0
}

$shDir = Join-Path $PSScriptRoot 'tests\harness\shaders'
$outDir = Join-Path $PSScriptRoot 'tests\harness\spirv_out'
...
& (Join-Path $PSScriptRoot 'scripts\gen_spirv_embed.ps1') `
    ...
    -OutC (Join-Path $PSScriptRoot 'tests\harness\sit_harness_spirv_gl_embed.c') `
...
& (Join-Path $PSScriptRoot 'scripts\gen_spirv_embed.ps1') `
    ...
    -OutC (Join-Path $PSScriptRoot 'tests\harness\sit_harness_spirv_vk_embed.c') `
    -HeaderInclude 'sit_harness_spirv_embed.h' `
