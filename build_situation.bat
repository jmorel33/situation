...
REM   SIT_OPTIMIZE_CFLAGS - Default: -O2 -mfma -ffp-contract=fast (set empty for -O0 debug DLL)
REM
...
REM Release CPU opts for YPQ/FMA hot paths (override: set SIT_OPTIMIZE_CFLAGS=)
...
REM NOTE: Each ^ must be immediately followed by the next line. Blank lines between
REM continuations make cmd pass empty arguments (gcc: linker input file not found).
...
REM NOTE: Each ^ must be immediately followed by the next line. Blank lines between
REM continuations make cmd pass empty arguments (g++: linker input file not found).
...
gcc -c "%DLL_SRC%" ^
    ...
    -Isit\k-term ^
    -I"%VULKAN_SDK%\Include" ^
    -Iext\shaderc\libshaderc\include ^
...
echo.
echo ========================================
echo  Situation Static Lib Build - OpenGL
echo  Compiler: GCC %GCC_VER% (C11)
echo  Backend:  OpenGL
echo ========================================
echo.
...
REM Step 2: Compile Situation (OpenGL, no SITUATION_BUILD_SHARED)
echo [2/3] Compiling Situation (C11 - OpenGL, static)...
gcc -c "%DLL_SRC%" ^
    -o "%BUILD_DIR%\situation_static_opengl.o" ^
    -std=c11 ^
    %SIT_OPTIMIZE_CFLAGS% ^
    -msse -msse2 -msse4.1 ^
    -I. ^
    ...
    -DKTERM_IMPLEMENTATION
if errorlevel 1 (
    echo [FAILED] Situation static compilation failed!
    exit /b 1
)
echo       OK

REM Step 3: Archive into static library
echo [3/3] Archiving situation_opengl.a...
ar rcs "%BUILD_DIR%\situation_opengl.a" ^
    "%BUILD_DIR%\situation_static_opengl.o" ^
    "%BUILD_DIR%\tinycthread.o"
if errorlevel 1 (
    echo [FAILED] Archiving failed!
    exit /b 1
)
echo       OK
echo.
...
echo.
echo ========================================
echo  Situation Static Lib Build - Vulkan
echo  Compiler:   GCC %GCC_VER% (C11)
echo  Vulkan SDK: %VULKAN_SDK%
echo  Backend:    Vulkan
echo ========================================
...
REM Step 1: Compile VMA wrapper
echo [1/4] Compiling VMA wrapper (C++)...
g++ -c "%VMA_SRC%" ^
    -o "%BUILD_DIR%\vma_wrapper.o" ^
    -std=c++11 ^
...
REM Step 3: Compile Situation (Vulkan, no SITUATION_BUILD_SHARED)
echo [3/4] Compiling Situation (C11 - Vulkan, static)...
gcc -c "%DLL_SRC%" ^
    -o "%BUILD_DIR%\situation_static_vulkan.o" ^
    -std=c11 ^
    %SIT_OPTIMIZE_CFLAGS% ^
    -msse -msse2 -msse4.1 ^
    -I. ^
...
REM Step 4: Archive into static library
echo [4/4] Archiving situation_vulkan.a...
ar rcs "%BUILD_DIR%\situation_vulkan.a" ^
    "%BUILD_DIR%\situation_static_vulkan.o" ^
    "%BUILD_DIR%\vma_wrapper.o" ^
    "%BUILD_DIR%\tinycthread.o"
if errorlevel 1 (
    echo [FAILED] Archiving failed!
...
echo  Output: %BUILD_DIR%\situation_opengl.a
echo  Link examples with: build_examples.bat static-opengl ^<name^>
echo ========================================
exit /b 0
...
echo ========================================
echo  Static build complete.
echo  Output: %BUILD_DIR%\situation_vulkan.a
echo  Link examples with: build_examples.bat static-vulkan ^<name^>
echo ========================================
exit /b 0
...
echo.
echo Targets:
echo   opengl          - Build OpenGL backend DLL (situation_opengl.dll)
echo   vulkan          - Build Vulkan backend DLL (situation_vulkan.dll)
echo   all             - Build both DLLs
echo   static-opengl   - Build OpenGL static library (situation_opengl.a)
echo   static-vulkan   - Build Vulkan static library (situation_vulkan.a)
