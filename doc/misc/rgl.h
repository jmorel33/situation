...
static bool _RGL_UploadGrayscaleAsRGBATexture(int width, int height, const unsigned char* grayscale,
    bool generate_mipmaps, SituationTexture* out_texture) {
    if (!grayscale || width <= 0 || height <= 0 || !out_texture) {
        return false;
    }
    ...
    for (size_t i = 0; i < pixel_count; i++) {
        unsigned char g = grayscale[i];
        rgba[i * 4 + 0] = g;
        rgba[i * 4 + 1] = g;
        rgba[i * 4 + 2] = g;
        rgba[i * 4 + 3] = 255;
    }
    bool ok = _RGL_UploadTextureFromPixels(width, height, 4, rgba, generate_mipmaps, out_texture);
    free(rgba);
    return ok;
}
...
static void _RGL_DrawDebugText(const char* text, int x, int y, int size, Color color) {
    ...
    for (const char* p = text; *p; p++) {
        ...
        RGL_DrawSpritePro(
            glyph_sprite,
            (Vector3){{.x = (float)x, .y = (float)y, .z = 0.0f}},
            _RGL_V2(char_size),
            (Vector2){{0.0f, 0.0f}},
...
SITAPI RGLBitmapFont RGL_CreateBitmapFontFromSystemFont(const char* font_name, int font_size, int char_width, int char_height) {
    ...
    if (!_RGL_UploadTextureFromPixels(atlas_width, atlas_height, 4, atlas_data, false, &font.atlas_texture.texture)) {
        free(atlas_data);
        return font;
    }
    free(atlas_data);
...
SITAPI RGLTrueTypeFont RGL_LoadTrueTypeFont(const char* font_path, float font_size) {
    ...
    if (!_RGL_UploadGrayscaleAsRGBATexture(atlas_width, atlas_height, atlas_bitmap, false, &font.atlas_texture.texture)) {
        free(font_data);
        free(atlas_bitmap);
        free(baked_chars);
        _SituationSetErrorFromCode(SITUATION_ERROR_GENERAL, "Failed to upload TTF atlas texture.");
        return font;
    }
    free(atlas_bitmap);
...
SITAPI RGLTexture RGL_StampTextToTexture(const char* text, RGLBitmapFont font, Color text_color, Color bg_color, int* out_width, int* out_height) {
    ...
    RGL_SetCamera2D((Vector2){tex_width/2.0f, tex_height/2.0f}, 0.0f, 1.0f);
    ...
    // Draw text
    RGL_DrawText(text, (Vector2){{.x = 2, .y = 2}}, font, text_color);
...
SITAPI RGLTexture RGL_StampTextToTextureAdvanced(const char* text, RGLTrueTypeFont font, Color text_color, Color bg_color, float wrap_width, int* out_width, int* out_height) {
    ...
    RGL_DrawTextTTF(text, (Vector2){{.x = 4, .y = 4}}, font, text_color);
...
SITAPI void RGL_DrawText(const char* text, Vector2 position, RGLBitmapFont font, Color color) {
    ...
    for (const char* c = text; *c != '\0'; c++) {
        ...
        RGL_DrawSprite(char_sprite, _RGL_V2(cursor), 0.0f, 1.0f, color);
...
SITAPI void RGL_DrawTextTTF(const char* text, Vector2 position, RGLTrueTypeFont font, Color color) {
    ...
    for (const char* c = text; *c != '\0'; c++) {
        ...
        if (char_rect.width > 0 && char_rect.height > 0) {
            ...
            RGL_DrawSprite(char_sprite, _RGL_V2(draw_pos), 0.0f, 1.0f, color);
        }
