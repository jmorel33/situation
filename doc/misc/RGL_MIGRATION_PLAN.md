...
| Textures | `SituationLoadTexture` / `SituationCreateTexture` | `RGL_LoadTexture` + font/terminal/TTF atlases via `_RGL_UploadTextureFromPixels` | Stamp-text / some paths untested |
...
| 3.5 | Font/bitmap atlases via `SituationCreateTexture` (`_RGL_UploadTextureFromPixels`, grayscale→RGBA for TTF) | ✅ |
...
| ID | Actionable |
|----|------------|
| 7A.1 | Bake atlases to `SituationImage` → `SituationCreateTexture` | ✅ (bitmap, terminal, packed, TTF) |
| 7A.2 | `RGL_StampTextToTexture*` — CPU rasterize, then Situation texture upload |
| 7A.3 | Consider delegating `RGL_DrawTextTTF` to `SituationCmdDrawText` where quality/perf acceptable |
...
| 7 Fonts/particles/shadows/debug | 🟡 | 7A fonts ✅; 7C/7D stubbed; test patterns compile fixes ongoing |
...
Defer path/level/stencil until smoke test is green. Font atlas upload can ride along (already on Situation path).
...
*Last updated: 2026-05-24 (Situation v2.4.124 bind-index API; smoke build in progress)*
