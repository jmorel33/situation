...
_For Core API library v2.4.258 — see **`doc/UPDATELOG.md`** for full patch notes per release._

### v2.4.258 — `SituationQueryShaderStorageBlocks` + Phase 27 reflection test

*   **`SituationQueryShaderStorageBlocks` (v2.4.258, OpenGL only):** New public API that enumerates a linked shader program's active SSBO blocks and their post-link binding points. Returns a `SituationShaderStorageBlockInfo` array with `name`, `binding_point`, and `block_index` per block. Vulkan returns `NOT_IMPLEMENTED`. Useful for diagnostics, harness reflection checks, and regression testing against the v2.4.81/82 duplicate-binding bug.
*   **`SituationShaderStorageBlockInfo` struct** added to `situation_api.h`.
*   **Harness — `spirv_ssbo_reflection_bindings` (Phase 27):** New test in `--module graphics` asserts `BlockA → binding 0`, `BlockB → binding 1` via `SituationQueryShaderStorageBlocks`, with graceful skip if the driver strips SSBO names. Completes all phases (23–29) of `doc/plan/TEST_HARNESS_GRAPHICS_UPGRADE.md`.

### v2.4.256 — `SituationCmdSetMultisampleState` GL implemented; Vulkan shadow state

*   **`SituationCmdSetMultisampleState` (v2.4.256):** GL path fully implemented — dispatches `GL_SAMPLE_SHADING`, `glMinSampleShading`, `glSampleMaski`, and `GL_SAMPLE_ALPHA_TO_COVERAGE` per draw. Previously returned `SITUATION_ERROR_NOT_IMPLEMENTED` on both backends since v2.4.185. Vulkan now records the desired state into shadow fields (`dynamic_ms_*`) and returns `SITUATION_SUCCESS`; GPU dispatch requires Phase 6 MSAA pipeline variants (pipeline-bake constraint in Vulkan 1.4 core).
*   **`SituationMultisampleState` struct fields** now documented in `situation_api.md` (`sample_shading_enable`, `min_sample_shading`, `sample_mask`, `alpha_to_coverage_enable`). `sample_mask == 0` maps to all-ones.
*   **Push/pop raster stack:** All four multisample fields added to `_SitGLRasterStackEntry` and `_SitVulkanRasterStackEntry` — `CmdPushRasterState`/`CmdPopRasterState` now saves and restores multisample state on both backends.
*   **`SituationGetGraphicsCaps::max_msaa_samples` (Vulkan):** Fixed — was hardcoded to `1`. Now queries `VkPhysicalDeviceLimits::framebufferColorSampleCounts & framebufferDepthSampleCounts` and returns the highest supported power-of-two sample count.
*   **Harness:** Three new tests in `--module graphics`: `multisample_state_no_crash`, `multisample_state_push_pop`, `multisample_caps_nonzero`. All pass on GL and VK (GTX 1070, GCC 15.1.0).

### v2.4.250 — Wrapper example builder backends

*   **Odin / Zig / Rust example builders (v2.4.250):** `build_odin_example.bat`, `build_zig_example.bat`, and `build_rust_example.bat` now accept `opengl`, `vulkan`, `static-opengl`, and `static-vulkan` — matching `build_examples.bat` / `build_tests.bat`. Shared link config in `scripts/wrapper_*.bat`. Documented in `doc/COMPILATION_GUIDE.md`, `README.md`, `situation_api.md`, and `situation_sdk.md`.

### v2.4.239 — Main thread OS name API

*   **`SituationSetCurrentThreadName(const char* name)` (v2.4.239):** Sets the OS-visible thread name for the calling thread (Task Manager / debugger). No-op if `NULL` or empty.
*   **`SituationInitInfo::main_thread_name` (v2.4.239):** Optional init override; `NULL` → **`SITUATION_MAIN_THREAD_NAME_DEFAULT`** (`"Sit Main"`). Pool snapshot reports the configured name.
...
### v2.4.229–237 — Async shader + thread pool hardening wave

*   **Render queue wedge (v2.4.229):** `render_queue_count` fixes full-ring vs empty ambiguity that wedged the render thread (~15 s fence timeouts).
*   **Job queue integrity (v2.4.229–233):** Slot reuse guards, completion ordering, in-place claim (v2.4.232), full-queue HOL scan (v2.4.233 — replaces capped 4–32 scan).
*   **Async shader contract (v2.4.230–237):** Terminal poll errors **-99** (`THREAD_JOB_LOST`) and **-557** (`COMPILE_TIMEOUT`); `compile_done` state machine (`0`, `-3`, `1`, `-1`, `-2`); poll-only driver (no acquire-time poll); orphan job retire; non-blocking compile pump. Harness: **`async_shader_poll_after_unload_during_load`**.
...
### v2.4.222 — VD idle compositor fallback

*   **Idle compositor fallback (v2.4.222):** When a Virtual Display exceeds **`idle_threshold_seconds`** without a content write, the compositor draws a **shader-only** standby image instead of sampling a stale texture. **`SituationSetVirtualDisplayFallbackMode`** — **`SITUATION_VD_FALLBACK_SOLID`** or **`SITUATION_VD_FALLBACK_COLORBURST`** (SMPTE EG 1-1990 test card).
*   **Fallback color (v2.4.222):** **`SituationSetVirtualDisplayFallbackColor`** for SOLID mode (default deep blue). OpenGL deferred GL path now bumps VD content timestamps on texture draws. Harness: **`vd_idle_content_switch`**, **`vd_idle_content_switch_colorburst`**.

### v2.4.221 — VD content update tracking

*   **`SituationGetVirtualDisplayUpdateInfo` (v2.4.221):** Query when a VD last received **pixel content** — optional `last_content_update_time`, `last_content_update_frame`, `frames_since_update`, `seconds_since_update` (distinct from the per-frame VD clock and manual `is_dirty`).
*   **`SituationSetVirtualDisplayIdleThreshold` (v2.4.221):** Per-VD idle threshold in seconds (default **1.0** at create); consumed by the **v2.4.222** compositor idle branch. Harness: **`vd_content_update_info_after_draw`**, **`vd_content_update_info_idle`**.
