...
`SituationCreateThreadPool(..., num_threads=0, ...)` uses `thread_pool_*` init fields via `_SitResolveAutoWorkerCount()`. High-queue scan depth is **dynamic (4–32)** from pending depth.
