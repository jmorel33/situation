...
**Status:** v2.4.126 (2026-06-28)  
**Canonical reference** for the two tone paths, graph **Tone Synth** controls/MIDI, migration, harness tests, and example **04** travel UI.
...
- Its own **42 control values** (indices **0–41**, voice template)
...
#### ADSR reset on legato note-on (control **41**, CC **118**)

When **mono**, portamento is active, and a **legato** note-on occurs (prior note still in ADSR), the envelope normally **stays at sustain** — no re-attack.

| `glide_reset_on_key` | Legato note-on behaviour |
|----------------------|---------------------------|
| **Off** (default) | Keep envelope phase (sustain held if already there) |
| **On** | **Retrigger ADSR** from attack; **pitch still glides** (`base_hz` → new `target_hz`) |

Detached note-ons (gap or release between notes) always get a full ADSR re-init regardless of this switch. Portamento glide is unaffected when this option is on.
...
|-------------------|------|-------|----------|
| Idle, or in **release** | `_SituationToneSynthVoiceInitFromControls` | `base_hz = target_hz = new note` | Full ADSR from attack |
| **Attack / decay / sustain** (still sounding) | `_SituationToneSynthVoiceLegatoFromControls` | Keeps `base_hz`, sets new `target_hz` → **portamento glides** | Sustain held if already in sustain; **re-attack** if control **41** on |
...
Per-voice ADSR (controls **4–8**) scales the combined main+sub signal before the filter.

| Idx | Name | Default | Range | Notes |
|-----|------|---------|-------|-------|
| 4 | `attack` | 0.01 s | 0.001–2 s | CC73 |
| 5 | `decay` | 0.1 s | 0.001–2 s | CC75 |
| 6 | `sustain` | 0.7 | 0–1 | CC76 |
| 7 | `release` | 0.2 s | 0.01–5 s | CC72 |
| 8 | `hold` | 1.0 s | −1 = ∞, 0–10 s | CC77 |

**Release curve** — the release phase is **exponential**, not linear. At progress `t ∈ [0,1]` through the release time:

`envelope = release_start × exp(ln(0.001) × t)`

That reaches roughly **−60 dB** (`0.001` of the level at note-off) at `t = 1`, giving a smooth RC-style tail typical of analog synths. The same envelope also drives filter cutoff when `filter_env_amount` &gt; 0 (§7).
...
|-----|------|---------|-----------|
| 37 | `patch_slot` | **114** | Slot **0–15** (`value` quantized to 16 steps). **Recall** when the CC value **changes** (empty slot = no change). **`SituationSetControl(..., 37, slot)`** recalls on every call @ v2.4.402+ (even if slot unchanged). |
| 38 | `patch_store` | **115** | **≥64** on a rising edge **saves** current controls into the active slot. **`SituationSetControl(..., 38, ≥0.5)`** saves on every call @ v2.4.402+ (not only 0→1 transitions). |
...
One **mod LFO instance per active voice** — rate/wave/targets are shared controls, but each note advances its own phase. Separate from **CC1 vibrato** (fixed 5 Hz sine on pitch) and **CC92 tremolo** (fixed 5 Hz on level). When no MIDI voices are active, the manual path uses the node’s template LFO state.
...
| 39 | `lfo_reset_on_key` | **on** | bool — reset LFO phase on note-on (free-run when off) |
| 40 | `lfo_start_phase` | 0 | 0–1 — cycle position at reset (0=trough/start, 0.25=mid rise, 0.5=peak, 0.75=mid fall) |
...
| 116 | `lfo_reset_on_key` (≥64 = on) |
| 117 | `lfo_start_phase` (`norm` 0–1) |
| 118 | `glide_reset_on_key` (≥64 on) |
...
## 8. Node controls (42 total)

These are **production / runtime parameters** — the values in each node’s `control_values[]` array that the DSP reads (`SituationSetControl`, UI, serialization, and MIDI **after** CC→ctrl mapping). Indices **0–41** (`registry_init.h`: `num_controls = 42`). They are **not** raw MIDI bytes.
...
| 2 | `volume` | 0–1 | 0.5 | Manual template; MIDI uses velocity + CC7/11 |
| 3 | `pan` | −1..1 | 0 | CC10 |
| 4 | `attack` | s | 0.01 | CC73 |
| 5 | `decay` | s | 0.1 | CC75 |
| 6 | `sustain` | 0–1 | 0.7 | CC76 |
| 7 | `release` | s | 0.2 | CC72; exponential tail (§3) |
| 8 | `hold` | s | 1.0 | CC77; −1 = infinite |
| 9–15 | Filter block | — | see §7 | |
...
| 35 | `sub_sync` | bool | Off | **CC112** |
| 36 | `sub_ring_mod` | bool | Off | **CC113** |
| 37 | `patch_slot` | slot | 0 | **CC114** recall on CC change; **SetControl** recall always @ v2.4.402+ |
| 38 | `patch_store` | trigger | — | **CC115** rising edge / **SetControl** ≥0.5 save always @ v2.4.402+ |
| 39 | `lfo_reset_on_key` | bool | on | **CC116** |
| 40 | `lfo_start_phase` | 0–1 | 0 | **CC117**; cycle at reset |
| 41 | `glide_reset_on_key` | bool | Off | **CC118**; mono legato ADSR retrigger (§3) |
...
| 116 | `lfo_reset_on_key` | ≥64 on → control **39** |
| 117 | `lfo_start_phase` | `norm` 0–1 → control **40** |
| 118 | `glide_reset_on_key` | ≥64 on → control **41** |
...
| Sub sync / ring / coarse | ✗ | ✓ | v2.4.119+ |
| Glide reset on key (CC118) | ✗ | ✓ | v2.4.380–383 |
| Exponential release tail | ✗ | ✓ | v2.4.380–383 |
| LFO start phase (CC117) | ✗ | ✓ | v2.4.380–383 |
| Patch slots (16×) | ✗ | ✓ | v2.4.123 |
...
| Example 04 travel UI | `examples/04_play_a_sound/travel_edit.inc`, `main.c` |
| Changelog | `doc/UPDATELOG.md` (v2.4.109–126) |
...
## 16. Example 04 — travel parameter UI

`examples/04_play_a_sound` exposes **all 42 tone synth controls** (plus FX chain parameters) through a single **travel editor** (`travel_edit.inc`). Piano keys stay on the keyboard row; everything else uses one consistent binding:

| Key | Action |
|-----|--------|
| **F1** | Toggle parameter edit mode |
| **PgUp / PgDn** | Section: ADSR → OSC → Filter → Sub → LFO → FX |
| **Up / Down** | Previous / next parameter in section |
| **Left / Right** | Decrease / increase (enums & bools: one step per tick) |
| **Hold L/R** | Repeats at **10 Hz** (frame-rate independent; `SituationGetFrameTime`) |
| **Shift** | Fine steps on float / log parameters |
| **Enter** | Save patch (OSC section) / cycle enum shortcut |
| **Home / End** | First / last parameter |

**OSC section** includes manual `frequency` and `volume` (controls **0–2**) for the no-MIDI path; the piano example drives pitch via virtual MIDI and leaves `volume` at **0** so level comes from note velocity.

**Not on travel keys** (dedicated): piano, sustain (SPACE), octave (F2/F3), FX preset (`` ` ``), master gain (`\` / BACKSPACE — also **10 Hz** repeat).

Build: `build\build_examples.bat static-opengl 04_play_a_sound`

---

## 17. Example 04 — stereo main output meters

Post-master-gain monitoring uses **`SITUATION_NODE_PEAK_METER`** in the FX chain:

`… → reverb → gain → **peak_meter** → mixer`

Each frame the UI reads stereo **peak** levels (ballistic decay) from the meter node and draws **L** / **R** vertical **segment** bars (18 green segments, dim off-state) in a compact strip between the piano and the 3×2 panels. Piano height is slightly reduced to make room.

Requires a rebuild of the library **and** example 04 (`build_situation.bat` then `build_examples.bat`).
