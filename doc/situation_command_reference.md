...
_Authoritative catalog of every `SituationCmd*` recording function — Situation **2.4.399**. Regenerate index: `python tools/generate_api_index.py`._
...
| **[RENDERER_COMMAND_STACK.md](RENDERER_COMMAND_STACK.md)** | Bolster **status** (shipped / partial / deferred) by category | Per-command signatures (link to this file) |
...
| **[situation_sdk.md](situation_sdk.md)** §3.2–3.5 | Short intros + **worked examples** (clear screen, shader workflow, GLTF) — may show commands inline in context | Full per-command matrix (see index: 71 `SituationCmd*`) |
...
## Command index (82 active + 7 deprecated)
...
| `SituationRendererBehaviorPolicyDefault` | Core | ✓ | ✓ | Strict defaults on every policy axis |
| `SituationCmdSetRendererBehavior` | Core | ✓ | ✓ | Replace active renderer behavior policy |
| `SituationCmdPushRendererBehavior` | Core | ✓ | ✓ | Push policy snapshot (max depth 32) |
| `SituationCmdPopRendererBehavior` | Core | ✓ | ✓ | Restore pushed policy |
...
| `SituationCmdSetMultisampleState` | Core | ✓ | ✓ (shadow) | In-pass flags; no MSAA attachments until VD-4b |
...
| `SituationCmdBindMeshPullBuffers` | Core | ✓ | ✓ | Push mesh BDA block for pull shaders |
...
| `SituationCmdDrawTextBoxed` | High | ✓ | ✓ | Text clipped/wrapped in rectangle |
...
| `SituationCmdGPUZoneBegin` | Core | ✓ | ✓ | GPU elapsed zone begin (P10.3; VK outside pass) |
| `SituationCmdGPUZoneEnd` | Core | ✓ | ✓ | GPU elapsed zone end |
| `SituationCmdResetQueryPool` | Core | ✓ | ✓ | Reset user query pool range (P10.4) |
| `SituationCmdWriteTimestamp` | Core | ✓ | ✓ | Write GPU timestamp query (P10.4) |
| `SituationCmdBeginOcclusionQuery` | Core | ✓ | ✓ | Begin occlusion query in render pass |
| `SituationCmdEndOcclusionQuery` | Core | ✓ | ✓ | End active occlusion query |
...
### Renderer behavior policy (Phase 14)

Scoped like raster push/pop; resets to strict at `SituationAcquireFrameCommandBuffer`. Only **user-recorded** transfer/barrier commands consult policy (VD/internal paths stay strict).
...
policy.validation = SIT_RENDERER_VALIDATION_WARN;
SituationCmdSetRendererBehavior(cmd, &policy);
```

| Command | Use case |
|---------|----------|
| `SituationRendererBehaviorPolicyDefault()` | Fill struct before mutating axes |
| `SituationCmdSetRendererBehavior(cmd, &policy)` | Replace whole active policy |
| `SituationCmdPushRendererBehavior(cmd, scope_id)` | Save policy before temporary opt-in block |
| `SituationCmdPopRendererBehavior(cmd, scope_id)` | Restore saved policy |

**Policy axes:** `transfer_usage` (`STRICT` / `COMPATIBLE_FALLBACK`), `texture_layout` (`EXPLICIT` / `ASSISTED`), `blit_filter` (`STRICT` / `DOWNGRADE_NEAREST`), `coordinate` (strict only in v2.4), `validation` (`STRICT` / `WARN` / `COMPAT`).

**Validation logging:** when `validation >= WARN` and a fallback runs, the library logs `renderer behavior: …` at `SIT_LOG_WARNING`. Visible only if `SituationSetTraceLogLevel` allows warnings (default `SIT_LOG_INFO` includes them). `COMPAT` matches `WARN` in v2.4 — neither disables hard errors for invalid handles or out-of-bounds regions.

See **[guide/renderer_bolster.md](guide/renderer_bolster.md)** § Workflow 3b and **[RENDERER_BARRIER_COOKBOOK.md](misc/RENDERER_BARRIER_COOKBOOK.md)**.
...
| `SituationCmdDrawText` | HUD labels | Requires baked `SituationFont` or zeroed default |
| `SituationCmdDrawTextEx` | Variable size/spacing text | Same font atlas |
| `SituationCmdDrawTextBoxed` | Panel/chat with clip or word wrap | Bounded `SitRectangle`; see [font.md](guide/font.md) |
| `SituationDrawMetricsOverlay` | FPS/latency debug panel | Not `SituationCmd*` — records on same command buffer |
...
**Use when:** Chat panels, tooltips, or HUD blocks that must stay inside a rectangle. `word_wrap` splits on spaces when text exceeds `bounds.width`.

**Requires:** Active render pass; font loaded/baked per [Fonts Module](guide/font.md).
...
**Use when:** Dev builds need FPS, frame time, queue depth, and memory stats (toggle with `M` in `sit_example.h`). Uses the built-in default font internally — not a `SituationCmd*` symbol, but recorded on the same command buffer inside a render pass.
...
<a id="9-gpu-profiling-zones-p103"></a>
## 9. GPU profiling zones (P10.3)
...
**Use when:** attributing GPU elapsed time to passes via **`SituationGetFrameProfile` → `gpu_zone_ns[]`**. Requires **`SIT_FEATURE_GPU_TIMESTAMPS`**. Zone IDs: **`SituationGPUProfileZone`**. Readback is one frame slot late.

**GL:** may bracket work inside an active render pass. **VK:** user zones must be **outside** an active render pass (`SITUATION_ERROR_PROFILING_ZONE_STATE`).

---

<a id="10-user-query-pools-p104"></a>
## 10. User query pools (P10.4)

Non-allocating pool create/destroy: **`SituationCreateQueryPool`**, **`SituationDestroyQueryPool`**, **`SituationGetQueryPoolResults`**.
...
**Timestamps:** pool type **`SITUATION_QUERY_TYPE_TIMESTAMP`**; requires timer query support (`SIT_FEATURE_GPU_TIMESTAMPS` / same probe as P10.3). VK results converted to nanoseconds in **`SituationGetQueryPoolResults`**.

**Occlusion:** pool type **`SITUATION_QUERY_TYPE_OCCLUSION`**; begin/end must be inside an active render pass. Results are sample-pass counts.

**Readback:** non-blocking returns **`SITUATION_ERROR_QUERY_RESULT_NOT_READY` (-566)**; use **`SITUATION_QUERY_RESULT_WAIT_BIT`** to block until ready.

---

<a id="11-debug-markers"></a>
## 11. Debug markers
