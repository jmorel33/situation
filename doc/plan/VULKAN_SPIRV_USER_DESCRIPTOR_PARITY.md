...
**Status**: **Phases 0–4 complete (v2.4.278)**; Phase 5 deferred indefinitely  
...
**Status**: **Complete (v2.4.278)** — library guard added, harness fully wired, 94/94 graphics tests pass  
...
- [x] **Audit**: verify `_SituationVulkanResolveBufferDescriptor` set 2 path for `UBO_SSBO_SAMPLER` — currently set 2 falls into the `else` branch which returns `text_sampler_layout` + `COMBINED_IMAGE_SAMPLER`. Cross-check: a `SituationBuffer` is never a sampler; set 2 must always be bound via `SituationCmdBindTextureSet`, not `SituationCmdBindDescriptorSet`. The resolver should return an explicit error (`SITUATION_ERROR_INVALID_PARAM`) if `SituationCmdBindDescriptorSet` is called with `set_index == 2` under `UBO_SSBO_SAMPLER` (buffer ≠ texture). Confirm or fix this guard.
- [x] **Push constants on `UBO_SSBO`**: decide: either add the 128-byte push range to `graphics_spirv_layout_ubo_ssbo` for consistency, or add a doc comment in `_SituationVulkanInitGraphicsSpirvLayouts` explicitly noting the deliberate omission. Do not leave it ambiguous.

**Harness tasks**

- [x] **New Vulkan FS: `tests/harness/shaders/harness_ubo_ssbo_sampler_vk.fs`**
  ```glsl
  #version 450
...
- [x] **Update `compile_harness_shaders.ps1`**: add sampler FS compile + VK embed invocation.

- [x] **Update `scripts/gen_spirv_embed.ps1`**: added `-FsSamplerSpv` optional parameter; emits stub zero array when absent.

- [x] **Update `compile_harness_shaders.ps1`** second `gen_spirv_embed.ps1` invocation passes `-FsSamplerSpv`.

- [x] **Update `tests/harness/sit_harness_spirv_embed.h`**: added `sit_harness_ubo_ssbo_sampler_fs_spv` / `_len` macro aliases and `extern` declarations (both backends; GL embed gets stub zero array).

- [x] **Update `tests/harness/sit_harness_spirv_vk_embed.c`** (regenerated by `gen_spirv_embed.ps1`).

- [x] **Update `spirv_harness_embed_ready()`** — added independent `spirv_harness_sampler_embed_ready()` guard for the new test under `#if SITUATION_USE_VULKAN`.

- [x] **New loader helper `spirv_harness_load_ubo_ssbo_sampler_shader()`** in `test_graphics_spirv.c`.

- [x] **New test `test_spirv_memory_ubo_ssbo_sampler_readback()`** in `test_graphics_spirv.c` — uses inline render pass; refreshes stale cached mesh before draw (pre-existing `post_link_resources` bug destroys shared mesh cache); pixel readback correct: R=77, G=255, B=200.

- [x] **Registered in `test_graphics.c`** under `#if defined(SITUATION_USE_VULKAN)` block.

- [x] **Updated `scripts/spirv_desc_spike.py`**: `EXPECTED_VARS` entry + `main()` loop entry for `harness_ubo_ssbo_sampler_vk.fs.spv`.
...
### Phase 4.1 — `UBO_SSBO` push-constant parity (resolved — no change)

**Duration**: N/A  
**Status**: **Resolved (v2.4.278)** — audit decision: document the deliberate omission, do not add the range. Source comment added to `_SituationVulkanInitGraphicsSpirvLayouts`. No code change required.

If the decision is ever revisited:

- [ ] In `_SituationVulkanInitGraphicsSpirvLayouts()`, add `VkPushConstantRange push = { VK_SHADER_STAGE_ALL_GRAPHICS, 0, 128 }` to the `layout_info` for `graphics_spirv_layout_ubo_ssbo`.
- [ ] Rebuild DLL, rerun full graphics test suite to confirm no regression.
- [ ] Update consumer quick reference to note all non-MESH profiles carry 128-byte push range.
...
| 2026-06-16 | v2.4.278 | **94/94**, spirv **9/9** | spirv **11/11** | **Phase 4 complete** — `UBO_SSBO_SAMPLER` harness + library set-2 buffer guard; `spirv_desc_spike.py` 3-shader gate; pre-existing `post_link_resources` stale-mesh bug diagnosed and worked around in test |
...
| v2.4.278 | Phase 4: `harness_ubo_ssbo_sampler_vk.fs` + embed; `test_spirv_memory_ubo_ssbo_sampler_readback`; `set_index==2` buffer guard in `_SituationVulkanResolveBufferDescriptor`; `spirv_desc_spike.py` 3-shader gate; `UBO_SSBO` push-constant omission documented |
