...
- [x] **0.1** Document current `HandleKTermResponse` behavior with editor inactive vs active.
- [x] **0.2** Spike: manual `DECSTBM` + `DECSCA` from a test command (`edit_test`) — verify protected rows resist typing.
- [x] **0.3** Spike: CPR round-trip while not in shell mode — measure latency.
