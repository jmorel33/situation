...
- [ ] After `SituationLoadShaderFromMemory` for Phase 24 shaders, query OpenGL:
...
- [ ] Assert block names `BlockA` / `BlockB` (or indices 0/1) map to bindings **0** and **1** respectively.
- [ ] Requires access to `gl_program_id` — if not exposed on `SituationShader`, add **harness-only** `SituationGetShaderGLProgramId` **or** duplicate minimal link in test with `#ifdef SITUATION_USE_OPENGL` internal include (prefer small public debug API only if already planned; otherwise test via readback-only in Phase 24 and mark this phase **optional**).
...
**Status**: **Implemented** (Phases 23–26, 28–29; Phase 27 optional, not added)  
