...
**Status:** Tooling complete. Active gaps: `UBO_SSBO_SAMPLER` harness coverage (Phase 4 of
descriptor parity plan), Async hardening Phases B–E, Vulkan shader cache (not started),
Demon Hunt Step A/D bisect pending.
**As of:** v2.4.264
...
- [ ] **[Open]** `UBO_SSBO_SAMPLER` harness coverage (library done, zero tests) — see §1
...
- [ ] Audit `_SituationVulkanResolveBufferDescriptor` set 2 path under `UBO_SSBO_SAMPLER` — the
  current `else` branch returns `text_sampler_layout + COMBINED_IMAGE_SAMPLER`; calling
  `SituationCmdBindDescriptorSet` with a buffer (not a texture) on set 2 should return
  `SITUATION_ERROR_INVALID_PARAM` explicitly rather than silently misrouting

### 1.2 — Harness shader

- [ ] Create `tests/harness/shaders/harness_ubo_ssbo_sampler_vk.fs` (Vulkan target only):
  ```glsl
  ...
  void main() { fragColor = vec4(float(tags.tagB)/255.0, frame.color.g, texture(feedback,vec2(0.5)).r, 1.0); }
  ```

### 1.3 — Compile pipeline

- [ ] Add `Invoke-GlslcCompile 'fragment' harness_ubo_ssbo_sampler_vk.fs → spirv_out/harness_ubo_ssbo_sampler_vk.fs.spv`
  to `compile_harness_shaders.ps1` using `$vkFlags` (no `-fauto-map-locations`)
- [ ] Add `-FsSamplerSpv` optional parameter to `scripts/gen_spirv_embed.ps1`; emit
  `${Prefix}_ubo_ssbo_sampler_fs_spv` / `_len` array when provided; keep backward-compat
- [ ] Pass `-FsSamplerSpv` to the VK embed invocation in `compile_harness_shaders.ps1`
- [ ] Update `tests/harness/sit_harness_spirv_embed.h` — add extern declarations under
  `#if defined(SITUATION_USE_VULKAN)` guard

### 1.4 — Descriptor spike gate

- [ ] Add entry to `EXPECTED_VARS` in `scripts/spirv_desc_spike.py`:
  `"harness_ubo_ssbo_sampler_vk.fs.spv": [(0, 0), (1, 0), (2, 0)]`
- [ ] Add the filename to the `main()` loop in `spirv_desc_spike.py`

### 1.5 — Test

- [ ] Add `spirv_harness_load_ubo_ssbo_sampler_shader()` helper in `test_graphics_spirv.c`
  (Vulkan: `SituationLoadShaderFromSpirvMemoryEx(… SIT_SPIRV_LAYOUT_PROFILE_UBO_SSBO_SAMPLER …)`;
  OpenGL: skip / return `SITUATION_SUCCESS`)
- [ ] Add `test_spirv_memory_ubo_ssbo_sampler_readback()` in `test_graphics_spirv.c`:
  1. Create 1×1 texture (e.g. RGBA `{0, 0, 200, 255}`)
  2. Create UBO (`frame.color.g = 1.0`) and SSBO (`tagB = 77`)
  3. Load shader with `SIT_SPIRV_LAYOUT_PROFILE_UBO_SSBO_SAMPLER`
  4. Bind: `CmdBindDescriptorSet(cmd, 0, ubo)`, `(cmd, 1, ssbo)`, `CmdBindTextureSet(cmd, 2, tex)`
  5. Draw + readback center pixel
  6. Assert: `rgba[0] ≈ 77/255`, `rgba[1] ≈ 255`, `rgba[2] ≈ texture R ± 10`
  7. Teardown: unload shader, destroy buffers, destroy texture
- [ ] Update `spirv_harness_embed_ready()` — guard the new test on
  `sit_harness_ubo_ssbo_sampler_fs_spv_len > 0u` independently (not block other tests)
- [ ] Register test in `test_graphics.c` under the `#if defined(SITUATION_USE_VULKAN)` block:
  `{"spirv_memory_ubo_ssbo_sampler_readback", test_spirv_memory_ubo_ssbo_sampler_readback, true}`

### 1.6 — Verification

- [ ] `compile_harness_shaders.bat` → `python scripts\spirv_desc_spike.py` — PASS on all three VK shaders
- [ ] `build\sit_test_vulkan.exe --module graphics --filter spirv` — all SPIR-V tests pass including `ubo_ssbo_sampler_readback`
- [ ] `build\sit_test_vulkan.exe --module graphics` — no new failures
- [ ] `build\sit_test.exe --module graphics --filter spirv` — OpenGL unaffected
- [ ] Vulkan validation layers — no ERROR during the new test draw

### 1.7 — Optional cleanup (Phase 4.1, parked)

- [ ] Decide: add 128-byte push constant range to `graphics_spirv_layout_ubo_ssbo` in
  `_SituationVulkanInitGraphicsSpirvLayouts()` for parity with MESH and `UBO_SSBO_SAMPLER`,
  or add an explicit doc comment in the init function noting the deliberate omission.
  Currently `UBO_SSBO` has `pushConstantRangeCount = 0` — not a current bug but a design divergence.
