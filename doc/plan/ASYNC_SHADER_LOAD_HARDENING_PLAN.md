...
**Scope**: Public **`SituationBeginLoadShaderFromMemory` / `SituationPollShaderLoad` / `SituationUnloadShader`** (all renderer builds); primary implementation file: **`sit/situation_impl_renderer_shader.h`** (v2.4.360+); split by backend inline `#if` — see § Scope & backends  
**Companion**: `doc/plan/LIBRARY_BUGFIX_PLAN.md`, `doc/plan/THREADING_BOLSTERING_PLAN.md`, `doc/plan/TEST_HARNESS_PLAN.md`, [`VULKAN_SHADER_CACHE_PLAN.md`](VULKAN_SHADER_CACHE_PLAN.md), [`RENDERER_MODULARIZATION_PLAN.md`](../done/RENDERER_MODULARIZATION_PLAN.md)
...
|-------|---------|--------|
| **A** — Fast unload (Vulkan progress driver) | v2.4.238 | ✅ Shipped — code confirmed in source |
| **B** — Starvation drive | v2.4.324 | 🟡 B.1 implemented (unclaimed fast-path inline) — B.2–B.5 open |
...
Situation is **not Vulkan-only**. Async shader loading is one **shared SITAPI** with **three distinct implementation paths** in `situation_impl_renderer_shader.h`:
...
```c
/* situation_impl_renderer_shader.h — internal only */
typedef enum {
...
**Files**: `sit/situation_impl_renderer_shader.h`, `sit/situation_impl_renderer_fwd.h`, `sit/situation_api.h`  
...
- [x] All A.1–A.8 boxes checked — code confirmed in `situation_impl_renderer_shader.h` (v2.4.324)
- [x] Master dashboard: Phase **A** → ✅
...
**Files**: `sit/situation_impl_renderer_shader.h`, `sit/situation_impl_threading.h`  
**Note**: validate `sit_test.exe --module threading` unchanged.

#### B.1 — Unclaimed fast path

- [x] In progress driver: detect unclaimed job after `UNCLAIMED_FAST_NS` — confirmed at ~L7001–7003
- [x] Retire job via `_SitThreadPoolRetireOrphanedJobMain` — confirmed at ~L7019
- [x] Set `ctx->compile_job = 0` — confirmed at ~L7020
- [x] Run `_SituationVkAsyncCompileWorker` inline on main — confirmed at ~L7021
...
- [ ] All B.1–B.5 boxes checked (B.1 done; B.2–B.5 open)
