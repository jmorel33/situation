...
- [x] `SITUATION_ERROR_INTERNAL_STATE_CORRUPTED` — wired into hot-reload fallthrough paths in `SituationReloadShader`, `SituationReloadTexture`, `SituationReloadComputePipeline` (v2.4.293; see `RENDERER_ERRNO_GAPS_PLAN.md` GAP-1)
...
- [x] `SITUATION_ERROR_VULKAN_PIPELINE_CACHE_INIT_FAILED` (-758) — wired into `_SituationInitVulkan` `vkCreatePipelineCache` failure (v2.4.293)
- [x] `SITUATION_ERROR_VULKAN_RENDER_PASS_CACHE_FULL` (-759) — wired into `_SituationVulkanGetOrCreateRenderPass` cache-full branch (v2.4.293)
...
- [x] `SITUATION_ERROR_THREAD_JOIN_FAILED` — wired into `SituationDestroyThreadPool` (thrd_join return checked for both worker and I/O threads; non-fatal, sets error and continues cleanup)
- [ ] `SITUATION_ERROR_THREAD_DETACH_FAILED` — deferred: no `thrd_detach` call sites exist in the library; all threads are joined at shutdown
- [x] `SITUATION_ERROR_THREAD_MUTEX_LOCK_FAILED` — `SITUATION_CHECK_MUTEX_LOCK` macro now calls `_SituationSetErrorFromCode(THREAD_MUTEX_LOCK_FAILED)` before returning
- [x] `SITUATION_ERROR_THREAD_MUTEX_UNLOCK_FAILED` — `SITUATION_CHECK_MUTEX_UNLOCK` macro now calls `_SituationSetErrorFromCode(THREAD_MUTEX_UNLOCK_FAILED)` before returning
- [x] `SITUATION_ERROR_THREAD_MUTEX_INIT_FAILED` — wired into `SituationCreateThreadPool` (both queue mtx_init calls now check return value; on failure, cleans up and returns this code)
- [x] `SITUATION_ERROR_THREAD_MUTEX_TIMEOUT` — `SituationMutexTryLockTimeout` now calls `_SituationSetErrorFromCode(THREAD_MUTEX_TIMEOUT)` before returning false
- [ ] `SITUATION_ERROR_THREAD_ATOMIC_FAILED` — deferred: C11 atomics do not have a failure return; no platform-specific fallback currently
- [x] `SITUATION_ERROR_THREAD_STATE_INVALID` — wired into `SituationSubmitJobEx` (pool != NULL but !is_active path now sets error code)
- [ ] `SITUATION_ERROR_THREAD_BUFFER_OVERFLOW` — deferred: name buffer is protected by strncpy + manual null-term; no overflow detection without length tracking
- [ ] `SITUATION_ERROR_THREAD_DEADLOCK_DETECTED` — deferred: no deadlock detection system exists (would require cycle detection in the lock graph)
- [ ] `SITUATION_ERROR_THREAD_NOT_AVAILABLE` — deferred: threading API lives entirely inside `#ifdef SITUATION_ENABLE_THREADING`; when disabled, the functions don't exist. No stub needed — callers can't reach the code.
- [ ] `SITUATION_ERROR_RENDER_BACKPRESSURE_TIMEOUT` — deferred: render thread is a future feature; no render-thread join timeout path exists yet
- [ ] `SITUATION_ERROR_RENDER_LIST_INCOMPLETE` — deferred: no render list validation system exists yet
- [ ] `SITUATION_ERROR_ARM_INTRINSICS_FAILED` — reserved (ARM platform not yet shipping)
- [x] `SITUATION_ERROR_UPDATE_AFTER_DRAW_VIOLATION` — wired into `SituationPollInputEvents` and `SituationUpdateTimers` (sets error and logs to stderr in debug builds when `sit_render.in_frame` is true at poll/update time)
- [x] `SITUATION_ERROR_ASSERTION_FAILED` — `SITUATION_ASSERT(cond)` macro added to `situation_api.h`; fires `_SituationSetErrorFromCode(ASSERTION_FAILED)` + stderr log in debug builds (NDEBUG not defined). Named `SITUATION_ASSERT` (not `SIT_ASSERT`) to avoid collision with the test harness's `SIT_ASSERT` macro.
