...
**Actions:**
- [ ] Create `sit/situation_impl_vd.h` with header/guard
- [ ] Move `_SituationInitGLVirtualDisplayRenderer` (~line 3580, ~50 lines)
- [ ] Add `#include "situation_impl_vd.h"` in `situation_impl.h` (LATE — after all renderer helpers, before the VD API block's current position)
- [ ] Build both backends
...
**Actions:**
- [ ] Move lines 18373–19195 (all VD API functions + sort callback) to `situation_impl_vd.h`
- [ ] Ensure `_SituationSortVirtualDisplaysCallback` forward declaration (line 564) remains for GL execute commands
- [ ] Build both backends
...
- [ ] Clean build passes
- [ ] Verify line counts:
  - [ ] `situation_impl_vd.h`: ~870–950 lines
  - [ ] `situation_impl.h`: reduced to ~19,800 lines
