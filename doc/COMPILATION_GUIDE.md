...
│   ├── situation_api.h           # Public API declarations (SITAPI functions, complex types, enums)
│   ├── situation_base_version.h  # Canonical version macros
│   ├── situation_base_errno.h    # Error enum (SituationError)
│   ├── situation_base_types.h    # Primitive types, math types, GPU handles, audio stream types — zero external deps
...
│   ├── situation_impl_renderer_fwd.h # Renderer forward declarations (complete coverage, grouped by subsystem)
...
├── wrappers/                     # Odin, Zig, Rust FFI bindings + examples
│   ├── Odin/                     # Generated bindings + hello_situation demo
│   ├── Zig/                      # build.zig + hello_situation demo
│   └── Rust/                     # Cargo crate + build.rs link logic
├── scripts/                      # Shared build helpers (wrapper link config, etc.)
...
├── doc/                         # Documentation
│
└── sit/gpu/                     # Core internal renderer GLSL (VD, composite, quad, text, YPQ)
    ├── compositor.vert          # VD + composite vertex (path A/B via compile defines)
    ├── vd.frag
    ├── composite.frag
    ├── quad.vert                # quad + YPQ grade
    ├── quad.frag
    ├── ypq_grade.frag
    ├── text.vert
    └── text.frag
...
```bash
build\build_situation.bat static-opengl   # → build/dll/situation_opengl.a
...
All build scripts live in the `build\` folder. Run them from the project root or from within `build\` — they use `%~dp0..` internally to resolve paths relative to the project root regardless of where you invoke them from:

```
build\build_situation.bat   ← build the library (DLL or static)
build\build_examples.bat    ← build example programs
build\build_tests.bat       ← build the test harness
build\run_tests.bat         ← run the test harness (with result logging)
build\build_zig_example.bat
build\build_rust_example.bat
build\build_odin_example.bat
build\compile_harness_shaders.bat
build\compile_demon_hunt_shaders.bat
```
...
gcc -o myapp.exe main.c \
    -std=c11 -I. -Iext -Iext/cglm/include -Iext/glfw/include \
    -DSITUATION_USE_OPENGL -DSITUATION_ENABLE_THREADING \
    -Lext/glfw/build/src build/dll/situation_opengl.a \
...
> **Note:** MSVC can consume the pre-built DLL but cannot compile the library core (GCC-only internals). Build the DLL with `build\build_situation.bat` first, then link from MSVC.
...
```batch
build\build_examples.bat static-opengl my_example
build\examples\my_example.exe
```

### Language Wrapper Examples (Odin, Zig, Rust)

Dedicated batch build scripts compile the FFI wrapper demos in `wrappers/`. They use the **same backend vocabulary** as `build_examples.bat` and `build_tests.bat`:
...
**Usage** (backend is required; example name defaults to `hello_situation`):
...
```batch
REM DLL-linked (copies situation_*.dll next to the exe)
build\build_odin_example.bat  opengl
build\build_zig_example.bat   vulkan
build\build_rust_example.bat  opengl hello_situation

REM Static (no DLL at runtime — same idea as build\build_examples.bat static-*)
build\build_odin_example.bat  static-opengl
build\build_zig_example.bat   static-opengl
build\build_rust_example.bat  static-opengl
```
...
|----------|--------|--------|
| Odin | `build\build_odin_example.bat` | `build\examples\odin\<name>.exe` |
| Zig | `build\build_zig_example.bat` | `build\examples\zig\<name>.exe` |
| Rust | `build\build_rust_example.bat` | `build\examples\rust\<name>.exe` |

**DLL modes** (`opengl`, `vulkan`): the matching `situation_opengl.dll` or `situation_vulkan.dll` is copied into the output folder automatically. MinGW `gendef` / `dlltool` may run once to create the import `.lib` if it is missing.

**Static modes** (`static-opengl`, `static-vulkan`): link against `build/dll/situation_opengl.a` or `situation_vulkan.a` plus GLFW and system libraries (Vulkan static also needs `VULKAN_SDK` and shaderc). No Situation DLL is required at runtime.

**Wrapper static support on Windows** (today):

| Language | `static-opengl` | `static-vulkan` | Notes |
|----------|-----------------|-----------------|-------|
| C (`build\build_examples.bat`) | ✅ Supported | ✅ Supported | Reference path — MinGW `gcc` links `situation_*.a` directly |
| Rust (`build_rust_example.bat`) | ✅ Supported | ✅ Supported | `gcc` linker + `wrappers/Rust/build.rs` (includes `libgcc_eh` for emulated TLS) |
| Zig (`build_zig_example.bat`) | ✅ Supported | ❌ Not supported | Zig uses `lld-link` (MSVC linker); OpenGL has no C++ deps so it works, but shaderc/VMA pull in MinGW C++ runtime archives (`libmsvcprt.a`, `libOLDNAMES.a`) that `lld-link` cannot resolve |
| Odin (`build_odin_example.bat`) | ❌ Not supported | ❌ Not supported | Toolchain mismatch — `lld-link` cannot consume any MinGW `.a` archives; see below |

**Shared helpers** (used by all three scripts):

- `scripts/wrapper_link_config.bat` — backend → link variables (`SIT_DLL_BASENAME`, `SIT_STATIC_A`, `SIT_ODIN_FOREIGN_IMPORT`, `SIT_ODIN_EXTRA_LDFLAGS`, etc.)
- `scripts/wrapper_ensure_import_lib.bat` — generate MinGW import lib from DLL when needed
- `scripts/wrapper_patch_odin_foreign.bat` / `wrapper_restore_odin_foreign.bat` — patch Odin `foreign import` for the active backend
- `scripts/wrapper_gcc_link_static.bat` — GCC final link helper (used by C examples; retained for other wrapper experiments)

- **Odin** — DLL: normal `odin build` against `situation_*.lib` (reliable). Static: patches `foreign import` to `situation_*.a` and runs `odin build` with `-extra-linker-flags` for GLFW/MinGW libs — **this path does not succeed on Windows today** (see *Odin static on Windows*). Regenerate bindings with `python tools\generate_odin_bindings.py` after API changes.
- **Zig** — `wrappers/Zig/build.zig` accepts `-Dlink=` and `-Dexample=`; `static-opengl` targets `x86_64-windows-gnu` and passes MinGW lib paths via `-Dmingw_lib=` / `-Dmingw_gcc_lib=`. `static-vulkan` is **blocked** — Zig's `lld-link` cannot resolve the MinGW C++ runtime archives (`libmsvcprt.a`, `libOLDNAMES.a`) that shaderc and VMA require. Use DLL `vulkan` mode instead. Regenerate with `python tools\generate_zig_bindings.py`.
- **Rust** — `wrappers/Rust/build.rs` reads `SITUATION_LINK` (`opengl` \| `vulkan` \| `static-opengl` \| `static-vulkan`). Static builds use GCC as the linker (`RUSTFLAGS=-C linker=gcc`). Regenerate with `python tools\generate_rust_bindings.py`.

**Requirements**: MSYS2 MinGW64 (`gcc`, `gendef`, `dlltool`) on `PATH`; bundled compilers under `_languages/odin`, `_languages/Zig`, `_languages/Rust`. For `vulkan` / `static-vulkan`, set `VULKAN_SDK` (or install under `C:\VulkanSDK\`).

#### Odin static on Windows — known limitation

Odin wrapper examples accept `static-opengl` / `static-vulkan` in `build_odin_example.bat` for parity with the other wrapper scripts, but **a self-contained Odin exe is not achievable with the current Windows toolchains**. This is not a Situation API or binding-generator issue — it is a linker/CRT mismatch between Odin and the MinGW-built Situation static library.

**Why C / Rust / Zig static works but Odin does not**

| Component | Windows toolchain | CRT / object format |
|-----------|-------------------|---------------------|
| `situation_*.a` (from `build_situation.bat static-*`) | MinGW `gcc` + `ar` | GNU CRT, ELF-style TLS (`__emutls_get_address`), GNU math (`sincos`, etc.) |
| Odin runtime + `odin build` | MSVC `lld-link` + UCRT | COFF objects with MSVC-style weak COMDAT symbols |
| C / Rust / Zig static link | MinGW `gcc` / `g++` | Same GNU side as `situation_*.a` — link succeeds |

**Two failed approaches (documented for future reference)**

1. **`odin build` + `situation_*.a` via `foreign import` / `-extra-linker-flags`** (current script path)  
   Odin's default linker (`lld-link`) pulls in the MSVC/UCRT runtime. The MinGW-built `situation_*.a` and GLFW `.a` expect the GNU CRT (`sincos`, `__imp__stricmp`, `__emutls_get_address`, pthread, etc.). Mixing the two produces hundreds of unresolved symbols or CRT conflicts.

2. **`-build-mode:obj` then `wrapper_gcc_link_static.bat` (gcc final link)**  
   MinGW `ld` cannot resolve Odin's internal COFF weak COMDAT symbols (`runtime::type_table`, `fmt::__DIGITS_LOWER`, `os::stdout`, etc.). Odin object files are produced for MSVC's linker, not GNU `ld`.

**Recommended workarounds for Odin users**

- **DLL mode** (supported): `build_odin_example.bat opengl` or `vulkan` — copy `situation_*.dll` next to the exe (script does this automatically).
- **Self-contained exe via C**: `build_examples.bat static-opengl hello_situation` if the goal is a portable binary without a Situation DLL.
- **Ship DLL alongside Odin exe**: same runtime model as DLL mode; users run from a folder containing both exes.

**What would be needed for true Odin static on Windows**

- A MSVC-built Situation `.lib` consumed by Odin's `lld-link`, *or*
- Odin emitting/linking with a MinGW-gnu target on Windows (not the default today), *or*
- A supported Odin link mode that keeps the full Odin runtime on `lld-link` while correctly bridging MinGW static archives (no known stable solution as of this guide).

> **Note:** DLL backends (`opengl`, `vulkan`) are the reliable choice for Odin wrapper examples on Windows. Rust and Zig static modes are supported; prefer `build_examples.bat static-*` for C. Odin `static-*` backends remain in the scripts for API parity and future toolchain improvements.
...
```batch
build\build_examples.bat vulkan kterm_console
...
- **v2.4.250** (2026-06-11) - Wrapper example builders (`build_odin_example.bat`, `build_zig_example.bat`, `build_rust_example.bat`) accept `opengl` / `vulkan` / `static-opengl` / `static-vulkan` backends; shared `scripts/wrapper_*.bat` link helpers
