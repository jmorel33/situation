...
| Metadata | Details |
| :--- | :--- |
| **Version** | **2.4.407** — canonical macros in **`sit/situation_base_version.h`**. Narrative releases: **`doc/UPDATELOG.md`**. |
...
| **Platform** | Windows 10+ primary; Linux compiles with system GLFW; macOS via Vulkan/MoltenVK (OpenGL 4.6 not native on macOS) |
| **License** | MIT License |
...
| Patch | Highlights |
|-------|------------|
| **v2.4.345** | **OpenGL VD PATTERN** — SPIR-V embed compositor + init/composite fix; `vd_idle_pattern_standby` on GL + VK |
| **v2.4.344** | **VD idle PATTERN** — full `SitTestPatternConfig` on compositor (Vulkan UBO); `Set/GetVirtualDisplayPatternConfig`; guide + harness |
| **v2.4.343** | **Test pattern harness** — std140 UBO dual-backend parity; `sit/gpu/test_patterns/` P7 gate (VK 9/9, GL 8/8); `sit_test_pattern_config.h/.c` |
| **v2.4.342** | **Python ctypes bindings** — sixth wrapper language; `generate_python_bindings.py`, `wrappers/python/`, `build_python_example.bat`, full `hello_situation.py` demo |
| **v2.4.341** | **Font migration (F0–F4)** — grid GPU text, retro font builders, measure/boxed/stamp, RGL bitmap delegates |
| **v2.4.340** | **P2 complete** — `doc/guide/` module split, 581/581 symbol coverage, binding verification |
| **v2.4.331** | **Phase B′ vertex pull** — mesh BDA addresses, `vertex_pull.glslh`, harness BDA tests |
...
| **[situation_api.md](situation_api.md)** | Module map + links to **`doc/guide/*.md`** (581/581 symbol coverage) |
...
| **[situation_api_generated.md](situation_api_generated.md)** | Header-sync supplement for symbols not yet expanded in module guides |
| **[COMPILATION_GUIDE.md](COMPILATION_GUIDE.md)** | **Build system reference** — static/DLL builds, **debug builds** (`debug.bat`), GLFW setup, linker flags, C examples, **language wrapper examples** (Odin, Zig, Rust, Fortran, Modula-2, **Python**) |
| **[misc/SITUATION_QUICK_REFERENCE.md](misc/SITUATION_QUICK_REFERENCE.md)** | Cheat sheet |
| **[plan/API_cleanliness_plan.md](plan/API_cleanliness_plan.md)** | Header organization roadmap (P0–P3) |

**Module guides** (detailed API prose by subsystem):

| Category | Guides |
|----------|--------|
| Core | [core](guide/core.md) · [window & display](guide/window_display.md) · [Windows app identity](guide/windows_app_identity.md) · [input](guide/input.md) · [image](guide/image.md) · [system introspection](guide/system_introspection.md) |
| Graphics | [graphics](guide/graphics.md) · [virtual display](guide/virtual_display.md) · [advanced GPU commands](guide/renderer_bolster.md) · [compute](guide/compute.md) · [2D grid](guide/grid.md) · [fonts](guide/font.md) · [text](guide/text_rendering.md) · [2D](guide/drawing_2d.md) · [3D](guide/drawing_3d.md) |
| Media | [audio](guide/audio.md) · [audio graph](guide/audio_graph.md) · [MIDI](guide/midi.md) · [filesystem](guide/filesystem.md) |
| Utilities | [threading](guide/threading.md) · [profiling & diagnostics](guide/profiling.md) · [YPQ color](guide/ypq_color.md) · [hot-reload](guide/hot_reload.md) · [logging](guide/logging.md) · [misc](guide/miscellaneous.md) · [deprecated](guide/deprecated.md) |
| Learning | [examples & FAQ](guide/examples_faq.md) |

**Deprecated APIs:** Symbols in the `Deprecated API` section of `sit/situation_api.h` remain callable and exported to all language bindings. GCC, Clang, and MSVC emit compile-time warnings via `SIT_DEPRECATED()` when you use them — migrate to the replacement named in the warning.
...
(`scripts\generate_situation_api_docs.py` is a shim that delegates to `tools\generate_api_index.py`.)
...
- Audio effects organized in `sit/aud/fx/` (23 DSP nodes)
...
```
situation/
├── situation.h                         # ← Public entry point (include this: api + profiling macros)
├── situation_dll.c                     # DLL entry point (for building DLL)
├── build/                              # Build scripts + build-only glue
│   ├── tracy_client.cpp                # Tracy client TU (SIT_TRACY=1)
│   └── dll/                            # Pre-built DLLs / static libs
├── ext/                                # External dependencies (glfw, cglm, glad, stb, miniaudio, shaderc, cgltf, tracy/public)
├── examples/                           # C example programs
├── tests/harness/                      # Test harness (sit_test_*.exe)
├── tools/                              # Binding generators, API index (tools/run_all.bat)
├── scripts/                            # Shared wrapper build helpers (wrapper_*.bat)
├── wrappers/                           # Generated FFI (Odin, Zig, Rust, Fortran, Modula-2, Python)
├── debug.bat                           # Debug rebuild (-O0 -g) + GDB test harness
├── doc/                                # SDK, COMPILATION_GUIDE, guide/*.md, UPDATELOG
└── sit/                                # ← Implementation
    ├── Makefile                        # Library build system (driven by build/build_situation.bat)
    ├── platform/                       # Platform-specific build resources
    │   ├── windows/                    # Windows: RC scripts, icon
    │   │   ├── situation_resource.rc   # DLL version info + icon (windres)
    │   │   ├── sit_app.rc              # EXE icon resource (windres)
    │   │   └── situation_icon.ico      # Multi-resolution icon (6 sizes)
    │   ├── linux/                      # (future) .desktop, XDG icons
    │   └── macos/                      # (future) Info.plist, .icns
    ├── gpu/                            # Embedded GLSL shaders (compositor, quad, text, YPQ)
    │   ├── compositor.vert / vd.frag / composite.frag
    │   ├── quad.vert / quad.frag / ypq_grade.frag
    │   └── text.vert / text.frag
    ├── situation_api.h                 # Public API umbrella (fans out to situation_api_*.h — see doc/situation_api_index.md)
    ├── situation_profiling.h           # Tracy CPU zone macros (P10.2 — via situation.h, not API umbrella)
    ├── situation_api_config.h          # Init, lifecycle config
    ├── situation_api_types_system.h    # Window flags, threading, system types
    ├── situation_api_types_gpu.h       # GPU handles, render types
    ├── situation_api_types_audio.h     # Audio graph, node types
    ...
    ├── situation_api_deprecated.h      # Legacy symbols (SITUATION_INCLUDE_DEPRECATED_API gate)
    ├── situation_base_version.h        # Version macros (canonical source)
    ├── situation_base_errno.h          # Error enum (SituationError)
    ├── situation_base_types.h          # Primitive types, math types, GPU resource handles, audio stream types — zero external deps
    ├── situation_base_callbacks.h      # All 18 callback typedef signatures — depends only on base_types, miniaudio-free
    ├── situation_base_etc.h            # Key codes, color constants, MIDI frequency table
    ├── situation_base_font.h           # Embedded VGA bitmap font
    ├── situation_base_trace.h          # Trace/profiling hooks (generated — do not edit by hand)
    ...
    ├── situation_impl_renderer_fwd.h   # Renderer static forward declarations (347/347; grouped by slice)
    ...
    ├── situation_impl_image.h          # Image, font, CPU draw/adjust loops (grid/bitmap font builders)
    ├── situation_impl_color.h          # HSV, YPQ, PQ/HDR, 10-bit color-space math (public + internal)
    ├── situation_impl_proj.h           # Camera & projection helpers
    ├── situation_impl_renderer.h       # Renderer orchestrator (includes slices below; v2.4.360+)
    ├── situation_impl_renderer_core.h      # Shared infra (uniform map, graveyard, GL backup, program cache)
    ├── situation_impl_renderer_lc.h        # Lifecycle, GL/VK bootstrap, render thread, hot-reload
    ├── situation_impl_renderer_shader.h    # Shader load/compile, pipelines, async workers
    ├── situation_impl_renderer_resources.h # Buffers, textures, meshes, slot registry
    ├── situation_impl_renderer_frame_cmd.h # Frame loop, SituationCmd*, model loaders
    ...
    ├── kfs/                            # Kaizen Filing System (KFS) — SQLite asset & knowledge store
    │   ├── kfs/                        # KFS source only
    │   │   ├── kfs.h                   # Orchestrator (-I sit/kfs, #include "kfs/kfs.h")
    │   │   ├── kfs_api.h               # Public types and declarations
    │   │   ├── kfs_impl.h              # Implementation (included via kfs.h)
    │   │   ├── kfs.c                   # Single KFS_IMPLEMENTATION translation unit
    │   │   └── Makefile                # outputs → ../build/
    │   ├── build/                      # smoke_main.c, api_only.c, obj/lib/bin
    │   ├── scripts/sync_api.py         # Regenerate kfs_api.h from impl
    │   ├── tests/                      # Harness (see doc/test_harness_plan.md)
    │   ├── doc/                        # architecture, COMPILATION_GUIDE, kfs_guide, …
...
**KFS (Kaizen Filing System):** internals — `sit/kfs/doc/architecture.md`; build — `sit/kfs/doc/COMPILATION_GUIDE.md`; security — `sit/kfs/doc/kfs_guide.md`. Include with `-I sit/kfs` and `#include "kfs/kfs.h"`.
...
**See:** `reports and notes/V2_4_0_FOLDER_REORGANIZATION_COMPLETE.md` for complete details.
...
- [2.0 Windowing & Display Subsystem](#20-windowing--display-subsystem)
  ...
  - [2.4 Application Identity](#24-application-identity)
- [3.0 The Graphics Pipeline](#30-the-graphics-pipeline)
  ...
  - [3.4 Geometry & Meshes](#34-geometry--meshes)
    ...
    - [3.4.7 Vertex Pull & Mesh BDA (v2.4.331)](#347-vertex-pull--mesh-bda-v24331)
  ...
  - [3.6 Virtual Display Compositor](#36-virtual-display-compositor)
    ...
    - [3.6.7 2D Grid playfields (compute)](#367-2d-grid-playfields-compute)
  ...
  - [3.8 Debugging & Profiling](#38-debugging--profiling)
    - [3.8.1 Metrics Overlay](#381-metrics-overlay)
    - [3.8.2 Frame Spike & Phase Timers (P10.0)](#382-frame-spike--phase-timers-p100)
    - [3.8.3 Structured Frame Profile (P10.1)](#383-structured-frame-profile-p101)
    - [3.8.4 GPU Timestamp Zones (P10.3)](#384-gpu-timestamp-zones-p103)
    - [3.8.5 Tracy CPU Profiling (P10.2)](#385-tracy-cpu-profiling-p102)
    - [3.8.6 Debug Builds & Thread Visibility](#386-debug-builds--thread-visibility)
...
| **Missing DLLs** | "Entry Point Not Found" | DLL builds: copy `situation_opengl.dll` or `situation_vulkan.dll` next to the exe. System `vulkan-1.dll` only when using the Vulkan loader dynamically. GLFW is usually static-linked. |
| **Old Drivers** | VK_ERROR_INCOMPATIBLE | Update GPU drivers (Vulkan **1.4+** target). |
...
Both backends **record** GPU work during the frame and **execute** at end-of-frame (OpenGL replays a soft command buffer; Vulkan submits the queue). To guarantee identical results, adhere to the Update-Before-Draw contract.

**The Rule:** Update all per-draw data (push constants, uniform buffer contents, SSBO writes) **before** recording the draw command that consumes them in the same render pass.

**The Violation:** If you record `SituationCmdDrawMesh` and then update the same buffer or push constants later in the same pass, both backends may use the later values at execution time — producing subtle ordering bugs.
...
The library uses return codes (`SituationError` enum) for critical failures and a logging system for warnings. See **[Logging Module](guide/logging.md)** for levels, filtering, custom callbacks, and OpenGL debug macros.
...
*   `SIT_FEATURE_BINDLESS_BUFFERS`: Buffer device address (BDA) / mesh pull path (Vulkan; see §3.4.7).
...
<a id="24-application-identity"></a>

## 2.4 Application Identity

Shell and bundle identity is **separate from window title and runtime icons**. Situation ships **library defaults** on each layer; authors **override** any layer without forking the library.

**Architecture (cross-platform — Windows Phase I @ v2.4.400; Linux **LI-***, macOS **MA-*** planned):** [architecture.md — Application Identity](architecture.md#application-identity-architecture-v24399).

**Implementation plan:** [plan/SIT_IDENTITY_PLAN.md](plan/SIT_IDENTITY_PLAN.md).

**Windows operational guide:** [guide/windows_app_identity.md](guide/windows_app_identity.md).

| Layer | Default | Override |
|-------|---------|----------|
| PE icon + version strings | `sit_app.rc` | Custom RC / `SIT_APP_RC` / `APP_RC=` |
| Shell AppUserModelID | `Situation.Application` | `SituationInitInfo::app_user_model_id`, `SituationWin32SetAppUserModelId()` |
| Window title | `init_info.window_title` | `SituationSetWindowTitle()` |
| Live window icon | Embedded PE icon; **`default_window_icon_path`** @ init (v2.4.400) | `SituationSetWindowIcons()` |
| Thread name | `main_thread_name` chain | `SituationSetCurrentThreadName()` |

#### SituationWin32SetAppUserModelId _(Win32, v2.4.399+)_

```c:disable-run
SituationError SituationWin32SetAppUserModelId(const char* app_id_utf8);
```

Set **before** `SituationInit` or before the first top-level window. **`NULL`**, empty, or strings containing spaces are rejected. When `SituationInitInfo::app_user_model_id` is **`NULL`**, init applies **`SITUATION_DEFAULT_APP_USER_MODEL_ID`** (`"Situation.Application"`).

**Trap Table: Identity**
| Symptom | Trap | Fix |
| :--- | :--- | :--- |
| Wrong Explorer icon | No custom RC linked | Link author RC or set `SIT_APP_RC` |
| Pin-to-taskbar groups wrong app | AppID mismatch | Match shortcut AppUserModelID to init |
| Task Manager Description wrong | EXE vs DLL resources | Version info must be on the **EXE** |
| AppID set too late | After `glfwCreateWindow` | Set in init info or pre-init API |
...
> Commands are recorded to a buffer on both backends (OpenGL soft replay at `SituationEndFrame`, Vulkan queue submit at `SituationEndFrame`). Barriers are explicit—miss one, and debug asserts abort with "UB Detected: Compute wrote, Vertex read without sync."
...
|---------|-----------|-----------|
| **OpenGL** | Soft buffer (`SIT_OP_*`) on each `SituationCmd*` / deferred `SituationSetShaderUniform*` | Replay at `SituationEndFrame()` |
| **Vulkan** | Command buffer recording on each `SituationCmd*` | Submit + present at `SituationEndFrame()` |
...
**Vulkan path (v2.4.105+):** GLSL→SPIR-V runs on the **high-priority worker queue** (`SituationSubmitJobEx` + `_SituationVkAsyncCompileWorker`), not the I/O thread. Pipeline creation runs on the **main thread** when poll returns success.

**Poll contract (v2.4.230+):** Drive progress **only** from `SituationPollShaderLoad` each frame — `SituationAcquireFrameCommandBuffer` does **not** poll async shader slots (v2.4.231). A compile that can never finish returns a **terminal** error instead of spinning forever:

| Error | Meaning |
|-------|---------|
| `SITUATION_ERROR_THREAD_JOB_LOST` (-99) | Job handle settled but shaderc never ran (scheduler defect) |
| `SITUATION_ERROR_SHADER_COMPILE_TIMEOUT` (-557) | Compile exceeded `SITUATION_VULKAN_ASYNC_COMPILE_DEADLINE_NS` (default 5 s) |
| `SITUATION_ERROR_SHADER_COMPILATION_FAILED` (-752) | shaderc failed (`compile_done == -1`) |

**`compile_done` states (Vulkan + shaderc):** `0` pending, `-3` compiling on worker, `1` SPIR-V ready, `-1` failed, `-2` abandoned (worker frees ctx).

**Unload during load (v2.4.238+):** `SituationUnloadShader` waits cooperatively, then abandons at **`SITUATION_VULKAN_ASYNC_UNLOAD_ABANDON_NS`** (2 s) via `_SituationVkAsyncCompileProgress` — same LOST/timeout logic as poll. **`SITUATION_VULKAN_ASYNC_UNLOAD_WAIT_NS`** (10 s) is shutdown last-resort only.

```mermaid
graph LR
    Begin["BeginLoadShaderFromMemory"] --> Submit["SubmitJobEx HIGH"]
    Submit --> Worker["shaderc worker<br/>0→-3→1"]
    Poll["PollShaderLoad"] --> Progress["_SituationVkAsyncCompileProgress"]
    Progress --> Pipelines["Build pipelines"]
    Unload["UnloadShader"] --> Progress
    Progress --> Abandon["2s abandon<br/>retire job"]
```

**OpenGL:** `glCompileShader` on begin (host GL context); poll checks driver link status — no `compile_done`, no thread-pool job.

See **`doc/plan/ASYNC_SHADER_LOAD_HARDENING_PLAN.md`** for the full multi-backend contract.
...
See also **`doc/misc/TEST_SPIRV_SHADER_API.md`** for harness verification paths.
...
**Behavior:** Destroys the GPU pipeline and frees resources. Sets the handle ID to 0. Safe during an in-flight async load (Vulkan waits or abandons the compile ctx per v2.4.229–238); OpenGL tears down GL shader objects immediately.
...
For quick prototyping on OpenGL, set standalone uniforms by name. **Not supported on Vulkan** — returns `SITUATION_ERROR_NOT_IMPLEMENTED`; use push constants or descriptor buffers instead.

```c:disable-run
SituationSetShaderUniform(shader, "uTime", &time, SIT_UNIFORM_FLOAT);
```

**OpenGL behavior (v2.4.330+):**
*   **Between frames:** Applies immediately via `glProgramUniform*`.
*   **During a frame** (`SituationAcquireFrameCommandBuffer` … `SituationEndFrame`): Records `SIT_OP_SET_UNIFORM` into the soft buffer (same deferral model as `SituationCmd*`).
*   **Render thread enabled:** Call only while a frame command buffer is active; otherwise `SITUATION_ERROR_NO_ACTIVE_COMMAND_BUFFER`.

Related OpenGL-only helpers: `SituationSetShaderUniformLocation`, `SituationSetShaderUniform1fv` / `1iv` / `Matrix4fv`, `SituationBindUniformBlock`, `SituationBindShaderStorageBlock`.

**VSync:** `SituationSetVSync` toggles swap interval at present time (OpenGL) or swapchain recreate (Vulkan). With the default render thread, VSync-off uncaps via `glfwSwapInterval(0)` — not by disabling queue backpressure (v2.4.330).
...
**Legacy Support:**
The previous 32-byte format (Pos/Norm/UV) remains supported when `vertex_stride` is 32 bytes — inferred as `SIT_MESH_LAYOUT_POS_NRM_TEX`. For explicit PBR uploads, use `SituationCreateMeshEx` with `SIT_MESH_LAYOUT_POS_NRM_TAN_TEX` and 48-byte interleaved vertices (as used by GLTF/OBJ loaders).
...
**Behavior:** Allocates GPU memory (VBO/EBO), uploads the data, and configures the input assembly state (VAO in OpenGL). Infers layout from `vertex_stride` (12/20/24/32/48 bytes). Unknown strides return `SITUATION_ERROR_INVALID_PARAM`.
**Explicit layout:** Use `SituationCreateMeshEx` when the stride/layout pairing must be unambiguous (e.g. `SIT_MESH_LAYOUT_POS_NRM_TAN_TEX` at 48 bytes).
...
<a id="347-vertex-pull--mesh-bda-v24331"></a>
### 3.4.7 Vertex Pull & Mesh BDA (v2.4.331)

**Phase B′** adds a Vulkan-first **vertex pull** path: shaders fetch vertex data via **buffer device address (BDA)** instead of fixed-function vertex attributes. Requires `SIT_FEATURE_BINDLESS_BUFFERS`.

| API | Purpose |
|-----|---------|
| `SituationGetMeshVertexBufferAddress(mesh)` | GPU VA of mesh VBO |
| `SituationGetMeshIndexBufferAddress(mesh)` | GPU VA of mesh IBO (0 if none) |
| `SituationGetBufferDeviceAddress(buffer)` | GPU VA of arbitrary `SituationBuffer` |

**GLSL helper:** `sit/gpu/vertex_pull.glslh` — `buffer_reference` pull VS pattern. BDA push constants need `GL_EXT_shader_explicit_arithmetic_types_int64` on some targets.

**Reference example:** `examples/other/vertex_pull_triangle.c` — smallest user-facing pull demo (Vulkan BDA; OpenGL falls back to VAO passthrough).

**Harness:** `tests/harness/test_vertex_pull_render.c`, `test_mesh_bda_compute_read.c` — compare passthrough vs pull draws; compute readback via BDA push constant.

**Plan:** `doc/plan/plan_handles_ssbo.md` — B′1–B′4 complete; optional bindless texture / generic buffer BDA (B′5) open.
...
> **Header note (v2.4.336):** `situation_api.h` lists many YPQ/HDR helpers under *Miscellaneous → YPQ / HDR color science*. Most apps only need **`SituationColorToYPQ`** / **`SituationColorFromYPQ`** (or the float variants) unless you are building color tooling — see [YPQ Color Module](guide/ypq_color.md) and [HD Color Output](guide/hd_color_output.md) for display-path HDR10.
...
Virtual displays are off-screen render targets composited onto the main window — pixel-art scaling, split-screen, PiP, post-FX chains, and compute-written textures.

**Full guide:** **[Virtual Display Module](guide/virtual_display.md)** — scaling/blend modes, z-order, patterns (CRT, PiP, 3D, compute target), troubleshooting, API quick reference.

**Per-symbol API detail:** [Graphics Module — Virtual Display](guide/graphics.md#virtual-displays).

**Trap table (see guide for detail):** wrong z-order, missing dirty flag, wrong scaling for pixel art — [Troubleshooting](guide/virtual_display.md#troubleshooting).

```mermaid
graph BT
    World[World VD Z=0] --> Comp[Compositor]
    UI[UI VD Z=10] --> Comp
    Comp --> Main[Main window]
```

**Canonical example:** `examples/05_virtual_display_retro/` — [Examples & FAQ](guide/examples_faq.md#05-virtual-display-retro).

<a id="361-creating-a-virtual-display"></a>
### 3.6.1 Creating a Virtual Display

→ [Creating a Virtual Display](guide/virtual_display.md#creating-a-virtual-display) · `SituationCreateVirtualDisplay` / `SituationCreateVirtualDisplayEx`

<a id="362-rendering-to-a-virtual-display"></a>
### 3.6.2 Rendering To a Virtual Display

→ [Rendering into a VD](guide/virtual_display.md#rendering-into-a-vd)

<a id="363-rendering-the-virtual-display-compositing"></a>
### 3.6.3 Rendering The Virtual Display (Compositing)

→ [Compositing](guide/virtual_display.md#compositing-situationrendervirtualdisplays) · `SituationRenderVirtualDisplays`

<a id="366-content-update-idle-fallback"></a>
### 3.6.6 Content update tracking & idle fallback

When a Virtual Display stops receiving **pixel writes**, the compositor must not show a frozen texture indefinitely. Situation tracks **content writes** separately from the VD **frame clock**:

| Field / API | Role |
|-------------|------|
| `last_update_time_seconds` | Per-frame animation clock (`SituationUpdateTimers`) |
| `last_content_update_time` | Timestamp of last draw / copy / compute write |
| `SituationGetVirtualDisplayUpdateInfo` | Query seconds/frames since last content write |
| `SituationSetVirtualDisplayIdleThreshold` | Idle when `(now - last_content_update_time) > threshold` |
| `SituationSetVirtualDisplayFallbackMode` | Standby: **SOLID**, **COLORBURST**, or **PATTERN** |
| `SituationSetVirtualDisplayPatternConfig` | Full pattern layers/tuning when mode is **PATTERN** |

**Full guide:** [Virtual Display — idle fallback](guide/virtual_display.md#idle-fallback-compositor-safety-net) · [Test patterns](guide/test_patterns.md#idle-compositor-flow-per-vd-layer)

Shipped: v2.4.221 (tracking API), v2.4.222 (SOLID/COLORBURST compositor), v2.4.344+ (**PATTERN** standby).

<a id="367-2d-grid-playfields-compute"></a>
### 3.6.7 2D Grid playfields (compute)

For **tile-map-style worlds** where each layer is a uniform cell grid (not free-form quads), use the grid subsystem: `SituationGridCreate` → edit `SitGridCell` (`code` + `fg` + `bg`) → stack layers with `SituationGridStack` → `SituationGridStackPresent` into a **`SITUATION_VD_FLAG_COMPUTE_TARGET`** → `SituationRenderVirtualDisplays`.

**Full guide:** **[2D Grid Module](guide/grid.md)** — cell model, scroll, compositing order, tile atlases, troubleshooting.

**Canonical example:** `examples/27_grid_playfield/` — [Examples & FAQ](guide/examples_faq.md#27-grid-playfield).

**Pairs with:** [Virtual Display — compute target](guide/virtual_display.md#pattern-compute-written-texture) · [Fonts — bitmap atlas from texture](guide/font.md#situationloadbitmapfontfromtexture) · [Compute](guide/compute.md).

<a id="364-configuration--optimization"></a>
### 3.6.4 Configuration & Optimization

→ [Runtime configuration](guide/virtual_display.md#runtime-configuration) · `SituationConfigureVirtualDisplay` · dirty flags

<a id="365-example-pixel-art-setup"></a>
### 3.6.5 Example: Pixel Art Setup

→ [Quick start — retro CRT](guide/virtual_display.md#quick-start-retro-crt-example-05)

<a id="37-compute--gpgpu"></a>

## 3.7 Compute & GPGPU

**Full guide:** **[Compute Shaders Module](guide/compute.md)** — layouts, dispatch, barriers, particles, readback, compute-only present, and VD compute targets.
...
For massive data arrays (e.g., GPU-driven rendering, vertex pull), query **buffer device addresses (BDA)** and pass them via push constants or `buffer_reference` in GLSL. Requires `SIT_FEATURE_BINDLESS_BUFFERS` on Vulkan.

| API | Returns |
|-----|---------|
| `SituationGetBufferDeviceAddress(buffer)` | GPU VA of a `SituationBuffer` |
| `SituationGetMeshVertexBufferAddress(mesh)` | GPU VA of mesh vertex data (§3.4.7) |
| `SituationGetMeshIndexBufferAddress(mesh)` | GPU VA of mesh index data (0 if unsupported) |

Access in shaders via `GL_EXT_buffer_reference` / `sit/gpu/vertex_pull.glslh`. See §3.4.7 for the pull draw recipe.
...
Situation ships **three complementary layers** for performance work. They are designed to be harmless when unused (near-zero cost) and composable:

| Layer | When to use | Cost when off |
| :--- | :--- | :--- |
| **P10.0 — Lightweight metrics** | Always-on spike detection, HUD, regression harnesses | Near-zero (monotonic timers + counters) |
| **P10.1 — `SituationGetFrameProfile`** | Headless snapshots, CI asserts, QSR training loops | Same as P10.0 (wraps existing getters) |
| **P10.2 — Tracy CPU zones** | Interactive stutter hunts, multi-thread timeline | **Zero** (macros expand to no-ops unless `SIT_TRACY=1`) |
| **P10.3 — GPU timestamp zones** | Per-pass GPU elapsed ns in `gpu_zone_ns[]` | **Zero** when unsupported; near-zero when supported (query pool only) |

Readback is **one frame slot late** (after the GPU fence). P10.4 user pools: **`SituationCreateQueryPool`** — see **[guide/profiling.md](guide/profiling.md)** §P10.4 and §3.8.4 below.

See also: [`guide/profiling.md`](guide/profiling.md), [`COMPILATION_GUIDE.md`](COMPILATION_GUIDE.md) (Tracy build flags), [`plan/AAA_ARCHITECTURE_PLAN.md`](plan/AAA_ARCHITECTURE_PLAN.md) §6, harness `--module frame_profile`.

<a id="381-metrics-overlay"></a>

### 3.8.1 Metrics Overlay
...
<a id="382-frame-spike--phase-timers-p100"></a>

### 3.8.2 Frame Spike & Phase Timers (P10.0)

Lightweight counters and phase breakdowns run on every frame. Use them for custom telemetry, automated spike tests, or to explain *why* a frame was slow.

#### Spike counters

```c:disable-run
double   SituationGetMaxFrameTime(void);    // seconds — highest observed frame delta since last reset
uint32_t SituationGetFrameSpikeCount(void);  // frames exceeding internal spike threshold
```

Call **`SituationResetFrameProfileStats()`** (P10.1) to clear max frame time, spike count, and the internal histogram buckets between test runs.

#### Last-frame phase attribution (render thread)

```c:disable-run
void SituationGetLastFramePhases(
    uint64_t* backpressure_ns,  // CPU waited on render queue / backpressure
    uint64_t* fence_wait_ns,    // GPU fence wait inside acquire
    uint64_t* execute_ns,       // command recording + submit on render thread
    uint64_t* present_ns);      // swap / present
```

When **`execute_ns`** or **`present_ns`** dominates, the bottleneck is GPU-side. When **`backpressure_ns`** or **`fence_wait_ns`** dominates, the CPU is waiting on the render thread or GPU completion.

#### Frame time histogram (text export)

```c:disable-run
void SituationExportRenderHistogram(char* buf, size_t buf_size);
```

Writes ASCII bucket counts (`<1ms`, `1–2ms`, … `>16ms`) into a caller buffer — useful for logging or debug consoles without Tracy.

#### Render latency & queue depth

When **`SITUATION_ENABLE_RENDER_THREAD`** is defined (default):
...
**Queue depth:** number of frames queued for the render thread. A consistently full queue (`depth == max`) means the GPU cannot keep up with the CPU.
...
<a id="383-structured-frame-profile-p101"></a>

### 3.8.3 Structured Frame Profile (P10.1)

For harnesses, headless regression, or QSR-style loops, prefer one snapshot call instead of many getters.

#### `SituationFrameProfile`

Defined in `situation_api_types_gpu.h` (`SITUATION_FRAME_PROFILE_VERSION`):

| Field | Meaning |
| :--- | :--- |
| `frame_time_ms` | Current frame delta (ms) |
| `max_frame_time_ms` | Peak since last reset |
| `spike_count` | Spike counter |
| `backpressure_ns` … `present_ns` | Same as `GetLastFramePhases` |
| `poll_ns`, `update_ns` | Main-thread `PollInputEvents` / `UpdateTimers` |
| `render_latency_avg_ns`, `render_latency_max_ns` | Input-to-present (render thread builds) |
| `queue_depth` | Render queue depth |
| `gpu_zone_ns[16]` | P10.3 GPU zone elapsed ns (`SituationGPUProfileZone` IDs); zero when unsupported or zone unused |
...
**`SituationGetFrameProfile`:** Non-allocating `memcpy`-style snapshot; safe after init from any thread. Sets `struct_version` / `struct_size` for forward compatibility.

**`SituationResetFrameProfileStats`:** Clears max frame time, spike count, and histogram only — does **not** zero last-frame phase timers.

**Harness:** `sit_test.exe --module frame_profile` (4 tests, GL + VK): struct parity, reset, user GPU zone (GL), overflow guard.

```c:disable-run
SituationFrameProfile profile = {0};
SituationGetFrameProfile(&profile);
if (profile.spike_count > 0) {
    printf("spikes=%u max=%.2f ms execute=%llu ns\n",
           profile.spike_count, profile.max_frame_time_ms,
           (unsigned long long)profile.execute_ns);
}
// Between CI scenarios:
SituationResetFrameProfileStats();
```

<a id="384-gpu-timestamp-zones-p103"></a>

### 3.8.4 GPU Timestamp Zones (P10.3)

When **`SituationIsFeatureSupported(SIT_FEATURE_GPU_TIMESTAMPS)`** is true, record GPU elapsed time into fixed zone slots. Results appear in **`SituationGetFrameProfile` → `gpu_zone_ns[]`** (nanoseconds).

#### Zone IDs (`SituationGPUProfileZone`)

| ID | Symbol | Used by |
| :--- | :--- | :--- |
| 0 | `SITUATION_GPU_ZONE_COMPOSITE` | Full `SituationRenderVirtualDisplays` pass |
| 1 | `SITUATION_GPU_ZONE_VD_PATH_A` | VD advanced-blend path (GL; last draw if multiple) |
| 2 | `SITUATION_GPU_ZONE_VD_PATH_B` | VD simple alpha path (GL; last draw if multiple) |
| 3 | `SITUATION_GPU_ZONE_TEXT_BATCH` | Internal batched text draw (GL) |
| 4–15 | `SITUATION_GPU_ZONE_USER_0` … `_USER_11` | App code via `SituationCmdGPUZoneBegin/End` |
...
**OpenGL:** zones may be recorded inside an active render pass (soft-buffer replay uses `GL_TIME_ELAPSED`).

**Vulkan:** user zones **must be recorded outside** an active render pass (`SITUATION_ERROR_PROFILING_ZONE_STATE` if inside). Internal composite zones are recorded around the VD compositor loop.

**Errors:** `-563` unsupported, `-564` bad `zone_id`, `-565` begin/end mismatch.

```c:disable-run
if (SituationIsFeatureSupported(SIT_FEATURE_GPU_TIMESTAMPS)) {
    SituationCmdGPUZoneBegin(cmd, SITUATION_GPU_ZONE_USER_0);
    SituationCmdClearColor(cmd, (ColorRGBA){64, 64, 64, 255}); /* GPU work inside zone */
    SituationCmdGPUZoneEnd(cmd, SITUATION_GPU_ZONE_USER_0);
}
/* After ~MAX_FRAMES_IN_FLIGHT frames: */
SituationFrameProfile p;
SituationGetFrameProfile(&p);
uint64_t user0_ns = p.gpu_zone_ns[SITUATION_GPU_ZONE_USER_0];
```

<a id="385-tracy-cpu-profiling-p102"></a>

### 3.8.5 Tracy CPU Profiling (P10.2)

Tracy adds labeled CPU zones on a live timeline (main, render, audio, I/O, worker threads). Default builds **do not** link Tracy — macros are no-ops with zero overhead.

#### Build (library + app must agree)

```powershell
# PowerShell — Tracy-enabled DLL
$env:SIT_TRACY = "1"
& ".\build\build_situation.bat" opengl tracy
```

Or pass `tracy` as the second argument to `build_situation.bat`. This defines **`SITUATION_ENABLE_TRACY`** and links `build/tracy_client.cpp` (vendored client under `ext/tracy/public/`).

**App compile lines** must also use `-DSITUATION_ENABLE_TRACY -Iext/tracy/public` when you want your zones to appear in captures.

Full flags and OpenGL link notes (`g++`, `-ldbghelp`, `-lsecur32`): **[COMPILATION_GUIDE.md](COMPILATION_GUIDE.md)** → Tracy profiling.

#### Attach the profiler

1. Build Situation and your app with Tracy enabled.
2. Run your app.
3. Launch the [Tracy profiler](https://github.com/wolfpld/tracy) and connect to the running process (default port **8086**).

Library instrumentation (always present when Tracy is on) includes:

- **`SituationEndFrame`** — frame mark (`SIT_PROFILE_FRAME_MARK`)
- **`SituationAcquireFrameCommandBuffer`** — backpressure / fence-wait zones
- **`RenderThread/Frame`**, **`SubmitGraphics`**, **`SubmitCompute`**
- **`SituationPollInputEvents`**, **`SituationUpdateTimers`**
- **`SituationPollShaderLoad`** (early-return safe ctx macros)
- **`ThreadPool/Job`**, **`AudioCallback`**

#### User zones (macros in `situation_profiling.h`, included from `situation.h`)

```c:disable-run
// Multi-statement block (preferred)
SIT_PROFILE_ZONE_SCOPED("MyGame/Update") {
    update_entities();
    update_physics();
}

// Early return — named context
SIT_PROFILE_ZONE_CTX(ctx, "MyGame/LoadShader");
if (!ready) SIT_PROFILE_RETURN_CTX(ctx, SITUATION_ERROR_SHADER_LOAD_IN_PROGRESS);
SIT_PROFILE_ZONE_END_CTX(ctx);

SIT_PROFILE_FRAME_MARK();  // optional per-subsystem frame boundary in app code
```

Public aliases: **`SIT_PROFILE_*`** (same as **`SIT_PROF_*`**). When Tracy is off, scoped macros compile to empty loops — no Tracy headers required in app TUs unless you enable Tracy.

**When to use Tracy vs `GetFrameProfile`:** Tracy for interactive multi-thread captures; **`GetFrameProfile`** for headless CI and programmatic thresholds without a profiler attached.

<a id="386-debug-builds--thread-visibility"></a>

### 3.8.6 Debug Builds & Thread Visibility

#### Debug rebuilds

Root **`debug.bat`** rebuilds with `-O0 -g` and can launch `sit_test_*.exe` under GDB. Manual equivalent:
...
See **[COMPILATION_GUIDE.md](COMPILATION_GUIDE.md)** → *Debug Builds* and section **Debug Builds** at the end of this document.

#### OS-visible thread names

Library threads are named for Task Manager, Process Explorer, and debuggers: **`Sit Main`**, **`Sit Worker N`**, **`Sit I/O`**, **`Sit Render`**, **`Sit Audio`**. Your worker code can call:

```c:disable-run
SituationSetCurrentThreadName("My Worker");
```

#### Logging & GL debug

API misuse is caught via return codes and the logging module. Debug builds use **`SIT_ASSERT_MAIN_THREAD()`** and OpenGL debug callbacks — see **[Logging Module](guide/logging.md)**.
...
**Module guides:** [Fonts](guide/font.md) (load, bake, bitmap builders, measure, lifecycle) · [Text rendering](guide/text_rendering.md) (GPU pipeline, `SituationCmdDrawText*`, metrics overlay) · [Image](guide/image.md) (CPU text onto pixel buffers).

### 3.9.1 The Font Workflow

To render text, you must first prepare a font atlas. This converts vector glyphs (TTF/OTF) into a GPU-compatible texture. Full coverage (TTF, 1bpp bitmap, terminal/VCR/VGA builders, measurement, unload): see [Fonts Module](guide/font.md).
...
**Authoritative catalog:** **[situation_command_reference.md](situation_command_reference.md)** — all **`SituationCmd*`** functions (71 in v2.4.331 index) plus deprecated aliases, with:
...
| Per-sound DSP / effects | `SituationSound*` + `SituationAttachAudioProcessor` |
| Procedural / MIDI tones | `SituationPlayToneEx` + graph routing |
...
**Canonical:** `SituationEnumerateAudioDevices` + `SituationFreeDeviceList`.  
**Deprecated:** `SituationGetAudioDevices` (compile-time warning via `SIT_DEPRECATED`; still callable).

To enumerate available playback hardware:
...
**Returns:** A heap-allocated array. Free with **`SituationFreeDeviceList(devices, count)`** (not raw `free()` unless you know the allocator matches).

#### SituationGetAudioDevices _(deprecated)_

```c:disable-run
SituationAudioDeviceInfo* SituationGetAudioDevices(int* count);
```

Legacy alias-style API. Prefer **`SituationEnumerateAudioDevices`** for new code.
...
>
> **Note:** Generational thread pool and async I/O queues ship in §7.0–§7.1. Further audio concurrency work is tracked for v2.5+.
...
(See **Path Carnage** table above in §6.0.)
...
> **Forward Look:** Need to mount a ZIP file as a virtual drive? **Virtual Mounts (PAK support)** are planned for v2.5. Video streaming (`sit/vid/`) is not yet implemented — see folder tree note in § “What's New in v2.4.0”.
>
> **Shipped:** Low-priority **async I/O queues** — `SituationSubmitIOJob`, `SituationPollIOJob` (§7.1).
...
*   **Dual Priority Queues:**
    *   **High Priority (Index 1):** Serviced by the general Worker Pool. Used for frame-critical tasks like Physics steps, Animation updates, Audio DSP, and **Vulkan async shaderc** (v2.4.105+). Workers always drain this queue before checking low priority items.
...
*   **Head-of-Line mitigation (v2.4.232–233):** Workers claim ready jobs **in-place** with a `CLAIM_BIT` CAS — jobs are never struct-swapped in the ring (preserves generational handles). Scan covers the **full pending range** (`head - tail`), not a capped window.
*   **OS thread names (v2.4.239):** `SituationInitInfo::main_thread_name` (default `"Sit Main"`) and **`SituationSetCurrentThreadName`** for any thread. Workers, I/O, render, and audio threads use built-in names (`"Sit Worker N"`, `"Sit I/O"`, etc.).
...
**582 public functions** are indexed in **[situation_api_index.md](situation_api_index.md)** (auto-generated from `sit/situation_api.h`, v2.4.331). **Full prose** for most symbols is in **[situation_api.md](situation_api.md)** (548/582 expanded; gaps in **[situation_api_generated.md](situation_api_generated.md)**).
...
| Graphics / SPIR-V / async shaders | §3.0–§3.4 | situation_api.md — Graphics; doc/misc/TEST_SPIRV_SHADER_API.md |
...
### Language Wrappers (Odin, Zig, Rust, Fortran, Modula-2, Python, Lua)

Official FFI bindings and interactive `hello_situation` demos are in `wrappers/`. Generators live in `tools/` (`generate_*_bindings.py`, wired into `tools/run_all.bat`). Example builders mirror C `build_examples.bat` backends:
...
Use `opengl` or `vulkan` for DLL-linked builds (DLL copied next to the exe), or `static-opengl` / `static-vulkan` for self-contained exes (not supported for Python v1 or Lua — see below).

| Language | Static OpenGL | Static Vulkan | Toolchain notes |
|----------|:-:|:-:|-------|
| Rust | ✅ | ✅ | GCC linker; `build.rs` handles `libgcc_eh` |
| Fortran | ✅ | ✅ | `gfortran`; static Vulkan final link via `g++` + `-lgfortran` |
| Modula-2 | ✅† | ✅† | `gm2 -c` then `gcc`/`g++` link — †when `gm2` is installed or bundled |
| Python | ❌ | ❌ | **DLL only v1** — PyInstaller `.exe` + `ctypes`; see `wrappers/Python/README.md` |
| Lua | ❌ | ❌ | **Embedded DLL exe** — `opengl`/`vulkan`; Situation + lua51 DLLs inside `.exe`; see `wrappers/lua/README.md` |
| Zig | ✅ | ❌ | Use DLL mode for Vulkan (`lld-link` vs MinGW C++ archives) |
| Odin | ❌ | ❌ | Use DLL mode (`lld-link` vs MinGW `.a` archives) |

**Status (v2.4.363):** All seven wrappers regenerate via `tools/run_all.bat`. Lua ships as `build/examples/lua/hello_situation.exe` (embedded bytecode + DLLs). Python `hello_situation.exe` builds via PyInstaller against OpenGL/Vulkan DLLs. Dev Lua iteration: `build/run_lua_dev.bat`.

See **COMPILATION_GUIDE.md** → *Language Wrappers* and *Debug Builds*; per-language docs: `wrappers/lua/README.md`, `wrappers/Python/README.md`, `wrappers/Rust/README.md`, `wrappers/Zig/README.md`, `wrappers/Fortran/README.md`, `wrappers/Modula2/README.md`, `wrappers/Odin/MANUAL_BINDINGS.md`.

### Debug Builds

For library + harness debugging, use root **`debug.bat`** (rebuilds with `-O0 -g`, launches `sit_test_*.exe` under GDB). For manual debug flags on any target:
...
Full options (`--no-build`, `--break`, `clean`, `distclean`) are in **COMPILATION_GUIDE.md** → *Debug Builds* and **BUILD_SITUATION_GUIDE.md** → *Environment Overrides*.

### Linux Support
*   **Compilation:** The library compiles cleanly on Linux with GCC/Clang.
*   **Dependencies:** Windows builds use `ext/glfw/build/src/libglfw3.a` (built by the Makefile) or `ext/glfw/lib/libglfw3.a` — both are **Windows PE/COFF** archives and will **not** link on Linux.
