...
| Metadata | Details |
| :--- | :--- |
| **Version** | **2.4.258** — canonical macros in **`sit/situation_base_version.h`**. Narrative releases: **`doc/UPDATELOG.md`**. |
...
| **[COMPILATION_GUIDE.md](COMPILATION_GUIDE.md)** | **Build system reference** — static/DLL builds, GLFW setup, linker flags, C examples, **Odin/Zig/Rust wrapper examples** |
...
└── sit/                                # ← Implementation
    ...
    ├── situation_impl_renderer.h       # GL + VK backends (~26K LOC)
...
Official FFI bindings and interactive `hello_situation` demos are in `wrappers/`. Example builders mirror C `build_examples.bat` backends:
...
Use `opengl` or `vulkan` for DLL-linked builds (DLL copied next to the exe), or `static-opengl` / `static-vulkan` for self-contained exes. See **COMPILATION_GUIDE.md** → *Language Wrapper Examples* and per-language READMEs under `wrappers/Odin/`, `wrappers/Zig/`, `wrappers/Rust/`.
