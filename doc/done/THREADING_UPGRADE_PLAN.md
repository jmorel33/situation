...
- [x] Run `./build/threading_diagnostic_test.exe` — capability check
- [x] Run `./build/threading_stress_test.exe` — stress test (catches HOL, races)
- [x] Verify Patch 2: dependent job handling in DispatchParallel
- [x] Verify Patch 3: throughput with dependency-heavy workloads
- [x] Verify Patch 5: allocation failure path
- [x] Full regression: all examples and harness threading module

Superseded by the official test harness `--module threading` which covers all of the above
and passes as part of the standard harness run.
...
**Author**: Kiro  
**Status**: ✅ Complete — all patches applied and verified via test harness threading module  
**Approval**: Done
