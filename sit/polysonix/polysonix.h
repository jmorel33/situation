...
typedef struct Voice {
    ...
    // Cached mod matrix results (computed at LFO rate, reused at audio rate)
    float cached_mod_sources[PX_MOD_SRC_COUNT];
    float cached_dest_mod[PX_MOD_DEST_COUNT];
} PX_ALIGN_64 Voice;
...
static void Filter_SetCoefficients(Filter* filter, float cutoff_hz, float resonance_q, PxFilterMode mode, int poles, float sample_rate) {
    ...
    float tan_val = tanf((float)M_PI * cutoff_hz / sample_rate);
...
static float Filter_Process_Internal(Filter* filter, float input_sample) {
    ...
    float dc_blocked = fmaf(dc_block_coeff, filter->dc_block_y1, input_sample - filter->dc_block_x1);
    ...
    if (filter->poles == 1) {
        ...
        filter->combo_lp_state = fmaf(filter->pole3_coeff, driven_input - filter->combo_lp_state, filter->combo_lp_state);
        filter->combo_hp_state = fmaf(filter->pole3_coeff, driven_input - filter->combo_hp_state, filter->combo_hp_state);
        ...
        switch (filter->current_mode) {
            ...
            case PX_FILTER_MODE_ALLPASS:  output = fmaf(-2.0f, bp, driven_input); break;
    ...
    float notch1 = fmaf(-filter->q_inv_coeff, filter->bp_state1, driven_input);
    ...
    if (filter->poles == 3) {
        ...
        filter->pole3_lp_state = fmaf(filter->pole3_coeff, final_lp - filter->pole3_lp_state, filter->pole3_lp_state);
        final_lp = filter->pole3_lp_state;

        // 18dB HP = 12dB HP -> 6dB HP stage
        filter->pole3_hp_state = fmaf(filter->pole3_coeff, final_hp - filter->pole3_hp_state, filter->pole3_hp_state);
        final_hp = final_hp - filter->pole3_hp_state;

        // 18dB BP = 12dB BP -> 6dB LP stage (to smooth the peak)
        filter->pole3_bp_state = fmaf(filter->pole3_coeff, final_bp - filter->pole3_bp_state, filter->pole3_bp_state);
        final_bp = filter->pole3_bp_state;

    } else if (filter->poles == 4) {
        // --- Correct 24dB Logic ---
        float stage2_input = filter->lp_state1;
        float notch2 = fmaf(-filter->q_inv_coeff, filter->bp_state2, stage2_input);
...
static float GenerateLFOValue(LFOInstance* lfo_instance) {
    ...
    lfo_instance->lfo_vm_params.x = lfo_instance->phase * 2.0f * (float)M_PI;
...
PX_API PxSynth* PX_Create(const PxConfig* config) {
    ...
    PxSynth* s = (PxSynth*)SIT_CALLOC(1, sizeof(PxSynth));
    if (!s) return NULL;
    ...
    s->cmd_queue.buffer = (PxQueueSlot*)SIT_CALLOC(CMD_QUEUE_SIZE, sizeof(PxQueueSlot));
    if (!s->cmd_queue.buffer) { SIT_FREE(s); return NULL; }
    ...
    s->patch.template_voice_adsrs = (PxADSRParams*)SIT_CALLOC(config->num_voice_adsrs, sizeof(PxADSRParams));
    s->patch.template_voice_adsr_mod_amounts = (float*)SIT_CALLOC(config->num_voice_adsrs * PX_ADSR_DEST_COUNT, sizeof(float));
    s->patch.template_lfos = (PxLFOParams*)SIT_CALLOC(config->num_lfos, sizeof(PxLFOParams));
    s->ui_snapshot.patch_copy.template_voice_adsrs = (PxADSRParams*)SIT_CALLOC(config->num_voice_adsrs, sizeof(PxADSRParams));
    s->ui_snapshot.patch_copy.template_voice_adsr_mod_amounts = (float*)SIT_CALLOC(config->num_voice_adsrs * PX_ADSR_DEST_COUNT, sizeof(float));
    s->ui_snapshot.patch_copy.template_lfos = (PxLFOParams*)SIT_CALLOC(config->num_lfos, sizeof(PxLFOParams));

    // 5. Allocate memory for the main voice pool and the template LFO instances.
    s->voices = (Voice*)SIT_CALLOC(config->num_voices, sizeof(Voice));
    s->template_lfo_instances = (LFOInstance*)SIT_CALLOC(config->num_lfos, sizeof(LFOInstance));
    ...
    for (int i = 0; i < config->num_voices; ++i) {
        ...
        s->voices[i].adsrs = (ADSR*)SIT_CALLOC(config->num_voice_adsrs, sizeof(ADSR));
        s->voices[i].lfo_instances = (LFOInstance*)SIT_CALLOC(config->num_lfos, sizeof(LFOInstance));
        s->voices[i].lfo_mod_amounts_snapshot = (float*)SIT_CALLOC(config->num_lfos * PX_LFO_DEST_COUNT, sizeof(float));
        s->voices[i].adsr_mod_amounts = (float*)SIT_CALLOC(config->num_voice_adsrs * PX_ADSR_DEST_COUNT, sizeof(float));
    ...
    s->limiter.delay_line_l = (float*)SIT_CALLOC(limiter_buf_size, sizeof(float));
    s->limiter.delay_line_r = (float*)SIT_CALLOC(limiter_buf_size, sizeof(float));
...
PX_API void PX_Destroy(PxSynth* s) {
    ...
    if (s->voices) {
        for (int i = 0; i < s->config.num_voices; ++i) {
            SIT_FREE(s->voices[i].adsrs);
            SIT_FREE(s->voices[i].lfo_instances);
            SIT_FREE(s->voices[i].lfo_mod_amounts_snapshot);
            SIT_FREE(s->voices[i].adsr_mod_amounts);
        }
        SIT_FREE(s->voices);
    }
    if (s->template_lfo_instances) SIT_FREE(s->template_lfo_instances);
    if (s->patch.template_voice_adsrs) SIT_FREE(s->patch.template_voice_adsrs);
    if (s->patch.template_voice_adsr_mod_amounts) SIT_FREE(s->patch.template_voice_adsr_mod_amounts);
    if (s->patch.template_lfos) SIT_FREE(s->patch.template_lfos);
    if (s->ui_snapshot.patch_copy.template_voice_adsrs) SIT_FREE(s->ui_snapshot.patch_copy.template_voice_adsrs);
    if (s->ui_snapshot.patch_copy.template_voice_adsr_mod_amounts) SIT_FREE(s->ui_snapshot.patch_copy.template_voice_adsr_mod_amounts);
    if (s->ui_snapshot.patch_copy.template_lfos) SIT_FREE(s->ui_snapshot.patch_copy.template_lfos);
    if (s->cmd_queue.buffer) SIT_FREE(s->cmd_queue.buffer);
    if (s->limiter.delay_line_l) SIT_FREE(s->limiter.delay_line_l);
    if (s->limiter.delay_line_r) SIT_FREE(s->limiter.delay_line_r);
    if (s->config.use_gpu) {
#ifdef POLYSONIX_USE_GPU
    ...
    for (int v_idx = 0; v_idx < voices_to_calc; ++v_idx) {
        ...
        if (v->active) {
            ...
            if (fabsf(current_pan - v->last_pan_effective) > 0.001f) {
                float pan_angle = (current_pan + 1.f) * .25f * (float)M_PI;
    ...
    for (int i = 0; i < num_frames; i++) {
        ...
        if (s->lfo_update_countdown <= 0) {
            ...
            for (int v_idx = 0; v_idx < s->config.num_voices; ++v_idx) {
                ...
                // Cache for audio-rate reuse
                memcpy(v->cached_mod_sources, mod_sources, sizeof(mod_sources));
                memcpy(v->cached_dest_mod, dest_mod, sizeof(dest_mod));
        ...
        for (int v_idx = 0; v_idx < s->config.num_voices; ++v_idx) {
            ...
            // === v1.3: Apply Modulation Matrix (cached from LFO update block) ===
            float* mod_sources = v->cached_mod_sources;
            float* dest_mod = v->cached_dest_mod;
            ...
            for (int o = 0; o < PX_MAX_OSC_PER_VOICE; ++o) {
                ...
                if (pd_amount > 0.001f) {
                    ...
                    effective_phase = fmaf(sinf(effective_phase * 2.0f * (float)M_PI), pd_amount * 0.5f, effective_phase);
                    // Wrap if needed, though sin is periodic so it might just warp safely.
                }
                v->osc[o].vm_params.x = effective_phase * 2.f * (float)M_PI;
                ...
                if (sq->current_sequence) {
                    ...
                    if (sq->step_flags & PX_WSEQ_AMP_MOD) {
                        ...
                        switch (sq->current_sequence->amp_mod_type) {
                            ...
                            case PX_WSEQ_AMP_SINE:      amp_scalar = sinf(t * (float)M_PI); break;
                    ...
                    if (sq->step_flags & PX_WSEQ_RING_MOD) {
                        float ring_mod_src = sinf(v->osc[o].phase * 4.0f * (float)M_PI);
