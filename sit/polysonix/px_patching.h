...
PX_API bool PX_SavePresetToBus(PxSynth* s, PxIOWriteFn write_fn, void* token, const char* patch_name) {
    ...
    uint8_t* buffer = (uint8_t*)SIT_MALLOC(total_size);
...
PX_API bool PX_LoadPresetFromBus(PxSynth* s, PxIOReadFn read_fn, void* token, char* error_msg, size_t error_len) {
    ...
    uint8_t* buffer = (uint8_t*)SIT_MALLOC(payload_size);
    if (!buffer) {
...
static bool px_allocate_patch_memory(PxPatch* p, const PxConfig* config) {
    p->template_voice_adsrs = (PxADSRParams*)SIT_CALLOC(config->num_voice_adsrs, sizeof(PxADSRParams));
    p->template_voice_adsr_mod_amounts = (float*)SIT_CALLOC(config->num_voice_adsrs * PX_ADSR_DEST_COUNT, sizeof(float));
    p->template_lfos = (PxLFOParams*)SIT_CALLOC(config->num_lfos, sizeof(PxLFOParams));

    if (!p->template_voice_adsrs || !p->template_voice_adsr_mod_amounts || !p->template_lfos) {
        if (p->template_voice_adsrs) SIT_FREE(p->template_voice_adsrs);
        if (p->template_voice_adsr_mod_amounts) SIT_FREE(p->template_voice_adsr_mod_amounts);
        if (p->template_lfos) SIT_FREE(p->template_lfos);
        return false;
    }
    return true;
}
...
PX_API PxPatchBank* PX_CreatePatchBank(const PxConfig* config) {
    ...
    PxPatchBank* bank = (PxPatchBank*)SIT_CALLOC(1, sizeof(PxPatchBank));
    if (!bank) return NULL;

    bank->config = *config;
    bank->patches = (PxPatch*)SIT_CALLOC(PX_PATCH_BANK_SIZE, sizeof(PxPatch));
    if (!bank->patches) {
        SIT_FREE(bank);
        return NULL;
    }
