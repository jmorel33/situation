...
static void processBuffer(PhaseShifter *ps, float *buffer, int num_samples, int num_channels) {
    for (int i = 0; i < num_samples; i++) {
        ...
        // Dry mix base
        float output_left = input_left * (1.0f - ps->mix);
        float output_right = input_right * (1.0f - ps->mix);
        ...
        for (int c = 0; c < 2; c++) {
            ...
            // Cross-mix: add this wet's contributions to both outputs
            output_left += x[c] * gain_to_left * ps->mix;
            output_right += x[c] * gain_to_right * ps->mix;
        }

        // Stereo widening
        float mid = (output_left + output_right) / 2.0f;
        float side = (output_left - output_right) / 2.0f;
        output_left = mid + ps->stereo_width * side;
