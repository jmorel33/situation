...
/* MinGW links IID_IDXGIOutput6 via -luuid; keep explicit GUID for header-only COM in wdm.h. */
static const IID SIT_IID_IDXGIOutput6 =
...
#if defined(_WIN32)
/* _SituationDxgiFillDisplayHdrMetadata — forward-declared in situation_impl_forward.h */
#endif
...
static BOOL CALLBACK _SituationMonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData) {
    ...
    if (GetMonitorInfoA(hMonitor, (LPMONITORINFO)&monitor_info_ex)) {
        ...
        if (EnumDisplaySettingsA(monitor_info_ex.szDevice, ENUM_CURRENT_SETTINGS, &current_dev_mode)) {
            ...
            display->current_refresh_hz = (float)display->current_mode.refresh_rate;
        } else {
            memset(&display->current_mode, 0, sizeof(SituationDisplayMode));
            display->current_refresh_hz = 0.0f;
        }
        ...
        _SituationDxgiFillDisplayHdrMetadata(display, hMonitor);
        enum_data->current_display_idx++;
    }
    return TRUE;
}

/** Map Win32 HMONITOR (and optional \\.\DISPLAYn name) to IDXGIOutput6::GetDesc1 metadata. */
static void _SituationDxgiFillDisplayRefreshHz(SituationDisplayInfo* display, IDXGIOutput* output) {
    if (!display || !output) {
        return;
    }
    const int want_w = display->current_mode.width;
    const int want_h = display->current_mode.height;
    const int want_hz = display->current_mode.refresh_rate;
    if (want_w <= 0 || want_h <= 0) {
        return;
    }

    UINT mode_count = 0;
    if (FAILED(output->lpVtbl->GetDisplayModeList(output, DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED,
            &mode_count, NULL))
            || mode_count == 0) {
        return;
    }

    DXGI_MODE_DESC* modes = (DXGI_MODE_DESC*)SIT_MALLOC(mode_count * sizeof(DXGI_MODE_DESC));
    if (!modes) {
        return;
    }
    if (FAILED(output->lpVtbl->GetDisplayModeList(output, DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED,
    ...
    for (UINT i = 0; i < mode_count; ++i) {
        ...
        if ((int)mode->Width != want_w || (int)mode->Height != want_h) {
            continue;
        }
        if (mode->RefreshRate.Denominator <= 0) {
            continue;
        }
        const float hz = (float)mode->RefreshRate.Numerator / (float)mode->RefreshRate.Denominator;
        const int diff = (int)(fabsf(hz - (float)want_hz) + 0.5f);
        if (diff < best_diff) {
            best_diff = diff;
            best_hz = hz;
        }
    ...
    SIT_FREE(modes);
    if (best_hz > 0.0f) {
        display->current_refresh_hz = best_hz;
    }
}
...
static void _SituationDxgiFillDisplayHdrMetadata(SituationDisplayInfo* display, HMONITOR hMonitor) {
    ...
    display->hdr_supported = false;
    display->hdr_enabled = false;
    display->bits_per_color = 0;
    display->max_luminance_nits = 0.0f;
    display->dxgi_color_space = 0;
    display->dxgi_metadata_valid = false;

    WCHAR target_name_w[CCHDEVICENAME] = {0};
    if (display->name[0]) {
        MultiByteToWideChar(CP_ACP, 0, display->name, -1, target_name_w, CCHDEVICENAME);
    }
    ...
    bool matched = false;
    for (UINT adapter_idx = 0; !matched; ++adapter_idx) {
        IDXGIAdapter1* adapter = NULL;
        HRESULT enum_adapter = factory->lpVtbl->EnumAdapters1(factory, adapter_idx, &adapter);
        if (enum_adapter == DXGI_ERROR_NOT_FOUND) {
            break;
        }
        if (FAILED(enum_adapter) || !adapter) {
            break;
        }

        for (UINT output_idx = 0; ; ++output_idx) {
            IDXGIOutput* output = NULL;
            HRESULT enum_output = adapter->lpVtbl->EnumOutputs(adapter, output_idx, &output);
            if (enum_output == DXGI_ERROR_NOT_FOUND) {
                break;
            }
            if (FAILED(enum_output) || !output) {
                break;
            }

            DXGI_OUTPUT_DESC desc = {0};
            if (SUCCEEDED(output->lpVtbl->GetDesc(output, &desc))) {
                const bool monitor_match = (desc.Monitor == hMonitor);
                const bool name_match =
                    (target_name_w[0] != L'\0' && lstrcmpiW(desc.DeviceName, target_name_w) == 0);
                if (monitor_match || name_match) {
                    _SituationDxgiFillDisplayRefreshHz(display, output);
                    IDXGIOutput6* output6 = NULL;
                    if (SUCCEEDED(output->lpVtbl->QueryInterface(output, &SIT_IID_IDXGIOutput6, (void**)&output6))
                            && output6) {
                        DXGI_OUTPUT_DESC1 desc1 = {0};
                        if (SUCCEEDED(output6->lpVtbl->GetDesc1(output6, &desc1))) {
                            display->dxgi_metadata_valid = true;
                            display->bits_per_color = (uint8_t)desc1.BitsPerColor;
                            display->max_luminance_nits = desc1.MaxFullFrameLuminance;
                            display->dxgi_color_space = (uint32_t)desc1.ColorSpace;
                            display->hdr_enabled =
                                (desc1.ColorSpace == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020);
                            display->hdr_supported = display->hdr_enabled
                                || (desc1.BitsPerColor >= 10
                                    && (desc1.MaxFullFrameLuminance > 100.0f
...
#endif /* _WIN32 monitor enum + DXGI HDR probe */
    ...
    if (glfw_count > 0) {
        ...
        for (int i = 0; i < glfw_count; ++i) {
            ...
            if (mode) {
                ...
                disp->current_refresh_hz = (float)mode->refreshRate;
            }
            int vid_mode_count;
...
/** Refresh cached display HDR metadata when monitors are connected or disconnected. */
...
/** Window monitor id from cache; -1 if unknown. Usable once GLFW window exists (before SituationInit completes). */
static int _SituationResolveWindowMonitorId(void) {
    ...
    if (!current_glfw_monitor) {
        ...
        if (win_w <= 0 || win_h <= 0) {
            win_w = sit_gs.main_window_width;
            win_h = sit_gs.main_window_height;
        }
...
/** True when DXGI reports OS "Use HDR" active on the monitor containing the main window. */
static bool _SituationWindowMonitorDxgiHdrEnabled(void) {
#if defined(_WIN32)
    if (sit_gs.cached_physical_display_count == 0) {
        _SituationCachePhysicalDisplays();
    }
    const int mid = _SituationResolveWindowMonitorId();
    if (mid < 0 || mid >= sit_gs.cached_physical_display_count || !sit_gs.cached_physical_displays_array) {
        return false;
    }
    const SituationDisplayInfo* d = &sit_gs.cached_physical_displays_array[mid];
    return d->dxgi_metadata_valid && d->hdr_enabled;
#else
    return false;
#endif
}
...
SITAPI int _SituationGetCurrentDisplayIdentifier(void) {
    if (!SituationIsInitialized() || !sit_gs.sit_glfw_window) {
        _SituationSetErrorFromCode(SITUATION_ERROR_NOT_INITIALIZED, "GetCurrentDisplayIdentifier");
        return -1;
    }
    return _SituationResolveWindowMonitorId();
}
...
SITAPI SituationError SituationSetDisplayMode(int situation_monitor_id, const SituationDisplayMode* mode, bool fullscreen) {
    ...
    if (glfw_mon) {
        if (fullscreen) {
            // [FIX v2.4.320] Capture canvas before fullscreen (same as SituationSetWindowMonitor fix).
            if (glfwGetWindowMonitor(sit_gs.sit_glfw_window) == NULL) {
                int fb_w = 0, fb_h = 0;
                glfwGetFramebufferSize(sit_gs.sit_glfw_window, &fb_w, &fb_h);
                if (fb_w <= 0 || fb_h <= 0) {
                    glfwGetWindowSize(sit_gs.sit_glfw_window, &fb_w, &fb_h);
                }
                if (fb_w <= 0 || fb_h <= 0) {
                    fb_w = sit_gs.main_window_width;
                    fb_h = sit_gs.main_window_height;
                }
                sit_gs.render_canvas_width = fb_w;
                sit_gs.render_canvas_height = fb_h;
            }
 ...
 */
/* _SituationComputeWindowStateFlags — forward-declared in situation_impl_forward.h */
...
SITAPI void SituationSetVSync(bool enable) {
    ...
    // Update both focus profiles so alt-tab behavior matches (same pattern as SituationToggleFullscreen).
    if (enable) {
        sit_gs.active_profile_window_flags |= SITUATION_FLAG_VSYNC_HINT;
        sit_gs.inactive_profile_window_flags |= SITUATION_FLAG_VSYNC_HINT;
    } else {
        sit_gs.active_profile_window_flags &= ~SITUATION_FLAG_VSYNC_HINT;
        sit_gs.inactive_profile_window_flags &= ~SITUATION_FLAG_VSYNC_HINT;
    }
    ...
    // For Vulkan, recreate the swapchain with the new present mode.
    ...
    #endif
    // OpenGL: glfwSwapInterval is applied on the render thread immediately before present.
    // Do not call SituationApplyCurrentProfileWindowState here — vsync is not a GLFW window attribute.
}
...
SITAPI void SituationSetWindowMonitor(int monitor_id) {
    ...
    if (monitor_id >= 0 && monitor_id < display_count) {
        ...
        if (target_display->glfw_monitor_handle) {
            // [FIX v2.4.320] Capture windowed framebuffer into render canvas before fullscreen
            // transition. The framebuffer size callback skips canvas updates when a monitor is set
            // (exclusive fullscreen), so we must freeze the canvas at the current windowed size.
            // Without this, _SituationGLEnsureCanvasResources / _SituationVulkanEnsureCanvasResources
            // receive stale or zero dimensions and fail. Same pattern as
            // SituationApplyCurrentProfileWindowState.
            if (glfwGetWindowMonitor(sit_gs.sit_glfw_window) == NULL) {
                int fb_w = 0, fb_h = 0;
                glfwGetFramebufferSize(sit_gs.sit_glfw_window, &fb_w, &fb_h);
                ...
                sit_gs.render_canvas_width = fb_w;
                sit_gs.render_canvas_height = fb_h;
                // Also save windowed position/size for restoring later
                glfwGetWindowPos(sit_gs.sit_glfw_window, &sit_gs.windowed_x, &sit_gs.windowed_y);
                sit_gs.windowed_w = fb_w;
                sit_gs.windowed_h = fb_h;
            }
...
/**
 * @brief Refresh rate (Hz) of the primary physical display (monitor index 0).
 * @details Convenience wrapper for HUD / frame pacing when the window monitor is unknown.
 */
SITAPI int SituationGetDisplayRefreshRate(void) {
    return SituationGetMonitorRefreshRate(0);
}

/**
 * @brief Fractional nominal refresh rate (Hz) for a monitor's current video mode.
 * @details On Windows, prefers DXGI RefreshRate rational (e.g. 59.94) over integer DEVMODE/GLFW values.
 */
SITAPI float SituationGetMonitorRefreshRateHz(int monitor_id) {
    if (!SituationIsInitialized()) {
        _SituationSetErrorFromCode(SITUATION_ERROR_NOT_INITIALIZED, "SituationGetMonitorRefreshRateHz");
        return 0.0f;
    }
    if (sit_gs.cached_physical_display_count == 0) {
        _SituationCachePhysicalDisplays();
    }
    if (monitor_id < 0 || monitor_id >= sit_gs.cached_physical_display_count) {
        _SituationSetErrorFromCode(SITUATION_ERROR_INVALID_PARAM, "SituationGetMonitorRefreshRateHz: monitor_id out of range");
        return 0.0f;
    }
    const SituationDisplayInfo* disp = &sit_gs.cached_physical_displays_array[monitor_id];
    if (disp->current_refresh_hz > 0.0f) {
        return disp->current_refresh_hz;
    }
    if (disp->current_mode.refresh_rate > 0) {
        return (float)disp->current_mode.refresh_rate;
    }
    return 0.0f;
}

SITAPI float SituationGetDisplayRefreshRateHz(void) {
    return SituationGetMonitorRefreshRateHz(0);
}

/**
 * @brief Measured present rate (Hz) from the latest present-to-present interval.
 * @details Uses render-thread present timing when available; otherwise falls back to SituationGetFrameTime().
 */
SITAPI float SituationGetMeasuredPresentRateHz(void) {
    if (!SituationIsInitialized()) {
        _SituationSetErrorFromCode(SITUATION_ERROR_NOT_INITIALIZED, "SituationGetMeasuredPresentRateHz");
        return 0.0f;
    }
    ...
    if (sit_render.enabled) {
        ...
        if (present_delta_ns > 0) {
            return (float)(1000000000.0 / (double)present_delta_ns);
        }
    }
#endif
    if (sit_gs.frame_time > 0.0) {
        return (float)(1.0 / sit_gs.frame_time);
    }
    return 0.0f;
}
...
SITAPI Vector2 SituationGetWindowPosition(void) {
    ...
    pos.x = (float)sit_gs.window_pos_x;
    pos.y = (float)sit_gs.window_pos_y;
    return pos;
}
...
SITAPI SituationError SituationApplyCurrentProfileWindowState(void) {
    ...
    // Skip entirely when fullscreen — GLFW may report GLFW_MAXIMIZED for exclusive-fullscreen
    // windows on some drivers, and calling glfwRestoreWindow would exit fullscreen.
    if (!(target_flags & SITUATION_FLAG_FULLSCREEN_MODE)) {
    ...
    // Update viewport/projection only when the framebuffer size actually changed (e.g. fullscreen toggle).
    int fb_w, fb_h;
    glfwGetFramebufferSize(sit_gs.sit_glfw_window, &fb_w, &fb_h);
    if (fb_w != sit_gs.main_window_width || fb_h != sit_gs.main_window_height) {
        _SituationGLFWFramebufferSizeCallback(sit_gs.sit_glfw_window, fb_w, fb_h);
    }
 ...
 * @details Display-anchored interval between successive present completions when a render
 *          thread is active (Phase 2). With VSync or target FPS, this reflects monitor pace,
 *          not main-thread poll-to-poll wall time. Use for rendering, camera, and animation
 *          tied to what was shown on screen. Single-threaded mode measures at present on the
 *          main thread.
 ...
 * @see SituationSetTargetFPS(), SituationGetFPS(), SituationGetDisplayRefreshRate()
 */
