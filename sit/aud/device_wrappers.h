    ...
    for (int i = 0; i < frames; i++) {
        ...
        if (any_voice) {
            ...
            for (int v = 0; v < SITUATION_TONE_SYNTH_MAX_VOICES; v++) {
                ...
                if (controls) {
                    _SituationToneSynthModLfoAdvanceVoice(voice, controls, sample_rate);
                    mod_lfo_val = _SituationToneSynthModLfoValueVoice(voice, controls);
                    mod_lfo_pitch = _SituationToneSynthModLfoPitchSemis(controls, mod_lfo_val);
                    mod_lfo_filter_hz =
                        _SituationToneSynthModLfoFilterCutoffOffset(controls, mod_lfo_val);
                    mod_lfo_pulse_width =
                ...
                float amp = envelope * voice->volume_peak * global_vol;
            ...
            if (amp > 0.0f) {
                ...
                if (controls && _SituationToneSynthFilterEnabled(controls)) {
                    ...
                    sample = _SituationToneSynthVoiceFilterProcess(
                        ...
                        sample_rate, filter_cutoff_mod, amp);
                } else {
                    sample *= amp;
                }
...
static void _SituationProcessChorusNode(
    ...
    if (controls) {
        ...
        // Global parameters (registry: 16=width, 17=dry_gain, 18=wet_gain, 19=feedback, 20=stereo_enhance)
        SituationChorus4Stage_SetWidth(chorus, controls[16]);
        SituationChorus4Stage_SetDryGain(chorus, controls[17]);
        SituationChorus4Stage_SetWetGain(chorus, controls[18]);
        SituationChorus4Stage_SetFeedback(chorus, controls[19]);
        SituationChorus4Stage_SetStereoEnhance(chorus, controls[20]);
    }
...
static void _SituationProcessPhaserNode(
    ...
    // Update parameters from controls
    // Registry: 0=lfo_freq, 1=feedback, 2=mix, 3=pan_depth, 4=stereo_width, 5=feedback_delay_ms
    if (controls) {
        setRate(phaser, controls[0]);
        setFeedback(phaser, controls[1]);
        setMix(phaser, controls[2]);
    }
...
static void _SituationProcessOverdriveNode(
    ...
    // Update parameters from controls
    // Registry: 0=mode, 1=drive, 2=input_gain, 3=output_gain, 4=mix, 5=filter_cutoff,
    //           6=filter_res, 7=low_shelf_db, 8=high_shelf_db, 9=asymmetry
    if (controls) {
        sit_overdrive_set_mode(overdrive, (sit_overdrive_mode)((int)controls[0]));
        sit_overdrive_set_drive(overdrive, controls[1]);
        sit_overdrive_set_input_gain(overdrive, controls[2]);
        sit_overdrive_set_output_gain(overdrive, controls[3]);
        sit_overdrive_set_mix(overdrive, controls[4]);
