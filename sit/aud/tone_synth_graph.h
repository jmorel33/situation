 ...
 *   waveforms, velocity). Per-voice Polysonix-style SVF (sit/aud/fx/filter.h). MIDI global:
...
#define SITUATION_TONE_SYNTH_PATCH_PARAM_COUNT 36 /* snapshot controls 1..36 (waveform..sub_ring_mod) */
...
static inline void _SituationToneSynthSumLimiterFree(SituationToneSynthSumLimiter* limiter) {
    if (limiter->delay_line_l) {
        free(limiter->delay_line_l);
        limiter->delay_line_l = NULL;
    }
    if (limiter->delay_line_r) {
        free(limiter->delay_line_r);
...
static inline bool _SituationToneSynthSumLimiterAlloc(SituationToneSynthSumLimiter* limiter,
    ...
    limiter->delay_line_l = (float*)calloc((size_t)limiter_buf_size, sizeof(float));
    limiter->delay_line_r = (float*)calloc((size_t)limiter_buf_size, sizeof(float));
...
typedef struct SituationToneSynthNodeState {
    ...
    /* Global mod LFO (rate/wave via controls 18–19; targets via 20–25 / CC24–31) */
    float mod_lfo_phase;
    ...
    /* Per-node patch memory: 16 slots × controls 1..36 */
    struct {
...
static inline void _SituationToneSynthModLfoAdvance(SituationToneSynthNodeState* s,
                                                    ...
                                                    float sample_rate) {
    if (!s || !controls || sample_rate <= 0.0f) return;
    ...
    } else if (prev <= 0.0f && s->mod_lfo_phase > 0.0f &&
...
static inline float _SituationToneSynthModLfoValue(const SituationToneSynthNodeState* s,
                                                   const float* controls) {
    if (!s || !controls || controls[SIT_TONE_CTRL_LFO_RATE] <= 0.0f) return 0.0f;
    ...
    return _SituationToneSynthModLfoWaveform(s, s->mod_lfo_phase, wf);
}
...
static inline float _SituationToneSynthOscSample(const SituationToneSynthVoice* v,
                                                 ...
                                                 float pulse_width) {
    (void)sample_rate;
    return _SituationToneSynthOscSampleWave(v->waveform, v->phase, v->note, freq_hz, pulse_width);
}
...
static inline void _SituationToneSynthVoiceFilterUpdate(SituationToneSynthVoiceFilter* vf,
    ...
    if (mode != vf->mode_cache || poles != vf->poles_cache ||
        fabsf(cutoff - vf->cutoff_cache) > 0.5f ||
        fabsf(q - vf->res_cache) > 0.01f) {
...
static inline float _SituationToneSynthVoiceFilterProcess(SituationToneSynthVoiceFilter* vf,
    ...
    if (vf->filter.use_oversampling) {
        return filter_process_oversampled(&vf->filter, sample);
    }
    return filter_process_internal(&vf->filter, sample);
}
...
static inline void _SituationToneSynthPatchSave(SituationToneSynthMidiCtx* ctx, int slot) {
    ...
    memcpy(ctx->synth->patch_slots[slot].param, &ctx->controls[SIT_TONE_CTRL_WAVEFORM],
...
static inline void _SituationToneSynthPatchRecall(SituationToneSynthMidiCtx* ctx, int slot,
    ...
    memcpy(&ctx->controls[SIT_TONE_CTRL_WAVEFORM], ctx->synth->patch_slots[slot].param,
           sizeof(float) * (size_t)SITUATION_TONE_SYNTH_PATCH_PARAM_COUNT);
    ctx->synth->patch_slot = (uint8_t)slot;
    ctx->controls[SIT_TONE_CTRL_PATCH_SLOT] = (float)slot;
