...
SIT_MidiLearnState* SIT_MidiLearn_Create(void) {
    SIT_MidiLearnState *state = (SIT_MidiLearnState*)SIT_CALLOC(1, sizeof(SIT_MidiLearnState));
...
static SIT_MidiLearnJSONBuffer* _SIT_MidiLearn_CreateJSONBuffer(void) {
    SIT_MidiLearnJSONBuffer* buf = (SIT_MidiLearnJSONBuffer*)SIT_CALLOC(1, sizeof(SIT_MidiLearnJSONBuffer));
    if (!buf) return NULL;
...
int SIT_MidiLearn_LoadPreset(SIT_MidiLearnState *state, const char *filename) {
    ...
    char* json = (char*)SIT_MALLOC(file_size + 1);
    if (!json) {
        fclose(file);
        return 0;
    }
