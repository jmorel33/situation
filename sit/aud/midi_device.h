...
static SIT_MidiProcessor* SIT_MidiProcessor_Create(double sample_rate) {
    SIT_MidiProcessor *proc = (SIT_MidiProcessor*)SIT_CALLOC(1, sizeof(SIT_MidiProcessor));
    if (!proc) return NULL;
...
SIT_MidiDevice* SIT_MidiDevice_Create(
    ...
    SIT_MidiDevice *device = (SIT_MidiDevice*)SIT_CALLOC(1, sizeof(SIT_MidiDevice));
...
void SIT_MidiDevice_SendIdentityReply(SIT_MidiDevice *device, uint8_t device_id) {
    ...
    uint8_t* reply = (uint8_t*)SIT_MALLOC(total_len);
    if (!reply) return;
