...
static const int SIT_REVERB_COMB_TUNINGS[SIT_REVERB_COMB_COUNT] =
    { 1116, 1188, 1277, 1356, 1422, 1491, 1557, 1617 };
static const int SIT_REVERB_ALLPASS_TUNINGS[SIT_REVERB_ALLPASS_COUNT] =
    { 225, 341, 441, 556, 161, 719 };

static float _sit_reverb_room_predelay_ms(float room_size)
{
    if (room_size < 0.0f) room_size = 0.0f;
    if (room_size > 1.0f) room_size = 1.0f;
    return 4.0f + room_size * 76.0f;
}

static void _sit_reverb_free_delay_lines(SituationReverbState* rev)
{
    if (!rev) return;
    for (int i = 0; i < SIT_REVERB_COMB_COUNT; ++i) {
        SIT_FREE(rev->combs_l[i].buffer);
        SIT_FREE(rev->combs_r[i].buffer);
        rev->combs_l[i].buffer = NULL;
        rev->combs_r[i].buffer = NULL;
        rev->combs_l[i].size = 0;
        rev->combs_r[i].size = 0;
        rev->combs_l[i].cursor = 0;
        rev->combs_r[i].cursor = 0;
    ...
    for (int i = 0; i < SIT_REVERB_ALLPASS_COUNT; ++i) {
        ...
        rev->allpasses_l[i].buffer = NULL;
        rev->allpasses_r[i].buffer = NULL;
        rev->allpasses_l[i].size = 0;
        rev->allpasses_r[i].size = 0;
        rev->allpasses_l[i].cursor = 0;
        rev->allpasses_r[i].cursor = 0;
    }
}

static bool _sit_reverb_alloc_delay_lines(SituationReverbState* rev)
{
    if (!rev || rev->sample_rate == 0u) return false;

    _sit_reverb_free_delay_lines(rev);

    float scale = (float)rev->sample_rate / 44100.0f;

    for (int i = 0; i < SIT_REVERB_COMB_COUNT; ++i) {
        int size_l = (int)(SIT_REVERB_COMB_TUNINGS[i] * scale);
        int size_r = (int)((SIT_REVERB_COMB_TUNINGS[i] + SIT_REVERB_STEREO_SPREAD) * scale);
        if (size_l < 2) size_l = 2;
        if (size_r < 2) size_r = 2;

        rev->combs_l[i].size = size_l;
        rev->combs_r[i].size = size_r;
        rev->combs_l[i].buffer = (float*)SIT_CALLOC((size_t)size_l, sizeof(float));
        rev->combs_r[i].buffer = (float*)SIT_CALLOC((size_t)size_r, sizeof(float));
    ...
    for (int i = 0; i < SIT_REVERB_ALLPASS_COUNT; ++i) {
        int size_l = (int)(SIT_REVERB_ALLPASS_TUNINGS[i] * scale);
        int size_r = (int)((SIT_REVERB_ALLPASS_TUNINGS[i] + SIT_REVERB_STEREO_SPREAD) * scale);
        if (size_l < 2) size_l = 2;
        if (size_r < 2) size_r = 2;

        rev->allpasses_l[i].size = size_l;
        rev->allpasses_r[i].size = size_r;
        rev->allpasses_l[i].buffer = (float*)SIT_CALLOC((size_t)size_l, sizeof(float));
        rev->allpasses_r[i].buffer = (float*)SIT_CALLOC((size_t)size_r, sizeof(float));
...
static float _sit_reverb_comb_process(SituationReverbComb* comb, float input, float mod_offset) {
    if (!comb || !comb->buffer || comb->size < 4) return 0.0f;

    float read_idx = (float)comb->cursor + mod_offset;
    if (read_idx >= (float)comb->size) read_idx -= (float)comb->size;
    if (read_idx < 0.0f) read_idx += (float)comb->size;
...
static float _sit_reverb_allpass_process(SituationReverbAllPass* ap, float input, float mod_offset) {
    if (!ap || !ap->buffer || ap->size < 4) return input;

    float read_idx = (float)ap->cursor + mod_offset;
    if (read_idx >= (float)ap->size) read_idx -= (float)ap->size;
    if (read_idx < 0.0f) read_idx += (float)ap->size;
...
static SituationError _SituationInitReverb(uint32_t sample_rate, void** out_state) {
    ...
    *out_state = NULL;
    ...
    if (!_sit_reverb_alloc_delay_lines(rev)) {
 ...
 *
 * room_size (control 0) drives comb feedback and predelay. Delay-line lengths are
 * fixed at init — no mid-stream buffer rebuilds (those caused crackle/runaway).
 */
static void _SituationProcessReverb(void* state_ptr, float* pOutput, const float* pInput, uint32_t frameCount, int channels) {
    if (!state_ptr) return;
    SituationReverbState* rev = (SituationReverbState*)state_ptr;

    float room_scale = rev->room_size * 0.48f + 0.5f; /* 0.5 to 0.98 — classic Freeverb-style */
    ...
    // Predelay follows room_size (was fixed 15 ms at init only).
    float predelay_ms = _sit_reverb_room_predelay_ms(rev->room_size);
    int pd_samples = (int)(predelay_ms * rev->sample_rate / 1000.0f);
    ...
    for (uint32_t i = 0; i < frameCount; ++i) {
        ...
        for(int j=0; j<SIT_REVERB_COMB_COUNT; ++j) {
            ...
            float lfo_l = (sinf(rev->lfo_phase + phase_offset) + 1.0f) * 0.5f * mod_depth_samples;
            float lfo_r = (sinf(rev->lfo_phase + phase_offset + 3.14159265f) + 1.0f) * 0.5f * mod_depth_samples;
        ...
        for(int j=0; j<SIT_REVERB_ALLPASS_COUNT; ++j) {
            ...
            float lfo_l = (sinf(rev->lfo_phase + phase_offset) + 1.0f) * 0.5f * (mod_depth_samples * 0.5f);
            float lfo_r = (sinf(rev->lfo_phase + phase_offset + 3.14159265f) + 1.0f) * 0.5f * (mod_depth_samples * 0.5f);
