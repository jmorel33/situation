...
// FMA detection
    ...
    #define SPRING_HAS_FMA 1
        ...
        #define SPRING_FMA(a, b, c) __builtin_fmaf((a), (b), (c))
    #else
        #define SPRING_FMA(a, b, c) fmaf((a), (b), (c))
    #endif
#else
...
static void SpringReverb_process(SpringReverb *reverb, float *in_l, float *in_r, float *out_l, float *out_r, int num_samples) {
    for (int i = 0; i < num_samples; i++) {
        ...
        // Compute mixed feedback states (FMA-optimized lerp)
        float cross_inv = 1.0f - reverb->cross_mix;
        float mix_l = SPRING_FMA(reverb->cross_mix, reverb->lpf_state_r, cross_inv * reverb->lpf_state_l);
        float mix_r = SPRING_FMA(reverb->cross_mix, reverb->lpf_state_l, cross_inv * reverb->lpf_state_r);
        ...
        // FMA-optimized one-pole LPF
        reverb->lpf_state_l = SPRING_FMA(reverb->a_lpf, reverb->lpf_state_l, (1.0f - reverb->a_lpf) * output_l);
        ...
        for (int k = 0; k < 4; k++) {
            ...
            // FMA-optimized allpass
            float temp = SPRING_FMA(reverb->g, delayed, reverb_l);
            reverb_l = SPRING_FMA(-reverb->g, temp, delayed);
            ap_delay[wp] = temp;
            reverb->ap_write_pos_l[k] = (wp + 1) % reverb->ap_delay_lengths[k];
        }

        float hpf_out_l = reverb_l - reverb->hpf_state_l;
        reverb->hpf_state_l = SPRING_FMA(reverb->a_hpf, hpf_out_l, reverb->hpf_state_l);
        reverb_l = hpf_out_l;
        ...
        // FMA-optimized one-pole LPF
        reverb->lpf_state_r = SPRING_FMA(reverb->a_lpf, reverb->lpf_state_r, (1.0f - reverb->a_lpf) * output_r);
        ...
        for (int k = 0; k < 4; k++) {
            ...
            float delayed = ap_delay[rp];
            // FMA-optimized allpass
            float temp = SPRING_FMA(reverb->g, delayed, reverb_r);
            reverb_r = SPRING_FMA(-reverb->g, temp, delayed);
            ap_delay[wp] = temp;
            reverb->ap_write_pos_r[k] = (wp + 1) % reverb->ap_delay_lengths[k];
        }

        float hpf_out_r = reverb_r - reverb->hpf_state_r;
        reverb->hpf_state_r = SPRING_FMA(reverb->a_hpf, hpf_out_r, reverb->hpf_state_r);
        reverb_r = hpf_out_r;

        reverb->main_write_pos = (reverb->main_write_pos + 1) % reverb->delay_length;

        // FMA-optimized output mix
        float wet_gain = reverb->reverb * reverb->gate_state;
        float mix_l_out = SPRING_FMA(wet_gain, reverb_l, reverb->direct * input_l);
        float mix_r_out = SPRING_FMA(wet_gain, reverb_r, reverb->direct * input_r);
