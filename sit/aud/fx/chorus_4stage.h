...
static void SituationChorus4Stage_Process(SituationChorus4Stage *effect, float *input_left, float *input_right, float *output_left, float *output_right, int num_samples) {
    for (int i = 0; i < num_samples; i++) {
        ...
        for (int stage = 0; stage < 4; stage++) {
            ...
            // Interpolate samples using FMA for cubic interpolation
            float t = frac_left;
            float sample_left = CHORUS_FMA(t, CHORUS_FMA(t, CHORUS_FMA(t, a3_left, a2_left), a1_left), a0_left);
            t = frac_right;
            float sample_right = CHORUS_FMA(t, CHORUS_FMA(t, CHORUS_FMA(t, a3_right, a2_right), a1_right), a0_right);
