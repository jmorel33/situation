...
// FMA detection
    ...
    #define EXCITER_HAS_FMA 1
        ...
        #define EXCITER_FMA(a, b, c) __builtin_fmaf((a), (b), (c))
    #else
        #define EXCITER_FMA(a, b, c) fmaf((a), (b), (c))
    #endif
#else
    #define EXCITER_HAS_FMA 0
    #define EXCITER_FMA(a, b, c) ((a) * (b) + (c))
#endif
...
static void process_exciter(ExciterState *state, const float *input, float *output, size_t num_frames, int num_channels) {
    ...
    for (size_t f = 0; f < num_frames; ++f) {
        for (int c = 0; c < num_channels; ++c) {
            ...
            // Stage 6: FMA-optimized mix with dry
            float dry_gain = 1.0f - state->params.mix;
            output[idx] = EXCITER_FMA(wet, state->params.mix, dry * dry_gain);
        }
    }
}
