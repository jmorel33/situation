...
static void dynamics_init(SituationDynamics* dyn, float sample_rate) {
    ...
    dyn->delay_line_l = (float*)calloc(dyn->buffer_capacity, sizeof(float));
    dyn->delay_line_r = (float*)calloc(dyn->buffer_capacity, sizeof(float));
...
static void dynamics_cleanup(SituationDynamics* dyn) {
    if (dyn->delay_line_l) {
        free(dyn->delay_line_l);
        dyn->delay_line_l = NULL;
    }
    if (dyn->delay_line_r) {
        free(dyn->delay_line_r);
