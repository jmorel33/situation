...
// FMA detection
    ...
    #define OVERDRIVE_HAS_FMA 1
    #if defined(__GNUC__) || defined(__clang__)
        #define OVERDRIVE_FMA(a, b, c) __builtin_fmaf((a), (b), (c))
    #else
        #define OVERDRIVE_FMA(a, b, c) fmaf((a), (b), (c))
    #endif
#else
    #define OVERDRIVE_HAS_FMA 0
    #define OVERDRIVE_FMA(a, b, c) ((a) * (b) + (c))
#endif
...
void sit_overdrive_process(sit_overdrive* od, const float* in, float* out, int frames) {
    ...
    for (int i = 0; i < frames; ++i) {
        ...
        // FMA-optimized dry/wet mix: out = dry*(1-mix) + wet*mix
        float dry_gain = 1.0f - od->mix;
        out[i*2 + 0] = OVERDRIVE_FMA(wet_l, od->mix, l * dry_gain);
        out[i*2 + 1] = OVERDRIVE_FMA(wet_r, od->mix, r * dry_gain);
    }
}
...
void sit_overdrive_process_split(sit_overdrive* od,
    ...
    for (int i = 0; i < frames; ++i) {
        ...
        // FMA-optimized dry/wet mix
        out_l[i] = OVERDRIVE_FMA(wet_l, od->mix, in_l[i] * dry_gain);
        out_r[i] = OVERDRIVE_FMA(wet_r, od->mix, in_r[i] * dry_gain);
    }
}
