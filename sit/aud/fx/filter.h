...
// Half-band FIR (matches Polysonix voice oversample decimator taps)
static const float HB[4] = {
    0.036681502163648017f,
   -0.136728736584138680f,
    0.613659362617572050f,
    0.613659362617572050f
};

#define FILTER_SOFTCLIP_RATIO 0.8f
#define FILTER_SOFTCLIP_K2 (FILTER_SOFTCLIP_RATIO * FILTER_SOFTCLIP_RATIO)
#define FILTER_SOFTCLIP_K4 (FILTER_SOFTCLIP_K2 * FILTER_SOFTCLIP_K2)
#define FILTER_SOFTCLIP_LIMIT (1.0f / FILTER_SOFTCLIP_RATIO)

static inline float filter_fast_tanh(float x) {
    float x2 = x * x;
    float y = x * (27.0f + x2) / FILTER_FMA(9.0f, x2, 27.0f);
    if (y > 1.0f) return 1.0f;
    if (y < -1.0f) return -1.0f;
    return y;
}

static inline float filter_soft_clip(float x) {
    float x2 = x * x;
    float x4 = x2 * x2;
    float num = x * FILTER_FMA(FILTER_SOFTCLIP_K4, x4,
                               FILTER_FMA(105.0f * FILTER_SOFTCLIP_K2, x2, 945.0f));
    float den = FILTER_FMA(15.0f * FILTER_SOFTCLIP_K4, x4,
                           FILTER_FMA(420.0f * FILTER_SOFTCLIP_K2, x2, 945.0f));
    float result = num / den;
    if (result > FILTER_SOFTCLIP_LIMIT) return FILTER_SOFTCLIP_LIMIT;
    if (result < -FILTER_SOFTCLIP_LIMIT) return -FILTER_SOFTCLIP_LIMIT;
    return result;
}
...
static float filter_process_internal(SituationFilter* filter, float input_sample) {
    ...
    float driven_input = filter_fast_tanh(dc_blocked * filter->drive);
    ...
    filter->lp_state1 = filter_fast_tanh(lp1 + anti_denormal) - anti_denormal;
    filter->bp_state1 = filter_fast_tanh(bp1 + anti_denormal) - anti_denormal;
    ...
    if (filter->poles == 3) {
        ...
        filter->lp_state2 = filter_fast_tanh(lp2 + anti_denormal) - anti_denormal;
        filter->bp_state2 = filter_fast_tanh(bp2 + anti_denormal) - anti_denormal;
...
static float filter_process_oversampled(SituationFilter* filter, float input_sample) {
    ...
    float out = FILTER_FMA(HB[0], filter->os_x2,
                           FILTER_FMA(HB[1], filter->os_x1,
                                      FILTER_FMA(HB[2], y0, HB[3] * y1)));
...
/**
 * @brief 2x oversampling with post-filter amplitude and soft clip (Polysonix voice path).
 */
static float filter_process_oversampled_amp(SituationFilter* filter, float input_sample, float amp) {
    float y0 = filter_soft_clip(filter_process_internal(filter, input_sample) * amp);
    float y1 = filter_soft_clip(filter_process_internal(filter, input_sample) * amp);
