...
static void sit_fft_init(sit_fft_plan *plan, int N) {
    plan->N = N;
    plan->bitrev = (int *)SIT_MALLOC(N * sizeof(int));
    plan->twiddle_re = (float *)SIT_MALLOC((N / 2) * sizeof(float));
    plan->twiddle_im = (float *)SIT_MALLOC((N / 2) * sizeof(float));
...
static void sit_fft_free(sit_fft_plan *plan) {
    SIT_FREE(plan->bitrev);
    SIT_FREE(plan->twiddle_re);
    SIT_FREE(plan->twiddle_im);
}
...
static void init_maximizer(MaximizerState *state, int sample_rate, int H, int N_dominant, float cutoff_frequency, int num_bands) {
    ...
    state->bands = (BandParams *)SIT_MALLOC(num_bands * sizeof(BandParams));
...
#else
    /* NOTE: aligned_alloc() memory must be freed with free(), not SIT_FREE/mbd_free.
     * These are platform aligned allocations — they are intentional raw free() calls. */
