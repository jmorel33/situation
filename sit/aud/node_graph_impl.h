...
SituationError SituationSetControl(
    ...
    const bool tone_synth = (node->type == SITUATION_NODE_TONE_SYNTH && node->device_data);
    const bool patch_slot_recall =
        tone_synth && control_id == (uint32_t)SIT_TONE_CTRL_PATCH_SLOT;
    const bool patch_store_save =
        tone_synth && control_id == (uint32_t)SIT_TONE_CTRL_PATCH_STORE && value >= 0.5f;

    /* patch_slot recall and patch_store save are intentional side effects even when the
     * stored control value is unchanged (e.g. recall slot 0 while already on slot 0). */
    if (!patch_slot_recall && !patch_store_save && value == node->control_values[control_id]) {
        return SITUATION_SUCCESS;
    }
    ...
    if (node->type == SITUATION_NODE_TONE_SYNTH && node->device_data) {
        if (control_id == (uint32_t)SIT_TONE_CTRL_FILTER_MODE ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_CUTOFF ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_RESONANCE ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_POLES ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_DRIVE ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_KEYTRACK ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_OVERSAMPLE ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_ENV_AMOUNT ||
            control_id == (uint32_t)SIT_TONE_CTRL_FILTER_ENV_RANGE ||
            control_id == (uint32_t)SIT_TONE_CTRL_LFO_FILTER_AMOUNT ||
            control_id == (uint32_t)SIT_TONE_CTRL_LFO_FILTER_RANGE) {
            _SituationToneSynthInvalidateVoiceFilterCaches(
                (SituationToneSynthNodeState*)node->device_data);
        }
    }
