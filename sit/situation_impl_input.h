...
/* Windows often reports main-row digit keys (GLFW_KEY_0..9) when the numpad is
 * pressed with NumLock on.  Strip those aliases after each poll so piano layouts
 * keyed by scancode or SIT_KEY_0..9 are not triggered by KP_0..9. */
static inline void _SituationReconcileNumpadDigitAlias(void) {
    if (!SituationIsInitialized() || !sit_gs.sit_glfw_window) return;
    GLFWwindow* w = sit_gs.sit_glfw_window;
    for (int d = 0; d <= 9; d++) {
        const int digit = SIT_KEY_0 + d;
        const int kp    = SIT_KEY_KP_0 + d;
        if (glfwGetKey(w, kp) != GLFW_PRESS)
            continue;
        sit_input.keyboard.current_state[digit] = false;
        sit_input.keyboard.down_this_frame[digit] = false;
        const int sc = glfwGetKeyScancode(digit);
        if (sc >= 0 && sc < SITUATION_MAX_SCANCODES)
            sit_input.keyboard.scancode_state[sc] = false;
    }
}
...
static void _SituationGLFWWindowMaximizeCallback(GLFWwindow* window, int maximized) {
    (void)window;
    // [FIX v2.4.318] Sync internal profile flags when the OS maximizes/restores.
    // Without this, active_profile_window_flags remained stale after a title-bar maximize,
    // causing backpressure, swap interval, and HUD state queries to use outdated data.
    // Guard: do NOT update flags during borderless fullscreen transitions — GLFW reports
    // "maximized" when the window fills the screen, but that's not a real maximize.
    if (!sit_gs.is_borderless_active) {
        if (maximized == GLFW_TRUE) {
            sit_gs.active_profile_window_flags |= SITUATION_FLAG_WINDOW_MAXIMIZED;
            sit_gs.active_profile_window_flags &= ~SITUATION_FLAG_WINDOW_MINIMIZED;
        } else {
            sit_gs.active_profile_window_flags &= ~SITUATION_FLAG_WINDOW_MAXIMIZED;
        }
    }
...
/**
 * @brief [INTERNAL] GLFW callback invoked when the window is moved (user drag or programmatic).
 * @details Keeps `sit_gs.window_pos_x/y` current so the library is always aware of the window's
 *          desktop position — needed for multi-monitor detection and VD offset calculations.
 */
static void _SituationGLFWWindowPosCallback(GLFWwindow* window, int xpos, int ypos) {
    (void)window;
    sit_gs.window_pos_x = xpos;
    sit_gs.window_pos_y = ypos;
}
    ...
    ma_mutex_lock(&sit_input.mouse.mutex);
    {
        ...
        if (button >= 0 && button <= GLFW_MOUSE_BUTTON_LAST) {
            if (action == GLFW_PRESS) {
                ...
                if (sit_input.pump_guard_depth > 0) {
                    sit_input.pending_mouse_down[button] = true;
                } else {
                ...
                if (sit_input.pump_guard_depth > 0) {
                    sit_input.pending_mouse_up[button] = true;
                } else {
...
/**
 * @brief Consumes (eats) a key-press event for the current frame.
 * @details After calling this, SituationIsKeyPressed(key) will return false for the rest of the frame.
 *          Also clears any deferred guarded-pump edge for this key so a window/VSync toggle that
 *          pumps events mid-handler does not re-fire the same hotkey on the next poll.
 *          Useful for global hotkey handlers (in scaffolding) so that application-specific key checks
 *          later in the frame do not also react to the same press.
 */
SITAPI void SituationConsumeKeyPress(int key) {
    if (!SituationIsInitialized() || key < 0 || key > GLFW_KEY_LAST) return;
    sit_input.keyboard.down_this_frame[key] = false;
    sit_input.pending_down[key] = false;
    sit_input.pending_up[key] = false;
}
