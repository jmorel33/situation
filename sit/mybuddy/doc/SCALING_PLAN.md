...
- [x] **Cross-Arena Frees & Remote Queues (Crucial):** Do not rely on offset math or page headers. Add an explicit arena pointer to the header. Furthermore, to prevent cross-core lock contention during foreign frees (e.g., producer/consumer patterns), introduce a lock-free `remote_free_queue` per arena. *(Implemented as a lock-free LIFO CAS stack — `_Atomic(block_header_t*)` head with `atomic_compare_exchange_weak` push. The plan text originally said "mutex-protected" but the implementation correctly went lock-free. ABA risk documented in a comment above `remote_push()`.)*
...
- [x] **Phase 2 (Correctness + Structure): 100% Complete**
    ...
    - [x] Introduce a lock-free CAS `remote_free_queue` per arena for safe, cross-core foreign frees. *(Implemented as `_Atomic(block_header_t*)` — the roadmap said "mutex-protected" but CAS was correctly chosen.)*
