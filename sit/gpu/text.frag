 ...
 * @var global_textures[] (VK bindless) or u_Texture (GL) glyph atlas sampler.
...
void main() {
    ...
    /* Preserve atlas RGB (retro outlined fonts bake fill + outline colors). */
    outColor = vec4(texColor.rgb * pc.color.rgb, pc.color.a * glyphAlpha);
}
...
void main() {
    ...
    if (glyphAlpha < 0.01) discard;
    /* Preserve atlas RGB (retro outlined fonts bake fill + outline colors). */
    outColor = vec4(texColor.rgb * u_color.rgb, u_color.a * glyphAlpha);
}
#endif
