...
#define SITUATION_ERRORS_CORE(X) \
    ...
    X(SITUATION_ERROR_SHUTDOWN_INCOMPLETE,          -13, "Shutdown completed with one or more subsystem teardown failures") \
    X(SITUATION_ERROR_INIT_STALE_DRIVER_STATE,      -14, "Init detected stale driver state from a previous session (possible display mode ghost)") \
...
#define SITUATION_ERRORS_PLATFORM(X) \
    ...
    X(SITUATION_ERROR_FULLSCREEN_RELEASE_FAILED,   -108, "Failed to restore windowed mode before window destruction") \
    X(SITUATION_ERROR_CONTEXT_RECLAIM_FAILED,      -109, "Failed to reclaim GL context on main thread after render thread shutdown") \
...
#define SITUATION_ERRORS_DISPLAY(X) \
    ...
    X(SITUATION_ERROR_DISPLAY_MODE_SETTLING,            -215, "Display mode transition still in progress (frame rendered at native resolution)")
...
#define SITUATION_ERRORS_AUDIO(X) \
    ...
    X(SITUATION_ERROR_AUDIO_DEVICE_TRANSITION_STALE,    -431, "Audio device in stale state from previous session exclusive-mode usage")
...
#define SITUATION_ERRORS_RENDERING(X) \
    ...
    X(SITUATION_ERROR_RENDER_TARGET_INVALID,            -558, "Invalid or inactive render target handle") \
    X(SITUATION_ERROR_RENDER_TARGET_CREATE_FAILED,      -559, "Failed to create render target attachments") \
    X(SITUATION_ERROR_PROFILING_GPU_UNSUPPORTED,        -563, "GPU timestamp queries unsupported on this device/backend") \
    X(SITUATION_ERROR_PROFILING_ZONE_OVERFLOW,          -564, "GPU profile zone_id exceeds SITUATION_FRAME_PROFILE_GPU_ZONE_COUNT") \
    X(SITUATION_ERROR_PROFILING_ZONE_STATE,             -565, "GPU profile zone begin/end mismatch (double begin or end without begin)") \
    X(SITUATION_ERROR_QUERY_RESULT_NOT_READY,           -566, "Query pool results not ready (GPU work still in flight)") \
    X(SITUATION_ERROR_QUERY_POOL_INVALID,               -567, "Invalid or destroyed query pool handle") \
    X(SITUATION_ERROR_QUERY_INDEX_OUT_OF_RANGE,         -568, "Query index out of pool range") \
    X(SITUATION_ERROR_QUERY_TYPE_MISMATCH,              -569, "Query command incompatible with pool type") \
    X(SITUATION_ERROR_OCCLUSION_QUERY_STATE,            -570, "Occlusion query begin/end state mismatch")
...
#define SITUATION_ERRORS_VULKAN(X) \
    ...
    X(SITUATION_ERROR_VULKAN_PIPELINE_CACHE_INIT_FAILED, -758, "Vulkan: vkCreatePipelineCache failed during init") \
    X(SITUATION_ERROR_VULKAN_RENDER_PASS_CACHE_FULL,     -759, "Vulkan: render pass cache full (32 max); likely leaking render passes")
