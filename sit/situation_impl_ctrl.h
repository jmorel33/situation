...
SITAPI SituationError SituationInit(int argc, char** argv, const SituationInitInfo* init_info) {
    ...
    sit_gs.output_color_depth_policy = init_info->output_color_depth;
    if ((int)sit_gs.output_color_depth_policy > (int)SIT_OUTPUT_COLOR_HDR10) {
        sit_gs.output_color_depth_policy = SIT_OUTPUT_COLOR_AUTO;
    }
    ...
    SituationError err = _SituationInitPlatform(init_info);
    ...
    /* Phase 5/6: DXGI display metadata must exist before Vulkan swapchain format pick (HDR gating). */
    ...
    if (init_info && init_info->default_window_icon_path) {
        _SituationApplyDefaultWindowIconPath(init_info->default_window_icon_path);
    }
    ...
    // [v2.4.322] Detect if previous shutdown was incomplete and report warning
...
*/
#if defined(_WIN32)
#define SIT_WIN32_IDENTITY_IMPLEMENTATION
#include "platform/windows/situation_win32_identity.h"
#endif
...
#define ENABLE_VIRTUAL_TERMINAL_PROCESSING 0x0004
        ...
        /* Shell AppUserModelID before any GLFW window (default or init_info override). */
        _SituationWin32InitAppUserModelIdFromInitInfo(init_info ? init_info->app_user_model_id : NULL);
    }
...
static SituationError _SituationInitWindow(const SituationInitInfo* init_info) {
...
#elif defined(SITUATION_USE_OPENGL)
    {
        ...
        if (_SituationWants10BitOutput(sit_gs.output_color_depth_policy)) {
    ...
    // Seed initial window position so the library knows placement from frame 1.
    glfwGetWindowPos(sit_gs.sit_glfw_window, &sit_gs.window_pos_x, &sit_gs.window_pos_y);
    _SituationCopyThreadName(sit_gs.main_thread_name, sizeof(sit_gs.main_thread_name),
        _SituationResolveMainThreadOsName(init_info->main_thread_name, init_info->window_title));
    _SituationSetCurrentThreadName(sit_gs.main_thread_name);
#ifdef SITUATION_ENABLE_THREADING
...
static SituationError _SituationInitSubsystems(const SituationInitInfo* init_info) {
    ...
    glfwSetWindowPosCallback(sit_gs.sit_glfw_window, _SituationGLFWWindowPosCallback);
    ...
    glfwSetMonitorCallback(_SituationGLFWMonitorCallback);
    ...
    sit_gs.last_backpressure_ns = 0;
    sit_gs.last_fence_wait_ns = 0;
    sit_gs.last_execute_ns = 0;
    sit_gs.last_present_ns = 0;

    sit_gs.last_poll_ns = 0;
    sit_gs.last_update_ns = 0;

    sit_gs.last_glfw_poll_ns = 0;
    sit_gs.last_input_reset_ns = 0;
    sit_gs.last_joystick_ns = 0;
...
SITAPI void SituationPollInputEvents(void) {
    ...
    uint64_t poll_t0 = _SitGetMonotonicTimeNS();

    SIT_PROF_ZONE_SCOPED("PollInputEvents") {
    if (sit_render.in_frame) {
        ...
        fprintf(stderr, "[Situation]   Fix: call SituationEndFrame() before the next poll/update. See doc/architecture.md (Frame Loop Contract).\n");
#endif
    }
    ...
    uint64_t reset_t0 = _SitGetMonotonicTimeNS();
    ...
    // Copy end-of-previous-poll held state for IsKeyDown edge comparisons elsewhere.
    ...
    // Merge edges deferred by guarded pumps during the previous frame's render phase.
    ...
    sit_gs.last_input_reset_ns = _SitGetMonotonicTimeNS() - reset_t0;
    ...
    uint64_t glfw_poll_t0 = _SitGetMonotonicTimeNS();
    glfwPollEvents();
    sit_gs.last_glfw_poll_ns = _SitGetMonotonicTimeNS() - glfw_poll_t0;
    ...
    _SituationReconcileNumpadDigitAlias();
    ...
    uint64_t joystick_t0 = _SitGetMonotonicTimeNS();
    ...
    sit_gs.last_joystick_ns = _SitGetMonotonicTimeNS() - joystick_t0;

    sit_gs.last_poll_ns = _SitGetMonotonicTimeNS() - poll_t0;
    } /* SIT_PROF_ZONE_SCOPED PollInputEvents */
}
...
SITAPI void SituationUpdateTimers(void) {
    ...
    uint64_t update_t0 = _SitGetMonotonicTimeNS();

    SIT_PROF_ZONE_SCOPED("UpdateTimers") {
    ...
    // ST: wall clock between UpdateTimers calls. RT: display delta published at present.
#if defined(SITUATION_ENABLE_RENDER_THREAD)
    if (sit_render.enabled) {
        _SituationConsumePresentTimingOnMain();
    } else {
#endif
    ...
    // General spike tracking for debugging assistance (harmless, cheap)
    if (sit_gs.frame_time > sit_gs.max_frame_time) {
        sit_gs.max_frame_time = sit_gs.frame_time;
    }
    // Conservative spike: frame-time significantly exceeds the expected cadence.
    // [v2.4.319] Separate VSync-paced from truly-uncapped:
    //   - Software target active: spike = 2× target (missed a full frame budget)
    //   - VSync is the pacer (target==0, VSync ON): spike = 25ms (~1.5× at 60Hz)
    //     This gives 8ms headroom above 16.67ms for normal OS/scheduler/DWM jitter
    //     without falsely counting every slightly-late wake as a spike.
    //   - Truly uncapped (target==0, VSync OFF): spike = 20ms.
    //     At high FPS the main thread occasionally yields to the OS scheduler for
    //     10-15ms (thread timeslice, DWM compositor pulse). These are normal and
    //     not actionable. 20ms represents a genuinely notable stall.
    double thresh;
    if (sit_gs.target_frame_time > 0.0) {
        thresh = sit_gs.target_frame_time * 2.0;
    } else if (sit_gs.active_profile_window_flags & SITUATION_FLAG_VSYNC_HINT) {
        thresh = 0.025;  // VSync-paced: 25ms (1.5× at 60Hz — catches missed frames, ignores jitter)
    } else {
        thresh = 0.020;  // Truly uncapped: 20ms (filters normal OS scheduler timeslice yields)
    }
    if (sit_gs.frame_time > thresh) {
        sit_gs.frame_spike_count++;
    }

    // Auto-log significant stutters for diagnosis (rare, only big drops).
    // 3× the spike threshold or 60ms minimum — whichever is larger.
    // This ensures normal VSync jitter never spams the console.
    double log_thresh = fmax(thresh * 3.0, 0.060);
    if (sit_gs.frame_time > log_thresh) {
        // Rate limit the spam so we don't flood the console on borderline frames
        static double last_log_t = 0.0;
        double now = glfwGetTime();
        if (now - last_log_t > 0.25) {  // at most ~4 logs/sec even during a storm
            printf("[STUTTER] dt=%.1fms  core[poll=%llu(%llu glfw %llu reset %llu joy) upd=%llu]  render[bp=%llu fw=%llu ex=%llu pr=%llu]  fps=%d\n",
                   sit_gs.frame_time * 1000.0,
    ...
    // Update basic frame time histogram (for ExportRenderHistogram debugging assistance)
    double ms = sit_gs.frame_time * 1000.0;
    int b = 0;
    if (ms >= 1) b++;
    if (ms >= 2) b++;
    if (ms >= 4) b++;
    if (ms >= 8) b++;
    if (ms >= 16) b++;
    if (b < 6) sit_gs.frame_time_hist[b]++;
    ...
    for (int i = 0; i < SITUATION_MAX_VIRTUAL_DISPLAYS; ++i) {
        if (sit_render.virtual_display_slots_used[i]) {
            ...
            if (vd->update_mode == SIT_VD_UPDATE_STATIC) {
                vd->frame_delta_time_seconds = 0.0;
                continue;
            }
    ...
    sit_gs.last_update_ns = _SitGetMonotonicTimeNS() - update_t0;
    } /* SIT_PROF_ZONE_SCOPED UpdateTimers */
}
...
* 4. **Dangling Resource Cleanup:** Calls `_SituationCleanupDanglingResources` to destroy any GPU handles still registered at shutdown (logged to stderr; explicit Destroy/Unload before shutdown keeps this quiet).
...
SITAPI void SituationShutdown(void) {
    ...
    // Stop the render thread before worker/I/O pool teardown (render thread must not outlive the pool).
    SituationError render_thread_err = SITUATION_SUCCESS;
#if defined(SITUATION_ENABLE_RENDER_THREAD) && !defined(__STDC_NO_THREADS__)
    ...
    // Kill Thread Pool (stops I/O thread hot-reload polling)
#if defined(SITUATION_ENABLE_THREADING)
    ...
    if (_sit_current_context) {
        ...
        // [v2.4.322] Track shutdown completeness
        bool shutdown_fully_clean = true;
        if (render_thread_err != SITUATION_SUCCESS) {
            shutdown_fully_clean = false;
        }
        if (sit_gs.last_error_code != SITUATION_SUCCESS && sit_gs.last_error_code != SITUATION_ERROR_SHUTDOWN_INCOMPLETE) {
            shutdown_fully_clean = false;
        }
        ...
        if (!shutdown_fully_clean) {
            ...
            _sit_previous_shutdown_had_errors = true;
        } else {
            if (render_thread_err == SITUATION_SUCCESS) {
    ...
    // All Situation threads have been joined and the context is freed.
}
...
static void _SituationCleanupSubsystems(void) {
    ...
    atomic_store(&sit_audio.audio_ready, false);
    atomic_store_explicit(&sit_audio.audio_meter_peak, 0.f, memory_order_relaxed);
    atomic_store_explicit(&sit_audio.audio_meter_rms, 0.f, memory_order_relaxed);
...
static void _SituationCleanupRenderer(void) {
    /* [FIX v2.4.320] Exit exclusive fullscreen before any resource teardown.
     * Since v2.4.3xx GLFW is no longer terminated between sessions (glfwTerminate
     * corrupted the OpenGL ICD on re-init). Without termination, GLFW retains internal
     * monitor-mode state from exclusive fullscreen. If the window is destroyed while
     * GLFW thinks it owns a monitor's display mode, the next glfwCreateWindow blocks
     * indefinitely waiting for display-mode arbitration. Explicitly release the monitor
     * here while the window and GL context are still valid.
     * NOTE: glfwPollEvents after the mode change can trigger ACCESS_VIOLATION in the
     * NVIDIA ICD during WM_DISPLAYCHANGE processing. We guard with a VEH on Windows.
     * Without the poll, glfwDestroyWindow later causes an unrecoverable process kill. */
#if defined(SITUATION_USE_OPENGL)
    if (sit_gs.sit_glfw_window && glfwGetWindowMonitor(sit_gs.sit_glfw_window) != NULL) {
        glfwSetWindowMonitor(sit_gs.sit_glfw_window, NULL,
            sit_gs.windowed_x > 0 ? sit_gs.windowed_x : 100,
            sit_gs.windowed_y > 0 ? sit_gs.windowed_y : 100,
            sit_gs.windowed_w > 0 ? sit_gs.windowed_w : 800,
            sit_gs.windowed_h > 0 ? sit_gs.windowed_h : 600,
            GLFW_DONT_CARE);
        glfwPollEvents();
    }
#endif
...
#endif
    out_caps->output_bits_per_channel = sit_render.output_bits_per_channel ? sit_render.output_bits_per_channel : 8u;
    out_caps->output_color_depth_active = sit_render.output_color_depth_active;
    out_caps->output_hdr_active = sit_render.output_hdr_active;
    out_caps->output_color_space = sit_render.output_color_space;
#if defined(SITUATION_USE_VULKAN)
    out_caps->wsi_supports_10bit_sdr = sit_render.vk.surface_supports_10bit_sdr ? 1u : 0u;
    out_caps->wsi_supports_hdr10 = sit_render.vk.surface_supports_hdr10 ? 1u : 0u;
#endif
}
