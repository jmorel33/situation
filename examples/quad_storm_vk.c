/*******************************************************************************
 * Situation Library - QUAD STORM (Vulkan)
 * ========================================
 * Stress test: Thousands of animated quads with physics.
 * Demonstrates raw Vulkan rendering throughput.
 *
 * Controls:
...
int main(int argc, char** argv) {
    ...
    SituationInitInfo config = {
        .window_title = "Quad Storm [Vulkan]",
    ...
    while (!SituationWindowShouldClose()) {
        ...
        if (fps_timer >= 0.5) {
            ...
            snprintf(title_buf, sizeof(title_buf),
                "QUAD STORM [VK] | %d quads | %.0f FPS | %.2f ms | VSync: %s | [UP/DOWN] +/-500 [V] vsync [SPACE] shuffle",
