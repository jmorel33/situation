/***************************************************************************************************
 * Superseded by examples/19_node_graph_piano/ — kept for dev reference.
 *  Situation — Node-Graph Piano Demo (Full Synth Edition)
 *  -------------------------------------------------------
 *  Signal chain: Tone Synth → Overdrive → Chorus → Phaser → Echo → Reverb → Gain
 *
 *  Three full octaves (C3–B5 = MIDI 48–83) across the keyboard, polyphonic,
 *  with proper key-down sustain via virtual MIDI NoteOn/Off.
 *
 ...
 *
 *  Master gain      =/−          (no KP – use =/- on main row for gain, already used by ADSR release)
 *    Actually: BACKSPACE/\       Master gain ±0.05  (0.1–2.0)
 *
 *  Build:
 *    build\build_examples.bat static-opengl  node_graph_piano_demo
 *    build\build_examples.bat static-vulkan  node_graph_piano_demo
 ***************************************************************************************************/
...
static SituationAudioGraph* build_graph(PianoNodes* n) {
    ...
    /* Overdrive: start bypassed (mix=0) */
    SituationSetControl(g, n->overdrive, 1, 40.0f); /* drive */
    SituationSetControl(g, n->overdrive, 4, 0.0f);  /* mix = 0 → dry */

    /* Chorus: start bypassed (wet_gain=0) */
    SituationSetControl(g, n->chorus, 18, 0.0f);    /* wet_gain (ctrl 18) */

    /* Phaser: start bypassed (mix=0) */
    SituationSetControl(g, n->phaser, 2, 0.0f);     /* mix */
...
static void apply_fx_preset(SituationAudioGraph* g, const PianoNodes* n, int preset) {
    /* Reset all effects wet to 0 first */
    SituationSetControl(g, n->overdrive, 4, 0.0f);  /* mix */
    SituationSetControl(g, n->chorus,   18, 0.0f);  /* wet_gain */
    SituationSetControl(g, n->phaser,    2, 0.0f);  /* mix */
    SituationSetControl(g, n->echo,      2, 0.0f);  /* wet_mix */
    SituationSetControl(g, n->reverb,    2, 0.0f);  /* wet */
    ...
    switch (preset) {
        ...
        case 2: /* Reverb only */
            ...
            break;
        case 3: /* Chorus */
            SituationSetControl(g, n->chorus, 18, 0.80f);
            break;
        case 4: /* Phaser */
            SituationSetControl(g, n->phaser, 2, 0.60f);
            break;
        case 5: /* Overdrive */
            SituationSetControl(g, n->overdrive, 4, 0.85f);
            break;
        case 6: /* All-in */
            SituationSetControl(g, n->overdrive, 4, 0.35f);
            SituationSetControl(g, n->chorus,   18, 0.45f);
            SituationSetControl(g, n->phaser,    2, 0.30f);
...
static void note_on(int midi_note, int raw_key) {
    ...
    /* Fallback direct tone (legacy pool) so we always have audible output even if the node-graph MIDI path
     * has issues. The graph's tone synth (driven by virtual MIDI) will also produce if working, with effects. */
    float hz = SITUATION_MIDI_NOTE_FREQUENCY[midi_note];
    SituationPlayToneEx((SituationWaveform)s_waveform, hz, 0.30f, 0.0f,
                        s_attack, s_decay, s_sustain, s_release, 0.0f);
}
...
static void draw_hud(SituationCommandBuffer cmd, int sw, int sh) {
    ...
    TXT(line, x, y, hi); y += dy + 4.0f;

    /* ── Piano keyboard visual (two rows) ── */
    /* Lower row label */
    TXT("Lower: Z S X D C V G B H N J M", x, y, dim); y += dy;

    /* Upper row */
    TXT("Upper: Q 2 W 3 E R 5 T 6 Y 7 U 8 I 9 O 0 P [ ]", x, y, dim); y += dy;
    ...
    if (any_held) TXT(held_buf, x, y, val);
    else          TXT("Held: (none)", x, y, dim);
    y += dy;
    ...
    TXT(fps, (float)sw - fpsw - 8.0f, 28.0f, (ColorRGBA){255, 240, 80, 255});
...
int main(int argc, char** argv) {
    if (SitExample_Init(argc, argv, "Situation — Full Synth Piano") != SITUATION_SUCCESS)
        return -1;

    /* Init device registry and build graph */
    ...
    /* Set up virtual MIDI loopback so NoteOn/Off reach the tone synth */
    int midi_in_id = -1;
    if (SituationSetupVirtualMidiLoopback(&midi_in_id) == SITUATION_SUCCESS) {
        SituationEnableMidiControl(graph, nodes.tone, midi_in_id);
        SituationSetNodeMidiChannel(graph, nodes.tone, 0);
    }
    ...
    /* Ensure sorted (some paths rely on explicit post-activation sort for first audio block) */
    if (SituationTopologicalSort(graph) != SITUATION_SUCCESS) {
        /* non-fatal, but helps guarantee audio flows on first block */
    }
    ...
    while (!SituationWindowShouldClose()) {
        ...
        if (SituationAcquireFrameCommandBuffer() == SITUATION_SUCCESS) {
            ...
            SitExample_DrawHUD(cmd,
                "Situation — Full Synth Piano",
                "Piano:Z/Q rows | ADSR:arrows+[]/+- | Sub:KP | LFO:KP | FX:F11 | Env:[/] | ESC");
    ...
    SituationSetActiveGraph(NULL); /* disable graph before destroying */
    SituationDestroyGraph(graph);
    SitExample_Shutdown();
    return 0;
}
