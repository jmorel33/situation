...
static void graphics_teardown(void) {
    if (g_init_ok) {
        ...
        sit_graphics_spirv_teardown_cached_meshes();
        SituationShutdown();
        g_init_ok = false;
    }
}
...
static void test_get_texture_handle(void) {
    ...
    // On Vulkan with descriptor indexing the slot_index is always valid for a live texture.
    // Slot 0 is the null/default slot; a freshly created texture must occupy a non-zero slot.
    SIT_ASSERT(handle != 0);
#elif defined(SITUATION_USE_OPENGL)
    if (SituationIsFeatureSupported(SIT_FEATURE_BINDLESS_TEXTURES)) {
        // ARB_bindless_texture is present — driver must return a non-zero resident handle.
        SIT_ASSERT(handle != 0);
    } else {
        // Extension absent — must return 0 (graceful skip, not a failure).
        SIT_ASSERT(handle == 0);
    }
#endif

    // A destroyed texture must yield 0 on any subsequent call (stale handle detection).
    SituationDestroyTexture(&tex);
    uint64_t stale = SituationGetTextureHandle(tex);
    SIT_ASSERT(stale == 0);
...
#if defined(SITUATION_USE_VULKAN)
    // On Vulkan bufferDeviceAddress is a hard requirement for compute pipelines —
    // any STORAGE_COMPUTE buffer must yield a valid non-zero GPU VA.
    SIT_ASSERT(addr != 0);
#elif defined(SITUATION_USE_OPENGL)
    if (SituationIsFeatureSupported(SIT_FEATURE_BINDLESS_BUFFERS)) {
        // NV extension present — address must be non-zero.
        SIT_ASSERT(addr != 0);
    } else {
        // Extension absent — must return 0 and set SITUATION_ERROR_OPENGL_UNSUPPORTED.
        SIT_ASSERT(addr == 0);
        SituationError last_err = SituationGetLastErrorCode();
        SIT_ASSERT(last_err == SITUATION_ERROR_OPENGL_UNSUPPORTED);
    }
#endif
...
static void test_mesh_vertex_buffer_address(void) {
    // Upload a tiny mesh with known vertex data
    float vertices[] = {
        0.0f, 0.5f, 0.0f,
       -0.5f,-0.5f, 0.0f,
        0.5f,-0.5f, 0.0f
...
#if defined(SITUATION_USE_VULKAN)
    // On Vulkan with BDA support all mesh buffers must carry a valid non-zero VA.
    SIT_ASSERT(vaddr != 0);
    SIT_ASSERT(iaddr != 0);
#elif defined(SITUATION_USE_OPENGL)
    if (SituationIsFeatureSupported(SIT_FEATURE_BINDLESS_BUFFERS)) {
        // NV extension present — both addresses must be non-zero.
        SIT_ASSERT(vaddr != 0);
        SIT_ASSERT(iaddr != 0);
    } else {
        // Extension absent — must return 0 + SITUATION_ERROR_MESH_DEVICE_ADDRESS_UNSUPPORTED.
        SIT_ASSERT(vaddr == 0);
        SIT_ASSERT(SituationGetLastErrorCode() == SITUATION_ERROR_MESH_DEVICE_ADDRESS_UNSUPPORTED);
        SIT_ASSERT(iaddr == 0);
        SIT_ASSERT(SituationGetLastErrorCode() == SITUATION_ERROR_MESH_DEVICE_ADDRESS_UNSUPPORTED);
    }
#endif

    // Stale handle must return 0.
    SituationDestroyMesh(&mesh);
    SIT_ASSERT(SituationGetMeshVertexBufferAddress(mesh) == 0);
    SIT_ASSERT(SituationGetMeshIndexBufferAddress(mesh) == 0);
}

static void test_mesh_index_buffer_address_vertex_only(void) {
    // Mesh without indices — index BDA must be 0 (no index buffer allocated).
    float vertices[] = {0.0f, 0.5f, 0.0f, -0.5f, -0.5f, 0.0f, 0.5f, -0.5f, 0.0f};
    SituationMesh mesh = {0};
    SituationError err = SituationCreateMesh(
        vertices, 3, sizeof(float) * 3, NULL, 0, &mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    uint64_t iaddr = SituationGetMeshIndexBufferAddress(mesh);
    SIT_ASSERT(iaddr == 0); // No index buffer — must be 0 on any backend.

    SituationDestroyMesh(&mesh);
}

#if defined(SITUATION_USE_VULKAN)
/** Phase B′4 — compute reads mesh vertex[0] via BDA push constant. */
static void test_mesh_bda_compute_read(void) {
    if (graphics_test_skip_vertex_pull_vulkan()) {
        return;
    }
    ...
    SituationBuffer out_buf = {0};
    err = graphics_test_create_ssbo(sizeof(vec3), &zero, &out_buf);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationComputePipeline pipeline = {0};
    err = SituationCreateComputePipelineFromMemory(
        graphics_test_glsl_cs_mesh_bda_read_vec3(),
    ...
    SituationCmdBindDescriptorSet(cmd, 0, out_buf);
    err = SituationCmdSetPushConstant(cmd, 0, &vaddr, sizeof(vaddr));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    err = SituationCmdDispatchEx(cmd, 1, 1, 1);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationBufferBarrierDesc barrier = {0};
    barrier.buffer = out_buf;
    barrier.offset = 0;
    barrier.size = sizeof(vec3);
    barrier.src_stages = SITUATION_PIPELINE_STAGE_COMPUTE_SHADER;
    barrier.src_access = SITUATION_ACCESS_SHADER_WRITE;
    barrier.dst_stages = SITUATION_PIPELINE_STAGE_TRANSFER;
    barrier.dst_access = SITUATION_ACCESS_TRANSFER_READ;
    err = SituationCmdBufferBarrier(cmd, &barrier);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    err = SituationEndFrame();
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    vec3 readback = {0.0f, 0.0f, 0.0f};
    err = SituationGetBufferData(out_buf, 0, sizeof(readback), &readback);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(fabsf(readback[0] - vertices[0]) < 0.001f);
    SIT_ASSERT(fabsf(readback[1] - vertices[1]) < 0.001f);
    SIT_ASSERT(fabsf(readback[2] - vertices[2]) < 0.001f);

    SituationDestroyComputePipeline(&pipeline);
    SituationDestroyBuffer(&out_buf);
    SituationDestroyMesh(&mesh);
}

/** Phase B′1 — vertex-pull render matches VAO attribute path (center pixel). */
static void test_vertex_pull_render(void) {
    if (graphics_test_skip_vertex_pull_vulkan()) {
        return;
    }
    ...
    SituationError err = SituationCreateMeshEx(
        quad_verts, 4, sizeof(float) * 3, quad_indices, 6, SIT_MESH_LAYOUT_PULL, &mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationMeshVertexLayout layout = (SituationMeshVertexLayout)-1;
    err = SituationGetMeshVertexLayout(mesh, &layout);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT_EQ((int)layout, (int)SIT_MESH_LAYOUT_PULL);

    SituationShader vao_shader = {0};
    err = SituationLoadShaderFromMemory(
        graphics_test_glsl_vs_passthrough(),
        graphics_test_glsl_fs_solid_red(),
        &vao_shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationShader pull_shader = {0};
    err = SituationLoadShaderFromMemory(
        graphics_test_glsl_vs_vertex_pull_simple(),
        graphics_test_glsl_fs_solid_red(),
        &pull_shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    err = graphics_test_render_mesh_red(vao_shader, mesh, false);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    uint8_t vao_rgba[4] = {0};
    err = graphics_test_read_center_pixel_rgba(vao_rgba);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    err = graphics_test_render_mesh_red(pull_shader, mesh, true);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    uint8_t pull_rgba[4] = {0};
    err = graphics_test_read_center_pixel_rgba(pull_rgba);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    for (int c = 0; c < 3; ++c) {
        SIT_ASSERT(graphics_test_pixel_approx_eq(vao_rgba[c], pull_rgba[c], 16));
    }
    SIT_ASSERT(vao_rgba[0] > 200);
    SIT_ASSERT(pull_rgba[0] > 200);

    SituationUnloadShader(&pull_shader);
    SituationUnloadShader(&vao_shader);
    SituationDestroyMesh(&mesh);
}
...
static void test_take_screenshot(void) {
    ...
    const char* path = sit_test_tmp("_sit_test_screenshot.png");
...
/**
 * Isolation test: unload-during-load immediately followed by begin+poll (no sync load).
 * Asserts the thread pool is quiescent after unload, then exercises the async path that
 * sync_shader_after_async_cycle relies on. Inserted before sync_shader to localize -557.
 */
static void test_async_shader_poll_after_unload_during_load(void) {
    SituationShader during = {0};
    SituationError err = SituationBeginLoadShaderFromMemory(
        graphics_test_glsl_vs_passthrough(),
        graphics_test_glsl_fs_solid_red(),
        &during);
    ...
    SituationUnloadShader(&during);
    SIT_ASSERT(during.generation == 0);

    SituationShader shader = {0};
    double t0 = sit_get_time_seconds();
    err = SituationBeginLoadShaderFromMemory(
        graphics_test_glsl_vs_passthrough(),
        graphics_test_glsl_fs_solid_red(),
        &shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    {
        double poll_ms = (sit_get_time_seconds() - t0) * 1000.0;
        if (poll_ms >= 500.0) {
            fprintf(stderr,
                "[async_shader_poll_after_unload_during_load] FAIL: reload poll took %.2f ms "
                "— expected < 500 ms (Layer A async fast path)\n", poll_ms);
        }
        SIT_ASSERT(poll_ms < 500.0);
        fprintf(stderr, "[async_shader_poll_after_unload_during_load] reload poll: %.3f ms\n", poll_ms);
    }
#endif
    SituationUnloadShader(&shader);
}
...
// ============================================================================
//  Multisample State Tests (v2.4.256)
// ============================================================================

/**
 * SituationCmdSetMultisampleState — structural correctness.
 *
 * Calls the function with three representative state combinations inside a
 * real render pass and verifies:
 *   1. Every call returns SITUATION_SUCCESS (GL: packets enqueued; VK: shadow stored).
 *   2. The subsequent draw still produces pixels — no state corruption.
 *   3. Resetting to the "all-default" state also succeeds.
 *
 * We do not assert specific pixel values for sample_shading or alpha_to_coverage
 * because both are no-ops on a single-sample default framebuffer.
 */
static void test_multisample_state_no_crash(void) {
    SituationShader shader = {0};
    SituationError err = SituationLoadShaderFromMemory(
        g_vs_passthrough, g_fs_solid_red, &shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    /* Variant A: sample shading on, min_shading = 1.0, mask = all, a2c off */
    SituationMultisampleState ms_a = {
        .sample_shading_enable    = true,
        .min_sample_shading       = 1.0f,
        .sample_mask              = 0,        /* 0 = all samples */
        .alpha_to_coverage_enable = false,
    };
    err = SituationCmdSetMultisampleState(cmd, &ms_a);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    err = SituationCmdDrawMesh(cmd, mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    /* Variant B: alpha_to_coverage on */
    SituationMultisampleState ms_b = {
        .sample_shading_enable    = false,
        .min_sample_shading       = 0.0f,
        .sample_mask              = 0,
        .alpha_to_coverage_enable = true,
    };
    err = SituationCmdSetMultisampleState(cmd, &ms_b);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    err = SituationCmdDrawMesh(cmd, mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    /* Variant C: full reset — everything off, all samples, no shading */
    SituationMultisampleState ms_reset = {
        .sample_shading_enable    = false,
        .min_sample_shading       = 0.0f,
        .sample_mask              = 0,
        .alpha_to_coverage_enable = false,
    };
    err = SituationCmdSetMultisampleState(cmd, &ms_reset);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    /* Verify the draw wasn't silently dropped — center pixel should be red */
    uint8_t rgba[4] = {0};
    err = graphics_test_read_center_pixel_rgba(rgba);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(rgba[0] > 200); /* R high */
...
/**
 * SituationCmdSetMultisampleState — push/pop raster stack symmetry.
 *
 * Push current raster state, set a non-default multisample state, draw,
 * then pop. The draw after pop must still produce red pixels (state was
 * cleanly restored) and must not crash.
 *
 * This is the critical regression guard: if push/pop doesn't save/restore
 * the new multisample fields in _SitGLRasterStackEntry, subsequent draws
 * may see stale GL enables, which on a single-sample surface is a no-op
 * visually but would indicate a bookkeeping bug.
 */
static void test_multisample_state_push_pop_symmetric(void) {
    SituationShader shader = {0};
    SituationError err = SituationLoadShaderFromMemory(
        g_vs_passthrough, g_fs_solid_red, &shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    /* Establish a known baseline: all multisample features off */
    SituationMultisampleState ms_off = {
        .sample_shading_enable    = false,
        .min_sample_shading       = 0.0f,
        .sample_mask              = 0,
        .alpha_to_coverage_enable = false,
    };
    err = SituationCmdSetMultisampleState(cmd, &ms_off);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    /* Push — saves the ms_off state alongside all other raster state */
    err = SituationCmdPushRasterState(cmd, 42u);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    /* Flip to a non-default state inside the bracket */
    SituationMultisampleState ms_on = {
        .sample_shading_enable    = true,
        .min_sample_shading       = 1.0f,
        .sample_mask              = 0xF0F0F0F0u,
        .alpha_to_coverage_enable = true,
    };
    err = SituationCmdSetMultisampleState(cmd, &ms_on);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    err = SituationCmdDrawMesh(cmd, mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    /* Pop — must restore to ms_off without crashing */
    err = SituationCmdPopRasterState(cmd, 42u);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    /* Draw after pop — must still produce pixels */
    err = SituationCmdDrawMesh(cmd, mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    /* Red pixels must survive the push/pop cycle */
    uint8_t rgba[4] = {0};
    err = graphics_test_read_center_pixel_rgba(rgba);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(rgba[0] > 200); /* R high */
...
/**
 * SituationGetGraphicsCaps — max_msaa_samples sanity.
 *
 * Verifies that max_msaa_samples is >= 1 on Vulkan, where it is now queried
 * from VkPhysicalDeviceLimits (fixed in v2.4.256 — was hardcoded to 1).
 *
 * On OpenGL the field is populated via glGetIntegerv(GL_MAX_SAMPLES) which
 * must be called on the GL context thread. SituationGetGraphicsCaps runs on
 * the main thread and therefore returns 0 for GL-context-dependent fields —
 * a pre-existing limitation documented in core.get_graphics_caps. This test
 * therefore only asserts on the Vulkan backend.
 */
static void test_multisample_quality_helpers(void) {
    SIT_ASSERT_EQ(SituationMultisampleQualitySampleCount(SITUATION_MULTISAMPLE_OFF), 1);
    SIT_ASSERT_EQ(SituationMultisampleQualitySampleCount(SITUATION_MULTISAMPLE_2X), 2);
    SIT_ASSERT_EQ(SituationMultisampleQualitySampleCount(SITUATION_MULTISAMPLE_4X), 4);
    SIT_ASSERT_EQ(SituationMultisampleQualityFromSampleCount(4), SITUATION_MULTISAMPLE_4X);
    SIT_ASSERT_EQ(SituationMultisampleQualityFromSampleCount(99), SITUATION_MULTISAMPLE_OFF);
    SituationMultisampleQuality clamped = SituationMultisampleQualityClamp(SITUATION_MULTISAMPLE_16X, 8);
    SIT_ASSERT_EQ(SituationMultisampleQualitySampleCount(clamped), 8);
}

static void test_multisample_caps_nonzero(void) {
    SituationGraphicsCaps caps = {0};
    SituationGetGraphicsCaps(&caps);
#if defined(SITUATION_USE_VULKAN)
    SIT_ASSERT(caps.max_msaa_samples >= 1);
#else
    /* GL: max_msaa_samples is GL-thread-dependent; just verify the call doesn't crash. */
    (void)caps.max_msaa_samples;
    SIT_ASSERT(true);
#endif
}

/** CI default: harness init forces 8-bit; caps and HDR flag stay consistent. */
static void test_output_color_depth_ci_default(void) {
    SituationGraphicsCaps caps = {0};
    SituationGetGraphicsCaps(&caps);
    ...
    if (g_sit_config.verbose) {
        fprintf(stderr, "[graphics] output_bits_per_channel=%u (harness SIT_OUTPUT_COLOR_8BIT)\n",
...
#if defined(SITUATION_USE_OPENGL)
/**
 * Phase 1 screen capture — RT urgent latch: EndFrame then Load without RequestScreenCapture.
 */
static bool graphics_draw_clear_frame_rgb(uint8_t r, uint8_t g, uint8_t b) {
    SituationPollInputEvents();
    SituationUpdateTimers();
    if (SituationAcquireFrameCommandBuffer() != SITUATION_SUCCESS) {
        return false;
    ...
    rp.color_attachment.loadOp = SIT_LOAD_OP_CLEAR;
    rp.color_attachment.storeOp = SIT_STORE_OP_STORE;
    rp.color_attachment.clear.color = (ColorRGBA){r, g, b, 255};
    rp.depth_attachment.loadOp = SIT_LOAD_OP_CLEAR;
    rp.depth_attachment.clear.depth = 1.0f;
...
static void test_gl_load_urgent_after_endframe(void) {
    SIT_ASSERT(graphics_draw_clear_frame_rgb(200, 40, 40));
    uint8_t rgba[4] = {0};
    SIT_ASSERT_EQ(graphics_test_read_center_pixel_rgba(rgba), SITUATION_SUCCESS);
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba[0], 200, 10));
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba[1], 40, 10));
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba[2], 40, 10));
}

/**
 * Phase 1 screen capture — ST sync-on-Load after gated EndFrame (render_thread_count = 0).
 * Re-inits with ST for this test only, then restores default RT setup for later tests.
 */
static void test_gl_load_after_endframe_st(void) {
    graphics_teardown();

    SituationInitInfo config = {0};
    sit_test_window_init_info(&config, "SIT_TEST_GRAPHICS_ST");
    ...
    SIT_ASSERT(graphics_draw_clear_frame_rgb(40, 180, 60));
    uint8_t rgba[4] = {0};
    SIT_ASSERT_EQ(graphics_test_read_center_pixel_rgba(rgba), SITUATION_SUCCESS);
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba[0], 40, 10));
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba[1], 180, 10));
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba[2], 60, 10));
...
static void test_descriptor_bind_ubo_color(void) {
    ...
    SituationUnloadShader(&shader);
    double t0 = sit_get_time_seconds();
    err = SituationLoadShaderFromMemory(g_vs_passthrough, g_fs_descriptor_ubo_color, &shader);
    ...
    if (reload_ms >= 3.0) {
        fprintf(stderr,
            "[descriptor_bind_ubo_color] FAIL: reload after unload took %.2f ms "
            "— expected < 3 ms (descriptor shader Layer A)\n", reload_ms);
    }
    SIT_ASSERT(reload_ms < 3.0);
    fprintf(stderr, "[descriptor_bind_ubo_color] reload after unload: %.3f ms\n", reload_ms);
#endif
...
static void test_texture_storage_write_readback(void) {
    ...
    // Barrier: compute writes → fragment sampling next frame (include TEXTURE_UPDATE for
    // NVIDIA stale texel reads — same mapping as compute_image_write / v2.4.327)
    SituationCmdPipelineBarrier(cmd,
        SITUATION_BARRIER_COMPUTE_SHADER_WRITE,
        SITUATION_BARRIER_FRAGMENT_SHADER_READ | SITUATION_BARRIER_TRANSFER_READ);
...
static void test_model_load_gltf(void) {
    ...
    // Write the .gltf file
    bool save_ok = SituationSaveFileText(sit_test_tmp("_sit_test_triangle.gltf"), gltf_json);
    if (!save_ok) {
    ...
    SituationError err = SituationSaveFileData(sit_test_tmp("_sit_test_triangle.bin"), bin_data, sizeof(bin_data));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    // Load the model
    SituationModel model = {0};
    err = SituationLoadModel(sit_test_tmp("_sit_test_triangle.gltf"), &model);
    ...
    // Cleanup temp files
    SituationDeleteFile(sit_test_tmp("_sit_test_triangle.gltf"));
    SituationDeleteFile(sit_test_tmp("_sit_test_triangle.bin"));
}
...
static void test_model_draw_verify(void) {
    ...
    bool save_ok = SituationSaveFileText(sit_test_tmp("_sit_test_model_draw.gltf"), gltf_json);
    if (!save_ok) {
        SIT_ASSERT(true);
        return;
    }
    ...
    memcpy(bin_data, verts, 36);
    memcpy(bin_data + 36, idxs, 6);
    SituationError err = SituationSaveFileData(sit_test_tmp("_sit_test_model_draw.bin"), bin_data, sizeof(bin_data));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationModel model = {0};
    err = SituationLoadModel(sit_test_tmp("_sit_test_model_draw.gltf"), &model);
    if (err != SITUATION_SUCCESS) {
        SituationDeleteFile(sit_test_tmp("_sit_test_model_draw.gltf"));
        SituationDeleteFile(sit_test_tmp("_sit_test_model_draw.bin"));
        SIT_ASSERT(true);
        return;
    }
...
static void test_model_save_as_gltf(void) {
    ...
    bool save_ok = SituationSaveFileText(sit_test_tmp("_sit_test_export_src.gltf"), gltf_json);
    if (!save_ok) {
        SIT_ASSERT(true);
        return;
    }
    ...
    memcpy(bin_data, verts, 36);
    memcpy(bin_data + 36, idxs, 6);
    SituationError err = SituationSaveFileData(sit_test_tmp("_sit_test_export_src.bin"), bin_data, sizeof(bin_data));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationModel model = {0};
    err = SituationLoadModel(sit_test_tmp("_sit_test_export_src.gltf"), &model);
    if (err != SITUATION_SUCCESS) {
        SituationDeleteFile(sit_test_tmp("_sit_test_export_src.gltf"));
        SituationDeleteFile(sit_test_tmp("_sit_test_export_src.bin"));
        SIT_ASSERT(true);
        return;
    }

    // Export to a new file
    bool export_ok = (SituationSaveModelAsGltf(model, sit_test_tmp("_sit_test_exported.gltf")) == SITUATION_SUCCESS);
    SIT_ASSERT(export_ok);

    // Verify the exported file exists
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_exported.gltf")));

    // Load the exported file as text and verify it starts with '{' (valid JSON)
    char* exported_text = SituationLoadFileText(sit_test_tmp("_sit_test_exported.gltf"));
    if (exported_text) {
    ...
    SituationDeleteFile(sit_test_tmp("_sit_test_export_src.gltf"));
    SituationDeleteFile(sit_test_tmp("_sit_test_export_src.bin"));
    SituationDeleteFile(sit_test_tmp("_sit_test_exported.gltf"));
    SituationDeleteFile(sit_test_tmp("_sit_test_exported.bin"));
}
...
static void test_model_unload_safety(void) {
    ...
    bool save_ok = SituationSaveFileText(sit_test_tmp("_sit_test_unload.gltf"), gltf_json);
    if (!save_ok) {
        SIT_ASSERT(true);
        return;
    }
    ...
    memcpy(bin_data, verts, 36);
    memcpy(bin_data + 36, idxs, 6);
    SituationError err = SituationSaveFileData(sit_test_tmp("_sit_test_unload.bin"), bin_data, sizeof(bin_data));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    SituationModel model = {0};
    err = SituationLoadModel(sit_test_tmp("_sit_test_unload.gltf"), &model);
    if (err != SITUATION_SUCCESS) {
        SituationDeleteFile(sit_test_tmp("_sit_test_unload.gltf"));
        SituationDeleteFile(sit_test_tmp("_sit_test_unload.bin"));
        SIT_ASSERT(true);
        return;
    }
...
/**
 * Phase 27 — SPIR-V SSBO reflection sanity.
 * After SituationLoadShaderFromMemory for the dual-SSBO shaders, query
 * GL program interface and assert BlockA -> binding 0, BlockB -> binding 1.
 * Catches the v2.4.81 regression where both blocks reported GL_BUFFER_BINDING 0.
 */
static void test_spirv_ssbo_reflection_unique_bindings(void) {
    if (graphics_test_skip_if_no_spirv()) {
        return;
    }

    SituationShader shader = {0};
    SituationError err = SituationLoadShaderFromMemory(g_vs_passthrough, g_fs_dual_ssbo_tags, &shader);
    if (err != SITUATION_SUCCESS) {
    ...
    /* --- Count-only query first --- */
    int block_count = 0;
    err = SituationQueryShaderStorageBlocks(shader, NULL, 0, &block_count);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    /* If reflection finds no blocks the driver may have stripped them; skip gracefully. */
    if (block_count == 0) {
        fprintf(stderr, "[test] spirv_ssbo_reflection: no active SSBO blocks reported by driver (skip)\n");
        SituationUnloadShader(&shader);
        SIT_ASSERT(true);
        return;
    }
    SIT_ASSERT(block_count == 2);

    /* --- Full query --- */
    enum { kMaxBlocks = 8 };
    SituationShaderStorageBlockInfo blocks[kMaxBlocks];
    memset(blocks, 0, sizeof(blocks));
    int filled = 0;
    err = SituationQueryShaderStorageBlocks(shader, blocks, kMaxBlocks, &filled);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(filled == 2);

    /* Binding points must be unique (0 and 1 in some order). */
    bool has_binding_0 = false;
    bool has_binding_1 = false;
    for (int i = 0; i < filled; i++) {
        if (blocks[i].binding_point == 0) has_binding_0 = true;
        if (blocks[i].binding_point == 1) has_binding_1 = true;
    }
    SIT_ASSERT(has_binding_0);
    SIT_ASSERT(has_binding_1);

    /* BlockA (block_index 0) must be at binding 0, BlockB (block_index 1) at binding 1.
     * _SituationBindGLProgramStorageBlocks assigns bindings in index order, preserving
     * the layout(binding=N) intent even when SPIR-V reflection reports all blocks at 0. */
    SIT_ASSERT(blocks[0].binding_point == 0);
    SIT_ASSERT(blocks[1].binding_point == 1);

    /* Log names for diagnostics (SPIR-V may report empty or mangled names). */
    fprintf(stderr, "[test] spirv_ssbo_reflection: block[0] name='%s' binding=%u, block[1] name='%s' binding=%u\n",
            blocks[0].name, blocks[0].binding_point,
            blocks[1].name, blocks[1].binding_point);
    fflush(stderr);
...
static void test_demon_hunt_sky_shader_link(void) {
    ...
    static const char* vs_paths[] = {"tests/harness/assets/demon_hunt_sky.vs", "build/examples/demon_hunt_sky.vs", "demon_hunt_sky.vs", NULL};
    static const char* fs_paths[] = {"tests/harness/assets/demon_hunt_sky.fs", "build/examples/demon_hunt_sky.fs", "demon_hunt_sky.fs", NULL};
...
static void test_demon_hunt_sky_spirv_begin_poll(void) {
    ...
    static const char* vs_paths[] = {
        "tests/harness/assets/demon_hunt_sky.vs.spv",
    ...
    static const char* fs_paths[] = {
        "tests/harness/assets/demon_hunt_sky.fs.devel.spv",
        "build/examples/demon_hunt_sky.fs.devel.spv",
        "tests/harness/assets/demon_hunt_sky.fs.spv",
...
// ============================================================================
//  Vulkan Shader Cache Tests (Gate 1D)
//  Plan: doc/plan/VULKAN_SHADER_CACHE_PLAN.md
//  Exit criteria: Phase 1 §Phase 1 exit criteria
// ============================================================================

/* Shared GLSL sources for all cache tests — Vulkan 450 / GL 460.
 * Using graphics_test_glsl_vs_passthrough() / graphics_test_glsl_fs_solid_red()
 * because those are the harness minimal shaders the plan references. */

/**
 * V1 — shader_cache_hit
 * Load the same GLSL source twice. Second load must be < 3 ms on Vulkan+shaderc.
 * On OpenGL or no-shaderc: just verifies no crash and correct handle.
 * Plan exit criterion 1: second load < 3 ms on GTX 1070 and RTX 2080.
 */
static void test_shader_cache_hit(void) {
    const char* vs = graphics_test_glsl_vs_passthrough();
    const char* fs = graphics_test_glsl_fs_solid_red();

    /* First load — populates Layer A / B / C. */
    SituationShader shader1 = {0};
    SituationError err = SituationLoadShaderFromMemory(vs, fs, &shader1);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(shader1.generation != 0 || shader1.slot_index != 0);

    /* Second load — must hit cache (Layer A → C). */
    double t0 = sit_get_time_seconds();
    SituationShader shader2 = {0};
    err = SituationLoadShaderFromMemory(vs, fs, &shader2);
    double elapsed_ms = (sit_get_time_seconds() - t0) * 1000.0;
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(shader2.generation != 0 || shader2.slot_index != 0);

#if defined(SITUATION_USE_VULKAN) && defined(SITUATION_ENABLE_SHADER_COMPILER) \
    && defined(SIT_VK_SHADER_CACHE_ENABLE) && SIT_VK_SHADER_CACHE_ENABLE
    /* Hard timing gate: < 3 ms on reference hardware. */
    if (elapsed_ms >= 3.0) {
        fprintf(stderr,
            "[shader_cache_hit] FAIL: second load took %.2f ms — expected < 3 ms "
            "(cache hit should skip shaderc + pipeline build)\n", elapsed_ms);
    }
    SIT_ASSERT(elapsed_ms < 3.0); /* second load of same GLSL must be < 3 ms (cache hit) */
    fprintf(stderr, "[shader_cache_hit] second load: %.3f ms\n", elapsed_ms);
#elif defined(SITUATION_USE_OPENGL) && SIT_GL_SHADER_CACHE_ENABLE
    if (elapsed_ms >= 3.0) {
        fprintf(stderr,
            "[shader_cache_hit] FAIL: second load took %.2f ms — expected < 3 ms "
            "(OpenGL program cache hit should skip compile/link)\n", elapsed_ms);
    }
    SIT_ASSERT(elapsed_ms < 3.0);
    fprintf(stderr, "[shader_cache_hit] second load: %.3f ms\n", elapsed_ms);
#endif

    SituationUnloadShader(&shader1);
    SituationUnloadShader(&shader2);
}

/**
 * V2 — shader_cache_reuse_after_unload
 * Load → draw (pixel readback red) → unload → load same source → draw (pixel readback red).
 * Both draws must produce a red centre pixel. Exercises the full reuse path including
 * bundle_ref / layout propagation to CmdBindPipeline.
 * Plan exit criterion 2.
 */
static void test_shader_cache_reuse_after_unload(void) {
    const char* vs = graphics_test_glsl_vs_passthrough();
    const char* fs = graphics_test_glsl_fs_solid_red();
    SituationMesh mesh = graphics_test_fullscreen_mesh();
    SIT_ASSERT(mesh.generation != 0 || mesh.slot_index != 0);
    ...
    SituationCommandBuffer cmd1 = graphics_test_begin_frame();
    SIT_ASSERT_NOT_NULL(cmd1);
    err = graphics_test_clear_and_draw(cmd1, shader1, mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    uint8_t rgba1[4] = {0};
    err = graphics_test_read_center_pixel_rgba(rgba1);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba1[0], 255, 16)); /* red ~255 */
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba1[1], 0, 16));   /* green ~0 */

    SituationUnloadShader(&shader1);

    /* --- Second load (must revive EVICT_PENDING bundle) + draw --- */
    double t0 = sit_get_time_seconds();
    SituationShader shader2 = {0};
    err = SituationLoadShaderFromMemory(vs, fs, &shader2);
    double reload_ms = (sit_get_time_seconds() - t0) * 1000.0;
...
#if (defined(SITUATION_USE_VULKAN) && defined(SITUATION_ENABLE_SHADER_COMPILER) \
     && defined(SIT_VK_SHADER_CACHE_ENABLE) && SIT_VK_SHADER_CACHE_ENABLE) \
    || (defined(SITUATION_USE_OPENGL) && SIT_GL_SHADER_CACHE_ENABLE)
    if (reload_ms >= 3.0) {
        fprintf(stderr,
            "[shader_cache_reuse_after_unload] FAIL: reload after unload took %.2f ms "
            "— expected < 3 ms (retention revival)\n", reload_ms);
    }
    SIT_ASSERT(reload_ms < 3.0);
    fprintf(stderr, "[shader_cache_reuse_after_unload] reload after unload: %.3f ms\n", reload_ms);
#endif

    SituationCommandBuffer cmd2 = graphics_test_begin_frame();
    SIT_ASSERT_NOT_NULL(cmd2);
    err = graphics_test_clear_and_draw(cmd2, shader2, mesh);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    uint8_t rgba2[4] = {0};
    err = graphics_test_read_center_pixel_rgba(rgba2);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba2[0], 255, 16)); /* red ~255 after cache reuse */
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba2[1], 0, 16));   /* green ~0 after cache reuse */

    SituationUnloadShader(&shader2);
}

/**
 * V3 — shader_cache_concurrent_loads
 * Two BeginLoad of the same source; poll both to SUCCESS. Phase 1: correctness only
 * (may compile shaderc twice — dedup deferred to Phase 2 tickets).
 * Plan exit criterion 3.
 */
static void test_shader_cache_concurrent_loads(void) {
    const char* vs = graphics_test_glsl_vs_passthrough();
    const char* fs = graphics_test_glsl_fs_solid_red();

    SituationShader shaderA = {0};
    SituationShader shaderB = {0};

    SituationError errA = SituationBeginLoadShaderFromMemory(vs, fs, &shaderA);
    SIT_ASSERT_EQ(errA, SITUATION_SUCCESS);
    SituationError errB = SituationBeginLoadShaderFromMemory(vs, fs, &shaderB);
    SIT_ASSERT_EQ(errB, SITUATION_SUCCESS);

    /* Poll both to completion. */
    SIT_ASSERT_EQ(graphics_test_async_poll_shader_ready(shaderA, 600), SITUATION_SUCCESS);
    SIT_ASSERT_EQ(graphics_test_async_poll_shader_ready(shaderB, 600), SITUATION_SUCCESS);

    /* Both handles must be valid after concurrent load. */
    SIT_ASSERT(shaderA.generation != 0 || shaderA.slot_index != 0);
    SIT_ASSERT(shaderB.generation != 0 || shaderB.slot_index != 0);

    SituationUnloadShader(&shaderA);
    SituationUnloadShader(&shaderB);
}

/**
 * V9 — shader_cache_no_memory_growth
 * 50 load/unload cycles of the same GLSL. VRAM must not grow after cycle 2
 * (bundle evicted and not re-leaked). Exercises the full load → unload → eviction loop.
 * Plan exit criteria 6 + 7.
 */
static void test_shader_cache_no_memory_growth(void) {
    const char* vs = graphics_test_glsl_vs_passthrough();
    const char* fs = graphics_test_glsl_fs_solid_red();

    /* Warm up: two cycles to prime any one-time allocations. */
    for (int i = 0; i < 2; i++) {
        SituationShader shader = {0};
        SIT_ASSERT_EQ(SituationLoadShaderFromMemory(vs, fs, &shader), SITUATION_SUCCESS);
        SituationUnloadShader(&shader);
        /* Pump a frame so eviction runs and graveyard flushes. */
        SituationPollInputEvents();
        SituationUpdateTimers();
        if (SituationAcquireFrameCommandBuffer() == SITUATION_SUCCESS) {
            SituationEndFrame();
    ...
    uint64_t vram_baseline = SituationGetVRAMUsage();

    /* 50 load/unload cycles. */
    for (int i = 0; i < 50; i++) {
        SituationShader shader = {0};
        SIT_ASSERT_EQ(SituationLoadShaderFromMemory(vs, fs, &shader), SITUATION_SUCCESS);
        SituationUnloadShader(&shader);
        /* One frame pump per cycle — eviction + graveyard. */
        SituationPollInputEvents();
        SituationUpdateTimers();
        if (SituationAcquireFrameCommandBuffer() == SITUATION_SUCCESS) {
            SituationEndFrame();
    ...
    uint64_t vram_after = SituationGetVRAMUsage();

    /* Allow a small slack (256 KiB) for driver/allocator rounding. */
    const uint64_t slack = 256u * 1024u;
    if (vram_baseline > 0 && vram_after > vram_baseline + slack) {
        fprintf(stderr,
            "[shader_cache_no_memory_growth] baseline=%llu after=%llu delta=%llu bytes\n",
            (unsigned long long)vram_baseline,
            (unsigned long long)vram_after,
            (unsigned long long)(vram_after - vram_baseline));
    }
    /* On OpenGL, SituationGetVRAMUsage() may return 0 — skip VRAM check in that case. */
    if (vram_baseline > 0) {
        SIT_ASSERT(vram_after <= vram_baseline + slack); /* VRAM must not grow over 50 cycles */
    }
    SIT_ASSERT(true); /* Always passes on GL — confirms no crash over 50 cycles. */
}

/**
 * V7 — shader_cache_hot_reload
 * Plan verification matrix V7: "mtime bump; handle stable"
 *
 * Writes VS+FS to temp files, loads via SituationLoadShader (records paths).
 * Round 1 (no-op): write identical GLSL back, call SituationReloadShader — handle must
 *   remain valid, pixel test must pass (content_hash unchanged → no GPU work).
 * Round 2 (real change): write different FS (green output), reload — handle still valid,
 *   pixel must now be green (new bundle acquired in-place).
 *
 * Vulkan+shaderc only — GL path does a different async recompile; skipped gracefully.
 */
static void test_shader_cache_hot_reload(void) {
#if defined(SITUATION_USE_VULKAN) && defined(SITUATION_ENABLE_SHADER_COMPILER)
    const char* vs_glsl =
        "#version 450\n"
        "layout(location=0) in vec3 aPos;\n"
        "void main() { gl_Position = vec4(aPos, 1.0); }\n";
    const char* fs_red_glsl =
        "#version 450\n"
        "layout(location=0) out vec4 fragColor;\n"
        "void main() { fragColor = vec4(1.0, 0.0, 0.0, 1.0); }\n";
    const char* fs_green_glsl =
        "#version 450\n"
        "layout(location=0) out vec4 fragColor;\n"
        "void main() { fragColor = vec4(0.0, 1.0, 0.0, 1.0); }\n";

    const char* vs_path = sit_test_tmp("_sit_cache_hot_vs.glsl");
    const char* fs_path = sit_test_tmp("_sit_cache_hot_fs.glsl");

    /* Write initial shader files. */
    SIT_ASSERT_EQ(SituationSaveFileText(vs_path, vs_glsl), SITUATION_SUCCESS);
    SIT_ASSERT_EQ(SituationSaveFileText(fs_path, fs_red_glsl), SITUATION_SUCCESS);

    SituationShader shader = {0};
    SituationError err = SituationLoadShader(vs_path, fs_path, &shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(shader.generation != 0 || shader.slot_index != 0);

    uint32_t original_slot  = shader.slot_index;
    uint32_t original_gen   = shader.generation;

    SituationMesh mesh = graphics_test_fullscreen_mesh();
    SIT_ASSERT(mesh.generation != 0 || mesh.slot_index != 0);

    /* --- Round 1: no-op reload (identical GLSL → same SPIR-V → content_hash match) --- */
    SIT_ASSERT_EQ(SituationSaveFileText(fs_path, fs_red_glsl), SITUATION_SUCCESS);
    err = SituationReloadShader(&shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    /* Handle must still point at the same slot (in-place swap preserves identity). */
    SIT_ASSERT(shader.slot_index == original_slot);
    SIT_ASSERT(shader.generation == original_gen);

    /* Draw and verify red pixel. */
    SituationCommandBuffer cmd = graphics_test_begin_frame();
    SIT_ASSERT_NOT_NULL(cmd);
    SIT_ASSERT_EQ(graphics_test_clear_and_draw(cmd, shader, mesh), SITUATION_SUCCESS);
    uint8_t rgba1[4] = {0};
    SIT_ASSERT_EQ(graphics_test_read_center_pixel_rgba(rgba1), SITUATION_SUCCESS);
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba1[0], 255, 16)); /* red ~255 */
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba1[1], 0, 20));   /* green ~0 */

    /* --- Round 2: real change (green FS → new SPIR-V → new bundle, same handle) --- */
    SIT_ASSERT_EQ(SituationSaveFileText(fs_path, fs_green_glsl), SITUATION_SUCCESS);
    err = SituationReloadShader(&shader);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    /* Handle still valid — the public SituationShader.slot_index is unchanged by in-place swap.
     * Note: SituationShader.generation is the SLOT generation (resource registry), not the
     * bundle's internal generation. _SitVkAttachBundleRef writes to vk_bundle_ref.generation
     * (internal), leaving the public handle generation intact. Slot identity is preserved. */
    SIT_ASSERT(shader.slot_index == original_slot);
    ...
    SIT_ASSERT_EQ(graphics_test_clear_and_draw(cmd, shader, mesh), SITUATION_SUCCESS);
    uint8_t rgba2[4] = {0};
    SIT_ASSERT_EQ(graphics_test_read_center_pixel_rgba(rgba2), SITUATION_SUCCESS);
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba2[1], 255, 16)); /* green ~255 */
    SIT_ASSERT(graphics_test_pixel_approx_eq(rgba2[0], 0, 20));   /* red ~0 */

    SituationUnloadShader(&shader);
    SituationDeleteFile(vs_path);
    SituationDeleteFile(fs_path);
#else
    /* GL / no-shaderc: just verify the API doesn't crash on NULL path slot. */
    SIT_ASSERT(true);
#endif
}
...
static SitTestCase graphics_tests[] = {
    ...
    {"mesh_vertex_buffer_address",      test_mesh_vertex_buffer_address,    true},
    {"mesh_index_buffer_address_vertex_only", test_mesh_index_buffer_address_vertex_only, true},
#if defined(SITUATION_USE_VULKAN)
    {"mesh_bda_compute_read",           test_mesh_bda_compute_read,         true},
    {"vertex_pull_render",              test_vertex_pull_render,            true},
#endif
    // Shaders
    ...
    {"async_shader_poll_after_unload_during_load", test_async_shader_poll_after_unload_during_load, true},
    {"sync_shader_after_async_cycle",   test_sync_shader_after_async_cycle, true},
    // Shader Cache (Gate 1D — Phase 1 exit criteria)
    {"shader_cache_hit",                test_shader_cache_hit,              true},
    {"shader_cache_reuse_after_unload", test_shader_cache_reuse_after_unload, true},
    {"shader_cache_concurrent_loads",   test_shader_cache_concurrent_loads, true},
    {"shader_cache_no_memory_growth",   test_shader_cache_no_memory_growth, true},
    // Shader Cache V7 — Phase 3 hot-reload (plan verification matrix)
    {"shader_cache_hot_reload",         test_shader_cache_hot_reload,       true},
    // Textures
    ...
    {"multisample_state_no_crash",        test_multisample_state_no_crash,        true},
    {"multisample_state_push_pop",        test_multisample_state_push_pop_symmetric, true},
    {"multisample_quality_helpers",         test_multisample_quality_helpers,         true},
    {"multisample_caps_nonzero",          test_multisample_caps_nonzero,          true},
    {"output_color_depth_ci_default",     test_output_color_depth_ci_default,     true},
...
#if defined(SITUATION_USE_OPENGL)
    {"gl_load_urgent_after_endframe",   test_gl_load_urgent_after_endframe,   true},
    {"gl_load_after_endframe_st",       test_gl_load_after_endframe_st,       true},
#endif
    ...
    {"spirv_ssbo_reflection_bindings",    test_spirv_ssbo_reflection_unique_bindings, true},
...
#endif
    {"pattern_smpte_vd_bar_color",        test_pattern_smpte_vd_bar_color,        true},
    {"pattern_smpte_full_bar_color",      test_pattern_smpte_full_bar_color,      true},
    {"pattern_checkerboard_corner",       test_pattern_checkerboard_corner,       true},
    {"pattern_grid_line",                 test_pattern_grid_line,                 true},
    {"pattern_pluge_black_bar",           test_pattern_pluge_black_bar,           true},
    {"pattern_crosshatch_center",         test_pattern_crosshatch_center,         true},
    {"pattern_convergence_moire_zone",    test_pattern_convergence_moire_zone,    true},
    {"pattern_multiburst_bands",          test_pattern_multiburst_bands,          true},
    {"pattern_cube_lit_faces",              test_pattern_cube_lit_faces,              true},
    {"pattern_compose_checker_plus_smpte", test_pattern_compose_checker_plus_smpte, true},
    {"pattern_zero_layers_noise",         test_pattern_zero_layers_noise,         true},
    {"pattern_chroma_snow",               test_pattern_chroma_snow,               true},
    {"pattern_config_defaults",           test_pattern_config_defaults,           true},
    {"pattern_layer_params_checker_tile", test_pattern_layer_params_checker_tile, true},
    {"pattern_layer_params_convergence_stripe", test_pattern_layer_params_convergence_stripe, true},
#if defined(SITUATION_USE_VULKAN) && defined(SITUATION_ENABLE_SHADER_COMPILER)
    {"pattern_runtime_include_compile",   test_pattern_runtime_include_compile,   true},
#endif
};
