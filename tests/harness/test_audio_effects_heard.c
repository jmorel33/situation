 ...
 *
 * Also includes a reverb dry/wet mix sweep: 440 Hz square, wet 0→1 (2 s), hold (1 s),
 * then 1→0 (2 s), verified via RMS envelope at dry vs wet windows.
 *
...
static bool sit_effect_heard_capture_tone_only(int sample_rate, SitAudioFreqCapture* cap) {
    ...
    sit_effect_heard_capture(cap, 400);
...
static void sit_effect_heard_run_named(const char* test_name, const SitEffectHeardSpec* spec) {
    ...
    if (!sit_effect_heard_capture_tone_only(sr, &dry)) {
        sit_test_listen_overlay_pump_ms(120);
...
static bool sit_effect_heard_capture_square_tone_only(int sample_rate, SitAudioFreqCapture* cap) {
    ...
    SituationSetAudioMasterVolume(0.8f);
    sit_test_audio_monitor_set_capture(cap);
    sit_effect_heard_capture(cap, 1000);
...
static void test_graph_tone_synth_reverb_mix_dry_wet_sweep(void) {
    ...
    const uint32_t ramp_up_ms = 2000u;
    const uint32_t hold_wet_ms = 1000u;
    const uint32_t ramp_down_ms = 2000u;
    const uint32_t total_ms = ramp_up_ms + hold_wet_ms + ramp_down_ms;
    const uint32_t window_ms = 450u;
    const uint32_t window_frames = (uint32_t)(((uint64_t)window_ms * (uint32_t)sr) / 1000u);
    const uint32_t capture_frames = (uint32_t)(((uint64_t)(total_ms + 300) * (uint32_t)sr) / 1000u);
    ...
    SituationSetAudioMasterVolume(0.8f);
    sit_test_audio_monitor_set_capture(&sweep);
    sit_audio_freq_capture_reset(&sweep);
