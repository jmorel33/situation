...
#if defined(_WIN32)
static bool sit_test_get_process_app_id_utf8(char* out, size_t out_sz) {
    if (!out || out_sz == 0) return false;
    out[0] = '\0';
    HMODULE mod = GetModuleHandleW(L"shell32.dll");
    if (!mod) return false;
    typedef HRESULT (WINAPI *PFN_GetCurrentProcessExplicitAppUserModelID)(PWSTR*);
    PFN_GetCurrentProcessExplicitAppUserModelID pfn =
        (PFN_GetCurrentProcessExplicitAppUserModelID)(void*)GetProcAddress(
            mod, "GetCurrentProcessExplicitAppUserModelID");
    if (!pfn) return false;
    PWSTR id = NULL;
    if (FAILED(pfn(&id)) || !id) return false;
    int need = WideCharToMultiByte(CP_UTF8, 0, id, -1, out, (int)out_sz, NULL, NULL);
    CoTaskMemFree(id);
    return need > 0;
}

static void test_win32_app_user_model_id_default(void) {
    char id[128];
    SIT_ASSERT(sit_test_get_process_app_id_utf8(id, sizeof(id)));
    SIT_ASSERT(strcmp(id, SITUATION_DEFAULT_APP_USER_MODEL_ID) == 0);
}

static void test_win32_set_app_user_model_id_invalid(void) {
    SituationError err = SituationWin32SetAppUserModelId("");
    SIT_ASSERT(err == SITUATION_ERROR_INVALID_PARAM);
    err = SituationWin32SetAppUserModelId(NULL);
    SIT_ASSERT(err == SITUATION_ERROR_INVALID_PARAM);
    err = SituationWin32SetAppUserModelId("has space");
    SIT_ASSERT(err == SITUATION_ERROR_INVALID_PARAM);
}
#endif
...
#if defined(_WIN32)
    {"win32_app_user_model_id_default", test_win32_app_user_model_id_default, true},
    {"win32_set_app_user_model_id_invalid", test_win32_set_app_user_model_id_invalid, true},
#endif
};
