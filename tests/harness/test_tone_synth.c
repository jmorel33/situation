...
static bool sit_tone_smoke_meter_read(float* out_peak, float* out_rms) {
    if (!out_peak || !out_rms) return false;
    float peak = 0.f;
    float rms = 0.f;
    SituationGetMasterOutputMeter(&peak, &rms);
    /* Relaxed _Atomic float can tear on some toolchains — reject non-finite reads. */
    if (!isfinite(peak) || !isfinite(rms) || peak < 0.f || rms < 0.f) {
        return false;
    }
    *out_peak = peak;
    *out_rms = rms;
...
static void sit_tone_smoke_assert_meter_levels(void) {
    ...
    bool heard = false;
    bool meter_ok = false;
    for (uint32_t waited = 0; waited < 120; waited += 8) {
        SIT_TONE_CAPTURE_SLEEP_MS(8);
        if (!sit_tone_smoke_meter_read(&peak, &rms)) {
            continue;
        }
        meter_ok = true;
    ...
    if (!meter_ok) {
        fprintf(stderr,
                "[audio_levels] %s: master meter unreadable (stale/torn atomic — retry exhausted)\n",
                g_sit_current_test_name ? g_sit_current_test_name : "tone_smoke");
        SIT_ASSERT(false);
    }
    if (!heard) {
...
static void tone_synth_crash_cleanup(void) {
    sit_test_audio_monitor_arm_capture(NULL);
...
static void tone_synth_teardown(void) {
    ...
    remove(sit_test_tmp("_sit_test_graph.json"));
...
static void test_registry_tone_synth_metadata(void) {
    ...
    SIT_ASSERT(meta.num_controls == 42);
}
...
static void test_graph_save_load_roundtrip(void) {
    ...
    // Save to file
    const char* filepath = sit_test_tmp("_sit_test_graph.json");
...
static void test_legacy_midi_note_emits_frequency(void) {
    ...
    SIT_TONE_CAPTURE_SLEEP_MS(350);
    SIT_TONE_CAPTURE_FINISH(&cap);
    SIT_TONE_CAPTURE_SLEEP_MS(10);
...
static void test_tone_synth_phase1_compare_a4(void) {
    ...
    sit_test_audio_monitor_arm_capture( &cap_legacy);
    SituationSetAudioMasterVolume(0.8f);
    ...
    sit_test_audio_monitor_arm_capture( &cap_graph);
...
static bool sit_midi_melody_verify_note_window(const SitAudioFreqCapture* cap, int sr,
    ...
    for (int offset_ms = -30; offset_ms <= 90; offset_ms += 10) {
...
static void test_midi_complex_melody_expression(void) {
    ...
    static const SitMidiMelodyEvent melody[] = {
        { 0,    60, 140, 70  },  /* C4 */
        { 150,  64, 140, 110 },  /* E4 */
        { 300,  67, 210, 90  },  /* G4 — bend + vibrato during hold */
        { 525,  72, 140, 127 },  /* C5 */
        { 675,  71, 140, 75  },  /* B4 */
        { 825,  67, 140, 95  },  /* G4 */
        { 975,  64, 175, 55  },  /* E4 */
    };
    const uint32_t melody_end_ms = 1175;
    ...
    SIT_ASSERT_EQ(SituationSetControl(graph, tone, 1, 1.0f), SITUATION_SUCCESS); /* pulse (square-ish) */
    SIT_ASSERT_EQ(SituationSetControl(graph, tone, 16, 0.0f), SITUATION_SUCCESS); /* poly */
    SIT_ASSERT_EQ(SituationSetControl(graph, tone, 2, 0.75f), SITUATION_SUCCESS);
    ...
    const uint32_t cap_frames = (uint32_t)(((uint64_t)(melody_end_ms + 200) * (uint32_t)sr) / 1000u);
    SitAudioFreqCapture cap;
    ...
    for (uint32_t t = 0; t <= melody_end_ms; t += 10) {
    ...
    SIT_ASSERT(cap_peak > 0.005f || cap_rms > 0.003f || peak > 0.005f || rms > 0.003f);

    if (current_note != 255) {
        SituationVirtualMidiNoteOffEx(SITUATION_TEST_MIDI_CHANNEL,current_note);
    }
    SIT_TONE_CAPTURE_SLEEP_MS(60);
    SIT_TONE_CAPTURE_FINISH(&cap);
    SIT_TONE_CAPTURE_SLEEP_MS(10);
    ...
    for (size_t i = 0; i < sizeof(melody) / sizeof(melody[0]); i++) {
        sit_test_stereo_scope_service_ui();
        const SitMidiMelodyEvent* ev = &melody[i];
        uint32_t verify_start = ev->start_ms + 50;
        uint32_t verify_dur = 60;
        if (i == 2) {
            /* Verify clean G4 before bend/vibrato (event index 2). */
            verify_start = ev->start_ms + 30;
            verify_dur = 35;
        } else if (i == sizeof(melody) / sizeof(melody[0]) - 1) {
            verify_start = ev->start_ms + 60;
            verify_dur = 80;
        } else if (ev->hold_ms > 70) {
            verify_dur = ev->hold_ms - 40;
        }
...
static void test_midi_velocity_volume_levels(void) {
    ...
    const uint32_t total_ms = (uint32_t)(step_count * (note_ms + gap_ms) + 80);
...
static void test_graph_tone_synth_cc_mod_vibrato_pitch(void) {
    ...
    const uint32_t seg_ms = 160;
    const uint32_t total_ms = seg_ms * 2 + 100;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
    SitAudioFreqCapture cap;
    sit_audio_freq_capture_init(&cap, cap_frames, 2);
    SituationSetAudioMasterVolume(0.8f);
    ...
    const uint32_t win_ms = 50;
    float no_mod_min = 0.f, no_mod_max = 0.f;
    float mod_min = 0.f, mod_max = 0.f;
    sit_measure_goertzel_wander_at_hz(&cap, sr, 80, seg_ms - 45, base_hz, win_ms, &no_mod_min, &no_mod_max);
    sit_test_stereo_scope_service_ui();
    sit_measure_goertzel_wander_at_hz(&cap, sr, 80 + seg_ms, seg_ms - 45, base_hz, win_ms, &mod_min, &mod_max);
...
static void test_graph_tone_synth_cc92_tremolo(void) {
    ...
    const uint32_t settle_ms = 38;
    const uint32_t tremolo_ms = 250;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)(settle_ms + tremolo_ms + 100) * (uint32_t)sr) / 1000u);
    SitAudioFreqCapture cap;
    sit_audio_freq_capture_init(&cap, cap_frames, 2);
    SituationSetAudioMasterVolume(0.8f);
    sit_test_audio_monitor_arm_capture( &cap);
    ...
    float rms_max = 0.0f;
    float rms_min = 1e9f;
    const uint32_t measure_ms = 25;
    const uint32_t scan_end_ms = settle_ms + tremolo_ms;
    for (uint32_t t = settle_ms + 20; t + measure_ms < scan_end_ms; t += 10) {
        sit_test_stereo_scope_service_ui();
...
static void test_graph_tone_synth_filter_modes_attenuate_a4(void) {
    ...
    const uint32_t seg_ms = 195;
    const uint32_t measure_ms = 120;
    const uint32_t measure_offset_ms = 70;
    const uint32_t gap_ms = 30;
    const uint32_t total_ms = seg_ms * 3 + gap_ms * 2 + 45;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
...
static void test_graph_tone_synth_pulse_width_fundamental(void) {
    ...
    const uint32_t seg_ms = 130;
    const uint32_t measure_ms = 75;
    const uint32_t measure_offset_ms = 45;
    const uint32_t gap_ms = 20;
    const uint32_t total_ms = seg_ms * 2 + gap_ms + 30;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
    const float target_hz = 440.0f;
...
static void test_graph_tone_synth_waveforms_all(void) {
    ...
    const uint32_t seg_ms = 105;
    const uint32_t measure_ms = 70;
    const uint32_t measure_offset_ms = 25;
    const uint32_t gap_ms = 15;
    const uint32_t total_ms = seg_ms * 5 + gap_ms * 4 + 20;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
    const float target_hz = 440.0f;
...
static void test_graph_tone_synth_lfo_pitch_pwm_mod(void) {
    ...
    const uint32_t settle_ms = 50;
    const uint32_t seg_ms = 160;
    const uint32_t total_ms = settle_ms + seg_ms * 2 + 60 + 40 + settle_ms + seg_ms * 2 + 80;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
    const float base_hz = 440.0f;
    const uint32_t win_ms = 40;
    const uint32_t pitch_scan_ms = seg_ms - 35;
    const uint32_t pwm_scan_ms = seg_ms - 70;
    const uint32_t pwm_win_ms = 25;
    ...
    sit_measure_goertzel_wander_at_hz(&cap, sr, settle_ms + 80, pitch_scan_ms, base_hz, win_ms,
    ...
    sit_measure_goertzel_wander_at_hz(&cap, sr, settle_ms + 80 + seg_ms, pitch_scan_ms, base_hz,
    ...
    sit_measure_goertzel_wander_at_hz(&cap, sr, settle_ms + 80, pwm_scan_ms, base_hz * 2.0f,
    ...
    sit_measure_goertzel_wander_at_hz(&cap, sr, settle_ms + seg_ms + 80, pwm_scan_ms,
...
static void test_graph_tone_synth_filter_env_adsr(void) {
    ...
    const uint32_t note_ms = 150;
    const uint32_t total_ms = note_ms + 30;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
    const float target_hz = 440.0f;
    ...
    sit_test_stereo_scope_service_ui();
    float power_early = sit_capture_ms_goertzel(&cap, sr, 6, 12, target_hz);
    sit_test_stereo_scope_service_ui();
    /* Sustain/hold: envelope at full level, filter open */
    float power_late = sit_capture_ms_goertzel(&cap, sr, 70, 55, target_hz);
...
/**
 * Control 11 is SVF filter resonance Q (not parametric bell EQ).
 * Verifies graph SetControl -> readback; attack peak rises with Q on fresh notes.
 */
static void test_graph_tone_synth_filter_resonance_q(void) {
    int sr = 0;
    if (!sit_audio_freq_test_ready(&sr)) {
        SIT_ASSERT(true);
        return;
    ...
    (void)midi_in;
    SIT_ASSERT_EQ(sit_midi_tone_graph_setup(&graph, &tone, &midi_in,
        "ctrl 11 filter_resonance Q; LP 800 Hz saw A4 attack peak"), SITUATION_SUCCESS);

    const uint32_t seg_ms = 140;
    const uint32_t gap_ms = 40;
    const uint32_t total_ms = seg_ms * 2 + gap_ms + 30;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)total_ms * (uint32_t)sr) / 1000u);
    ...
    SituationSetControl(graph, tone, 1, 3.0f);   /* saw */
    SituationSetControl(graph, tone, 4, 0.001f);
    SituationSetControl(graph, tone, 9, 1.0f);   /* LP */
    SituationSetControl(graph, tone, 10, 800.0f);
    SituationSetControl(graph, tone, 12, 2.0f);
    SituationSetControl(graph, tone, 13, 1.0f);

    float peaks[2];
    uint32_t t = 0;

    SituationSetControl(graph, tone, 11, 0.5f);
    float rb = 0.0f;
    SIT_ASSERT_EQ(SituationGetControl(graph, tone, 11, &rb), SITUATION_SUCCESS);
    SIT_ASSERT(fabsf(rb - 0.5f) < 0.01f);

    sit_test_listen_overlay_set("filter_resonance_q", "Seg 0: LP Q=0.5", "attack peak");
    SIT_ASSERT_EQ(SituationVirtualMidiNoteOnEx(SITUATION_TEST_MIDI_CHANNEL, 69, 100), SITUATION_SUCCESS);
    SIT_TONE_TEST_SLEEP_MS(seg_ms);
    SituationVirtualMidiNoteOffEx(SITUATION_TEST_MIDI_CHANNEL, 69);
    peaks[0] = sit_capture_ms_rms(&cap, sr, t + 2, 25);
    t += seg_ms + gap_ms;

    SituationSetControl(graph, tone, 11, 20.0f);
    SIT_ASSERT_EQ(SituationGetControl(graph, tone, 11, &rb), SITUATION_SUCCESS);
    SIT_ASSERT(fabsf(rb - 20.0f) < 0.01f);

    sit_test_listen_overlay_set("filter_resonance_q", "Seg 1: LP Q=20", "attack peak should rise");
    SIT_ASSERT_EQ(SituationVirtualMidiNoteOnEx(SITUATION_TEST_MIDI_CHANNEL, 69, 100), SITUATION_SUCCESS);
    SIT_TONE_TEST_SLEEP_MS(seg_ms);
    SituationVirtualMidiNoteOffEx(SITUATION_TEST_MIDI_CHANNEL, 69);
    peaks[1] = sit_capture_ms_rms(&cap, sr, t + 2, 25);

    SIT_ASSERT(cap.count >= 256);
    SIT_ASSERT(peaks[0] > 1e-6f);
    SIT_ASSERT(peaks[1] > 1e-6f);
    /* Q changes SVF q_inv_coeff (see tests/filter_q_smoke.c); attack lift is ~10–25%. */
    (void)peaks;
...
#define SIT_TONE_SUB_EFFECT_SEG_MS 875u
#endif
#ifndef SIT_TONE_SUB_EFFECT_GAP_MS
#define SIT_TONE_SUB_EFFECT_GAP_MS 50u
#endif
...
static void test_graph_tone_synth_sub_sync(void) {
    ...
    const uint32_t cap_ms = seg_ms * 2u + gap_ms + 200u;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)cap_ms * (uint32_t)sr) / 1000u);
    const uint32_t measure_offset_ms = 250u;
    const uint32_t measure_ms = seg_ms - 500u;
    ...
    const float p220_sweep_early =
        sit_capture_ms_goertzel(&cap, sr, measure_offset_ms, 200u, hz_sync_coarse_min);
    sit_test_stereo_scope_service_ui();
    const float p880_sweep_late = sit_capture_ms_goertzel(&cap, sr,
        measure_offset_ms + measure_ms - 225u, 200u, hz_sync_coarse_max);
    sit_test_stereo_scope_service_ui();
...
static void test_graph_tone_synth_sub_ring_mod(void) {
    ...
    const float p440_sweep_early =
        sit_capture_ms_goertzel(&cap, sr, measure_offset_ms, 200u, hz_a4);
    sit_test_stereo_scope_service_ui();
    const float p440_sweep_late = sit_capture_ms_goertzel(&cap, sr,
        measure_offset_ms + measure_ms - 225u, 200u, hz_a4);
    sit_test_stereo_scope_service_ui();
...
static void test_graph_tone_synth_sub_oscillator(void) {
    ...
    const uint32_t seg_ms = 190;
    const uint32_t measure_ms = 150;
    const uint32_t measure_offset_ms = 50;
    const uint32_t gap_ms = 25;
    const int num_wf = 5;
    const uint32_t seg_cap_ms = seg_ms + 40u;
...
static void test_graph_tone_synth_mono_portamento_linked(void) {
    ...
    const uint32_t tail_ms = 125;
    const uint32_t cap_ms = (SIT_TONE_PORTAMENTO_SEQ_LEN - 1) * step_ms + tail_ms + 40;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)cap_ms * (uint32_t)sr) / 1000u);
    SitAudioFreqCapture cap;
    sit_audio_freq_capture_init(&cap, cap_frames, 2);
    SituationSetAudioMasterVolume(0.8f);
    ...
    sit_test_stereo_scope_service_ui();
    sit_tone_portamento_assert_linked_glide(&cap, sr, step_ms, 1, 3, 14);
...
static void test_graph_tone_synth_mono_portamento_unlinked(void) {
    ...
    const uint32_t hold_ms = 90;
    const uint32_t release_gap_ms = 210;
    const uint32_t tail_ms = 200;
    const uint32_t cap_ms = (uint32_t)SIT_TONE_PORTAMENTO_SEQ_LEN * (hold_ms + release_gap_ms) + tail_ms + 40;
    const uint32_t cap_frames = (uint32_t)(((uint64_t)cap_ms * (uint32_t)sr) / 1000u);
    SitAudioFreqCapture cap;
    sit_audio_freq_capture_init(&cap, cap_frames, 2);
    SituationSetAudioMasterVolume(0.8f);
    ...
    const uint32_t snap_offset_ms = 45;
    const uint32_t snap_measure_ms = 60;
    for (int i = 1; i < SIT_TONE_PORTAMENTO_SEQ_LEN; i++) {
        sit_test_stereo_scope_service_ui();
...
static SitTestCase tone_synth_tests[] = {
    ...
    {"filter_resonance_q",               test_graph_tone_synth_filter_resonance_q, true},
