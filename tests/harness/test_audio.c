...
#define TEST_WAV_PATH sit_test_tmp("_sit_test_sine.wav")
...
static void audio_teardown(void) {
    ...
    remove(sit_test_tmp("_sit_test_session.json"));
    remove(sit_test_tmp("_sit_test_midi.json"));
...
// ============================================================================
//  Format-Specific Playback Tests (MP3, OGG, FLAC, WAV)
// ============================================================================

static void audio_require_harness_asset(const char* filename, char* out_path, size_t out_path_sz) {
    if (sit_test_resolve_harness_asset(filename, out_path, out_path_sz)) {
        return;
    }
    char msg[256];
    snprintf(msg, sizeof(msg), "harness asset %s not found", filename);
    SIT_TEST_SKIP(msg);
}

static void test_load_play_mp3(void) {
    char path[512];
    audio_require_harness_asset("sample.mp3", path, sizeof(path));

    SituationSound sound = SITUATION_NULL_HANDLE;
    SituationError err = SituationLoadSoundFromFile(
        path, SITUATION_AUDIO_LOAD_FULL, false, &sound);
...
static void test_load_play_ogg(void) {
    char path[512];
    audio_require_harness_asset("sample.ogg", path, sizeof(path));
    ...
    if (err == SITUATION_ERROR_AUDIO_DECODER_INIT_FAILED ||
        ...
        SIT_TEST_SKIP("OGG/Vorbis decoder not available");
    }
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(sound.slot_index != 0 || sound.generation != 0);
...
static void test_load_play_flac(void) {
    char path[512];
    audio_require_harness_asset("sample.flac", path, sizeof(path));
...
static void test_stream_mp3(void) {
    ...
    SituationSound sound = SITUATION_NULL_HANDLE;
    SituationError err = SituationLoadSoundFromFile(
        path, SITUATION_AUDIO_LOAD_STREAM, false, &sound);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
...
static void test_load_play_wav(void) {
    char path[512];
    audio_require_harness_asset("sample.wav", path, sizeof(path));
...
static void test_midi_save_preset(void) {
    ...
    SituationError err = SituationSaveMidiPreset(graph, reverb, sit_test_tmp("_sit_test_midi.json"));
    // Graceful — no crash regardless of return code
    SIT_ASSERT(true);
...
static void test_midi_load_preset(void) {
    ...
    FILE* f = fopen(sit_test_tmp("_sit_test_midi.json"), "w");
    if (f) {
        fprintf(f, "{\"mappings\":[]}");
        fclose(f);
    }
    ...
    SituationError err = SituationLoadMidiPreset(graph, reverb, sit_test_tmp("_sit_test_midi.json"));
    // Graceful — no crash regardless of return code
    SIT_ASSERT(true);
