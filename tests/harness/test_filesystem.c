 ...
 * All temp files use the "_sit_test_" prefix under build/tests/scratch/.
 *
...
static void test_file_exists_positive(void) {
    ...
    SituationError err = SituationSaveFileText(sit_test_tmp("_sit_test_text.txt"), "hello");
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_text.txt")));
    SituationDeleteFile(sit_test_tmp("_sit_test_text.txt"));
}

static void test_file_exists_negative(void) {
    SIT_ASSERT(!SituationFileExists(sit_test_tmp("_sit_test_nonexistent_xyz_12345.txt")));
}

static void test_directory_exists(void) {
    SituationError err = SituationCreateDirectory(sit_test_tmp("_sit_test_dir"), false);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(SituationDirectoryExists(sit_test_tmp("_sit_test_dir")));
    SituationDeleteDirectory(sit_test_tmp("_sit_test_dir"), false);
    SIT_ASSERT(!SituationDirectoryExists(sit_test_tmp("_sit_test_dir")));
}

static void test_get_file_mod_time(void) {
    SituationSaveFileText(sit_test_tmp("_sit_test_text.txt"), "modtime test");
    long mod_time = SituationGetFileModTime(sit_test_tmp("_sit_test_text.txt"));
...
static void test_save_load_text_roundtrip(void) {
    ...
    SituationError err = SituationSaveFileText(sit_test_tmp("_sit_test_text.txt"), content);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);

    char* loaded = SituationLoadFileText(sit_test_tmp("_sit_test_text.txt"));
...
static void test_save_load_binary_roundtrip(void) {
    ...
    SituationError err = SituationSaveFileData(sit_test_tmp("_sit_test_binary.bin"), data, size);
    SIT_ASSERT_EQ(err, 0); // SITUATION_SUCCESS

    unsigned int bytes_read = 0;
    unsigned char* loaded = NULL;
    err = SituationLoadFileData(sit_test_tmp("_sit_test_binary.bin"), &bytes_read, &loaded);
    SIT_ASSERT_EQ(err, 0);
    SIT_ASSERT_NOT_NULL(loaded);
    ...
    SituationDeleteFile(sit_test_tmp("_sit_test_binary.bin"));
}

static void test_copy_file(void) {
    SituationSaveFileText(sit_test_tmp("_sit_test_text.txt"), "copy me");
    SituationError err = SituationCopyFile(sit_test_tmp("_sit_test_text.txt"), sit_test_tmp("_sit_test_copy.txt"));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_copy.txt")));

    char* content = SituationLoadFileText(sit_test_tmp("_sit_test_copy.txt"));
    SIT_ASSERT_NOT_NULL(content);
    SIT_ASSERT_STR_EQ(content, "copy me");
    free(content);

    SituationDeleteFile(sit_test_tmp("_sit_test_text.txt"));
    SituationDeleteFile(sit_test_tmp("_sit_test_copy.txt"));
}

static void test_move_file(void) {
    SituationSaveFileText(sit_test_tmp("_sit_test_text.txt"), "move me");
    SituationError err = SituationMoveFile(sit_test_tmp("_sit_test_text.txt"), sit_test_tmp("_sit_test_moved.txt"));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(!SituationFileExists(sit_test_tmp("_sit_test_text.txt")));
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_moved.txt")));

    char* content = SituationLoadFileText(sit_test_tmp("_sit_test_moved.txt"));
    SIT_ASSERT_NOT_NULL(content);
    SIT_ASSERT_STR_EQ(content, "move me");
    free(content);

    SituationDeleteFile(sit_test_tmp("_sit_test_moved.txt"));
}

static void test_delete_file(void) {
    SituationSaveFileText(sit_test_tmp("_sit_test_text.txt"), "delete me");
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_text.txt")));
    SituationError err = SituationDeleteFile(sit_test_tmp("_sit_test_text.txt"));
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(!SituationFileExists(sit_test_tmp("_sit_test_text.txt")));
}
...
static void test_create_delete_directory(void) {
    ...
    err = SituationDeleteDirectory(sit_test_tmp("_sit_test_dir"), false);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(!SituationDirectoryExists(sit_test_tmp("_sit_test_dir")));
}

static void test_create_directory_with_parents(void) {
    SituationError err = SituationCreateDirectory(sit_test_tmp("_sit_test_dir/sub/deep"), true);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(SituationDirectoryExists(sit_test_tmp("_sit_test_dir/sub/deep")));

    // Recursive delete
    err = SituationDeleteDirectory(sit_test_tmp("_sit_test_dir"), true);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    SIT_ASSERT(!SituationDirectoryExists(sit_test_tmp("_sit_test_dir")));
}

static void test_list_directory_files(void) {
    SituationCreateDirectory(sit_test_tmp("_sit_test_dir"), false);
    SituationSaveFileText(sit_test_tmp("_sit_test_dir/_sit_test_a.txt"), "a");
    SituationSaveFileText(sit_test_tmp("_sit_test_dir/_sit_test_b.txt"), "b");

    int count = 0;
    char** files = SituationListDirectoryFiles(sit_test_tmp("_sit_test_dir"), &count);
    SIT_ASSERT_NOT_NULL(files);
    SIT_ASSERT(count >= 2);

    SituationFreeDirectoryFileList(files, count);
    SituationDeleteDirectory(sit_test_tmp("_sit_test_dir"), true);
}
...
static void test_load_file_async(void) {
    ...
    SituationError err = SituationSaveFileText(sit_test_tmp("_sit_test_async.txt"), content);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    SituationJobId job = SituationLoadFileAsync(&pool, sit_test_tmp("_sit_test_async.txt"), async_load_callback, NULL);
    ...
    SituationDeleteFile(sit_test_tmp("_sit_test_async.txt"));
}
...
static void test_load_file_text_async(void) {
    ...
    SituationError err = SituationSaveFileText(sit_test_tmp("_sit_test_async_text.txt"), content);
    SIT_ASSERT_EQ(err, SITUATION_SUCCESS);
    ...
    SituationJobId job = SituationLoadFileTextAsync(&pool, sit_test_tmp("_sit_test_async_text.txt"), async_text_load_callback, NULL);
    SIT_ASSERT_NEQ(job, 0);
    ...
    SituationDestroyThreadPool(&pool);
    SituationDeleteFile(sit_test_tmp("_sit_test_async_text.txt"));
}
...
static void test_save_file_async(void) {
    ...
    SituationJobId job = SituationSaveFileAsync(&pool, sit_test_tmp("_sit_test_async_save.bin"), data, sizeof(data), async_save_callback, NULL);
    SIT_ASSERT_NEQ(job, 0);
    ...
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_async_save.bin")));

    SituationDestroyThreadPool(&pool);
    SituationDeleteFile(sit_test_tmp("_sit_test_async_save.bin"));
}
...
static void test_save_file_text_async(void) {
    ...
    SituationJobId job = SituationSaveFileTextAsync(&pool, sit_test_tmp("_sit_test_async_save_text.txt"), "hello async world", async_save_callback, NULL);
    SIT_ASSERT_NEQ(job, 0);
    ...
    SIT_ASSERT(g_async_save_done);
    SIT_ASSERT(g_async_save_success);
    SIT_ASSERT(SituationFileExists(sit_test_tmp("_sit_test_async_save_text.txt")));

    // Verify content
    char* loaded = SituationLoadFileText(sit_test_tmp("_sit_test_async_save_text.txt"));
    SIT_ASSERT_NOT_NULL(loaded);
    SIT_ASSERT_STR_EQ(loaded, "hello async world");
    free(loaded);

    SituationDestroyThreadPool(&pool);
    SituationDeleteFile(sit_test_tmp("_sit_test_async_save_text.txt"));
}
