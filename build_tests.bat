...
REM
REM Prerequisites:
REM   Static: build_situation.bat static-opengl (or static-vulkan)
REM   DLL:    build_situation.bat opengl (or vulkan)
REM
REM Output: build\tests\sit_test.exe (OpenGL) or sit_test_vulkan.exe (Vulkan)
REM
...
echo         Note: run via run_tests.bat or add build\dll to PATH
echo.
...
echo [SUCCESS] %BUILD_DIR%\%TEST_EXE% (Vulkan, DLL-linked — use run_tests.bat to run)
goto :done
...
echo   opengl          DLL-linked OpenGL (faster build, use run_tests.bat to run)
echo   vulkan          DLL-linked Vulkan (faster build, use run_tests.bat to run)
echo.
echo Output:
...
echo   opengl          ^> build_situation.bat opengl
echo   vulkan          ^> build_situation.bat vulkan
echo.
exit /b 1
