...
param(
    ...
    # Optional: path to UBO_SSBO_SAMPLER FS SPIR-V blob.
    # When supplied, emits ${Prefix}_ubo_ssbo_sampler_fs_spv / _len arrays.
    # When omitted, emits zero-length stub arrays so the embed header always compiles.
    [string]$FsSamplerSpv = ""
)
...
# Optional UBO_SSBO_SAMPLER FS blob — emitted only for VK builds.
# When -FsSamplerSpv is absent or the file doesn't exist, emit a zero-length stub
# so the embed header's extern declaration always links (the test guards on _len > 0).
$sw.WriteLine('')
if ($FsSamplerSpv -ne '' -and (Test-Path -LiteralPath $FsSamplerSpv)) {
    $fsSampler = [System.IO.File]::ReadAllBytes((Resolve-Path -LiteralPath $FsSamplerSpv).Path)
    Write-SpirvArray "${Prefix}_ubo_ssbo_sampler_fs_spv" $fsSampler
} else {
    $sw.WriteLine("const unsigned char ${Prefix}_ubo_ssbo_sampler_fs_spv[] = {0};")
    $sw.WriteLine("const size_t ${Prefix}_ubo_ssbo_sampler_fs_spv_len = 0;")
}
