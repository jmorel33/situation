...
EXPECTED_VARS = {
    ...
    "harness_ubo_ssbo_sampler_vk.fs.spv": [(0, 0), (1, 0), (2, 0)],  # set0=Frame UBO, set1=TagBlock SSBO, set2=sampler
}
...
def main():
    ...
    rc = 0
    for name in (
        "harness_dual_ssbo_vk.fs.spv",
        "harness_ubo_ssbo_vk.fs.spv",
        "harness_ubo_ssbo_sampler_vk.fs.spv",
    ):
        p = root / name
        if not p.is_file():
