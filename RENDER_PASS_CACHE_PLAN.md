...
**Actionables:**
- [ ] Create a `union _SituationRenderPassKey` packed into a `uint32_t`.
- [ ] Define bitfields inside the union for `target_type` (1 bit), `color_load_op` (2 bits), `depth_load_op` (2 bits), `color_store_op` (2 bits), and `depth_store_op` (2 bits).
- [ ] Write a static inline helper `_SituationHashRenderPassKey(SituationRenderPassInfo* info, bool is_main_window)` to pack the struct fields into the 32-bit key.
...
**Actionables:**
- [ ] Define a new internal struct `_SituationCachedRenderPass` containing `uint32_t key` and `VkRenderPass handle`.
- [ ] Add `_SituationCachedRenderPass render_pass_cache[32];` to `_SituationVulkanState`.
- [ ] Add `uint32_t render_pass_cache_count;` to `_SituationVulkanState` and ensure it initializes to `0`.
...
**Actionables:**
- [ ] Verify `SituationRenderPassInfo` includes explicit fields for Stencil operations (if supported) to ensure parity with Depth operations.
- [ ] Ensure clear values (`clear_color`, `clear_depth`) are accurately represented as unions matching Vulkan's `VkClearValue`.
...
**Actionables:**
- [ ] Implement the signature: `VkRenderPass _SituationVulkanGetOrCreateRenderPass(_SituationVulkanState* vk_state, const SituationRenderPassInfo* info)`.
- [ ] Generate the `_SituationRenderPassKey`.
- [ ] Iterate over `render_pass_cache` up to `render_pass_cache_count`. Return the `VkRenderPass` handle immediately if a matching key is found.
- [ ] **Cache Miss Logic:** If not found, dynamically construct `VkAttachmentDescription` and `VkSubpassDescription` based on the requested operations.
- [ ] Call `vkCreateRenderPass`.
- [ ] Store the newly created handle and key in the cache, and increment `render_pass_cache_count`. 
- [ ] Add an assertion or fallback warning if `render_pass_cache_count` exceeds 32.
...
**Actionables:**
- [ ] Remove the existing `TODO` and fallback routing to `SituationCmdBeginRenderToDisplay`.
- [ ] Call `_SituationVulkanGetOrCreateRenderPass()` to resolve the `VkRenderPass`.
- [ ] Retrieve the correct `VkFramebuffer` (from `main_window_framebuffers[current_image_index]` or the specific VD slot).
- [ ] Populate `VkRenderPassBeginInfo` with the resolved Render Pass, Framebuffer, and Clear Values.
- [ ] Call `vkCmdBeginRenderPass`.
- [ ] **Crucial:** Automatically inject `vkCmdSetViewport` and `vkCmdSetScissor` covering the full framebuffer dimensions immediately after beginning the pass, maintaining API convenience.
- [ ] **Crucial:** Ensure the `ViewDataUBO` memory mapped updates are still occurring during pass commencement for the main window to ensure independent 2D rendering calls continue functioning correctly.
...
**Actionables:**
- [ ] Audit the `SIT_OP_BEGIN_RENDER_PASS` packet handling in `_SituationGLExecuteCommands`.
- [ ] Check `info.color_attachment.loadOp`. If it equals `SIT_LOAD_OP_LOAD`, strictly bypass `glClear(GL_COLOR_BUFFER_BIT)`.
- [ ] Apply the same exact logic independently to the Depth attachment (`GL_DEPTH_BUFFER_BIT`) and Stencil attachment (`GL_STENCIL_BUFFER_BIT`).
...
**Actionables:**
- [ ] Locate `SituationRenderVirtualDisplays` (or equivalent compositing logic).
- [ ] Update the logic to use `SituationCmdBeginRenderPass` with `SIT_LOAD_OP_LOAD` when advanced blending (Photoshop-style modes) requires accumulating multiple passes onto the same target.
- [ ] Ensure the final blit/accumulation onto the main backbuffer respects the `SIT_LOAD_OP_LOAD` contract to avoid wiping the main scene.

### 4.2 Deprecate the Legacy API
Maintaining two code paths for rendering is a vector for bugs. We must unify them.

**Actionables:**
- [ ] Mark `SituationCmdBeginRenderToDisplay` as deprecated in the public header (`situation_api.h`) using compiler-specific deprecation macros (e.g., `__attribute__((deprecated))` or `[[deprecated]]`).
- [ ] Rewrite the internal implementation of `SituationCmdBeginRenderToDisplay`. It should no longer write raw command packets.
- [ ] Make the legacy function a simple wrapper that constructs a `SituationRenderPassInfo` (hardcoding `LOAD_OP_CLEAR` for backwards compatibility) and immediately passes it to `SituationCmdBeginRenderPass`.
...
**Actionables:**
- [ ] Add a `VkSubpassDependency` array when calling `vkCreateRenderPass`.
- [ ] Set `srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT` and `dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT`.
- [ ] Set `srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT` and `dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT`.
- [ ] Repeat for Depth/Stencil stages (`VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT`).

### 5.2 Stencil Buffer Support
The API defines `clear.stencil`, but the operations focus heavily on Color/Depth. 

**Actionables:**
- [ ] Ensure Stencil `loadOp` and `storeOp` are explicitly mapped in the cache generator (`_SituationVulkanGetOrCreateRenderPass`).
- [ ] Ensure `VkAttachmentDescription.stencilLoadOp` and `stencilStoreOp` are populated correctly, rather than defaulting to `DONT_CARE`.
...
**Actionables:**
- [ ] **Monitor & Profile:** If the duration exceeds ~1-2ms, it will cause noticeable hitches. Given the maximum limit of permutations inside the 32 cache entries, the small penalty will happen smoothly. (Fallback handled).
