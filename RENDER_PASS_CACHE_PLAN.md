...
[x] **Rule 2:** If `loadOp == SIT_LOAD_OP_LOAD`, `initialLayout` must be `VK_IMAGE_LAYOUT_PRESENT_SRC_KHR` for the main window, and `VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL` for Virtual Displays. (Depth remains `VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL`).
...
[x] **Pipeline Compatibility:** Ensure `VkPipeline` creation continues to use the persistent template render passes (`sit_render.vk.main_window_render_pass` and `vd->vk.render_pass`), NOT the dynamically cached ones, so pipelines survive cache flushes.
...
[x] **Contingency Plan (If hitching occurs):** Refactor to move the cache lookup and `vkCreateRenderPass` call to the *Main Thread* (during the `SituationCmdBeginRenderPass` API call). Pass the resolved `VkRenderPass` handle directly through the Momentum queue to the Render Thread.
